...
export function AIProvider({ children }: AIProviderProps) {
  ...
  const processPrompt = useCallback(
    async (prompt: string, promptType: string = 'general', language: string = 'en'): Promise<AIResponse> => {
      ...
      if (!aiEnabled) {
        return {
          ...
          tier: 'worker',
          creditsUsed: 0,
          error: 'AI is disabled. Enable it to use AI features.',
        };
      }
  ...
  const generateChart = useCallback(
    async (prompt: string, data: Record<string, unknown>[]): Promise<ChartResponse> => {
      // Check if AI is enabled
      if (!aiEnabled) {
  ...
  const generateTable = useCallback(
    async (prompt: string, data: Record<string, unknown>[]): Promise<TableResponse> => {
      if (!aiEnabled) {
        return { success: false, error: 'AI is disabled. Enable it to use AI features.' };
      }
  ...
  const generateForecast = useCallback(
    async (prompt: string, data: Record<string, unknown>[], periods: number = 6): Promise<ForecastResponse> => {
      if (!aiEnabled) {
        return { success: false, error: 'AI is disabled. Enable it to use AI features.' };
      }
  ...
  const generateTips = useCallback(
    async (focusArea?: string): Promise<TipsResponse> => {
      if (!aiEnabled) {
        return { success: false, error: 'AI is disabled. Enable it to use AI features.' };
      }
  ...
  // Extract GTD tasks
  const extractGTDTasks = useCallback(
    async (input: string): Promise<GTDTaskResponse> => {
      if (!aiEnabled) {
        return { success: false, error: 'AI is disabled. Enable it to use AI features.' };
      }
  ...
  const uploadFile = useCallback(
    async (fileData: FileUploadData): Promise<FileMappingResponse> => {
      if (!aiEnabled) {
        return { success: false, error: 'AI is disabled. Enable it to use AI features.' };
      }
  ...
  const analyzeDeep = useCallback(
    async (prompt: string, context?: Record<string, unknown>): Promise<AIResponse> => {
      if (!aiEnabled) {
        return {
          success: false,
          tier: 'thinker',
  ...
  const previewTier = useCallback(
    async (prompt: string): Promise<TierPreview> => {
      if (!aiEnabled) {
  ...
  const analyzeComplexity = useCallback(
    async (prompt: string): Promise<ComplexityAnalysis> => {
      if (!aiEnabled) {
  ...
  const getUsageStats = useCallback(
    async (startDate?: Date, endDate?: Date): Promise<UsageStats> => {
      try {
        const headers = await getAuthHeaders();
  ...
  return <AIContextReact.Provider value={value}>{children}</AIContextReact.Provider>;
}
