...
import { Shield, Users, Pencil, Trash, Crown, Star } from 'phosphor-react';
...
export const RoleManager: React.FC<RoleManagerProps> = ({ roles, onCreateRole, onEditRole, onDeleteRole }) => {
  ...
  return (
    <div className="space-y-6">
      {/* System Roles */}
      <div>
        <h3 className="text-sm font-medium text-stone-500 uppercase mb-3">System Roles</h3>
            ...
            .map((role) => {
              ...
              return (
                ...
                >
                  ...
                  <h4 className="font-semibold text-stone-800 dark:text-stone-200">{role.name}</h4>
                  ...
                  <div className="mt-3 pt-3 border-t border-stone-100 dark:border-stone-800">
                    <p className="text-xs text-stone-400">{role.permissions.length} permission groups</p>
                  </div>
                </div>
              );
            })}
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-medium text-stone-500 uppercase">Custom Roles</h3>
          <button onClick={onCreateRole} className="text-sm text-rose-600 hover:text-rose-700">
            + Create Role
          </button>
        </div>
        {roles.filter((r) => !r.isSystem).length === 0 ? (
          <div className="p-6 bg-white dark:bg-stone-900 rounded-xl border-2 border-dashed border-stone-300 dark:border-stone-700 text-center">
            ...
            <button onClick={onCreateRole} className="mt-2 text-sm text-rose-600 hover:text-rose-700">
      ...
      <div className="p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg text-center">
        <p className="text-sm text-amber-700 dark:text-amber-300">Role Manager - Full functionality coming soon</p>
      </div>
    </div>
  );
};
