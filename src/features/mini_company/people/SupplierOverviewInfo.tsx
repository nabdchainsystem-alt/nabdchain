...
import { X, Info, CaretRight, CaretDown, Package } from 'phosphor-react';
import { useLanguage } from '../../../contexts/LanguageContext';

interface SupplierOverviewInfoProps {
  isOpen: boolean;
  onClose: () => void;
}

export const SupplierOverviewInfo: React.FC<SupplierOverviewInfoProps> = ({ isOpen, onClose }) => {
  ...
  const questions = [
    { q: t('supplier_q1'), a: t('supplier_a1') },
    { q: t('supplier_q2'), a: t('supplier_a2') },
    { q: t('supplier_q3'), a: t('supplier_a3') },
    { q: t('supplier_q4'), a: t('supplier_a4') },
  ];
  ...
  return ReactDOM.createPortal(
    <div className={`fixed inset-0 z-[9999] flex overflow-hidden pointer-events-none font-sans justify-end`} dir={dir}>
      ...
      >
        <div
          className={`flex-none flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-700 bg-white dark:bg-monday-dark-surface z-10 ${isRTL ? 'flex-row-reverse' : ''}`}
        >
          <div className={isRTL ? 'text-right' : ''}>
            <h2
              className={`text-xl font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}
            >
        ...
        <div
          className={`flex-1 overflow-y-auto p-6 space-y-8 text-gray-600 dark:text-gray-300 pb-24 ${isRTL ? 'text-right' : ''}`}
        >
          <section>
            <h3
              className={`text-sm font-bold text-gray-900 dark:text-gray-100 mb-3 uppercase tracking-wider flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}
            >
            ...
            <div className="grid gap-2">
              {questions.map((item, i) => (
                ...
                >
                  ...
                  >
                    <p
                      className={`text-xs text-gray-500 dark:text-gray-400 pb-2 leading-relaxed ${isRTL ? 'pr-7' : 'pl-7'}`}
                    >
                      {item.a}
                    </p>
                  </div>
          ...
          <section>
            <h3
              className={`text-sm font-bold text-gray-900 dark:text-gray-100 mb-4 uppercase tracking-wider flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}
            >
              <span className="w-6 h-6 rounded bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 flex items-center justify-center text-xs">
                03
              </span>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="space-y-3">
                  <DetailItem title={t('total_suppliers')} desc={t('total_suppliers_desc')} isRTL={isRTL} />
                  <DetailItem title={t('active_suppliers')} desc={t('active_suppliers_desc')} isRTL={isRTL} />
                  <DetailItem title={t('on_time_delivery')} desc={t('on_time_delivery_desc')} isRTL={isRTL} />
                  <DetailItem title={t('quality_score')} desc={t('quality_score_desc')} isRTL={isRTL} />
                  <DetailItem title={t('avg_order_value')} desc={t('avg_order_value_desc')} isRTL={isRTL} />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  <DetailItem title={t('supplier_growth')} desc={t('supplier_growth_desc')} isRTL={isRTL} />
                  <DetailItem title={t('supplier_categories')} desc={t('supplier_categories_desc')} isRTL={isRTL} />
                  <DetailItem title={t('top_suppliers')} desc={t('top_suppliers_desc')} isRTL={isRTL} />
                  <DetailItem title={t('spend_by_category')} desc={t('spend_by_category_desc')} isRTL={isRTL} />
                </div>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="space-y-6">
              <div>
                <div
                  className={`flex items-center gap-2 mb-4 text-gray-800 dark:text-gray-200 font-semibold text-xs uppercase tracking-wide ${isRTL ? 'flex-row-reverse' : ''}`}
                >
                  <Package size={14} className="text-gray-500" />
                  <span>{t('supplier_tiers')}</span>
                </div>
                ...
                <div className="space-y-4">
                  <div className="bg-gray-50 dark:bg-gray-800/30 rounded-lg border border-gray-100 dark:border-gray-700 p-3">
                    ...
                    <p className="text-[10px] text-gray-500 dark:text-gray-400">{t('tier_definition_desc')}</p>
                  </div>
                </div>
              </div>
            </div>
