...
import { X, Info, CaretRight, CaretDown, Users, UserPlus, Wallet } from 'phosphor-react';

interface CustomerOverviewInfoProps {
    isOpen: boolean;
    onClose: () => void;
}

export const CustomerOverviewInfo: React.FC<CustomerOverviewInfoProps> = ({ isOpen, onClose }) => {
    ...
    const questions = [
        { q: 'How is "Active Customer" defined?', a: 'A customer who has placed at least one order in the last 90 days.' },
        { q: 'What counts as "Churned"?', a: 'Customers who have not purchased in the last 12 months despite having previous activity.' },
        { q: 'How is "Engagement Rate" calculated?', a: 'The percentage of active customers who visit the store or interact with emails weekly.' },
        { q: 'What is the "Growth %" comparing?', a: 'It compares the total active customer base size of the current month vs. the previous month.' }
    ];
            ...
            >
                <div className="flex-none flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-700 bg-white dark:bg-[#1a1d24] z-10">
                    <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Base Size & Health</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8 text-gray-600 dark:text-gray-300 pb-24">
                    <section>
                        ...
                        <p className="text-sm leading-relaxed p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-700">
                            This dashboard provides a holistic view of the customer base, tracking acquisition, retention, and overall value metrics to gauge business health.
                        </p>
                    </section>
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <div className="space-y-3">
                                    <DetailItem title="Total Customers" desc="All registered accounts." />
                                    <DetailItem title="Active Customers" desc="Purchased recently." />
                                    <DetailItem title="New Customers" desc="Acquired in current period." />
                                    <DetailItem title="Churned" desc="Lost customers." />
                                    <DetailItem title="Avg Revenue/User" desc="Mean spend per active user." />
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="space-y-3">
                                    <DetailItem title="Customers by Month" desc="Growth trend over time." />
                                    <DetailItem title="Customer Distribution" desc="Segment split (e.g., VIP, New)." />
                                    <DetailItem title="Customer List" desc="Top active customers table." />
                                    <DetailItem title="Value Density" desc="Radial map of revenue concentration." />
                                </div>
                            </div>
                        </div>
                    </section>
                            ...
                            <div>
                                ...
                                <div className="space-y-4">
                                    <div className="bg-gray-50 dark:bg-gray-800/30 rounded-lg border border-gray-100 dark:border-gray-700 p-3">
                                        <div className="font-bold text-xs text-gray-800 dark:text-gray-200 mb-1">Segment Definition</div>
                                        <p className="text-[10px] text-gray-500 dark:text-gray-400">VIP = Top 10% by spend. Loyal = &gt;5 orders. New = &lt;30 days since first order.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
