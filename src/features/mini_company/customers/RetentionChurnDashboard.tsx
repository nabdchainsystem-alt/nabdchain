...
import { ArrowsOut, ArrowsIn, Info, TrendUp, FirstAid, Prohibit, Heart, ShieldWarning, Coin } from 'phosphor-react';
...
export const RetentionChurnDashboard: React.FC = () => {
    ...
    () => [
      {
        ...
        subtitle: t('year_over_year'),
        value: '85%',
        change: '-2%',
        trend: 'down',
        icon: <Heart size={18} />,
        sparklineData: [88, 87, 87, 86, 86, 85],
        color: 'blue',
      },
      {
        id: '2',
        label: t('churn_rate'),
        subtitle: t('monthly_churn'),
        value: '15%',
        change: '+2%',
        trend: 'down',
        icon: <Prohibit size={18} />,
        sparklineData: [12, 13, 13, 14, 14, 15],
        color: 'blue',
      },
      {
        id: '3',
        label: t('at_risk_customers'),
        subtitle: t('high_churn_risk'),
        value: '45',
        change: '+5',
        trend: 'down',
        icon: <ShieldWarning size={18} />,
        sparklineData: [35, 38, 40, 42, 44, 45],
        color: 'blue',
      },
      {
        id: '4',
        label: t('avg_customer_lifespan'),
        subtitle: t('customer_tenure'),
        value: '18mo',
        change: '+1mo',
        trend: 'up',
        icon: <TrendUp size={18} />,
        sparklineData: [16, 16, 17, 17, 18, 18],
        color: 'blue',
      },
    ],
    [t],
  ...
  const SIDE_KPIS = useMemo<(KPIConfig & { rawValue?: number; isCurrency?: boolean; color?: string })[]>(
    () => [
      ...
      },
      {
        ...
        trend: 'down',
        icon: <Coin size={18} />,
        sparklineData: [10, 11, 11.5, 12, 12.2, 12.5],
        color: 'blue',
      },
      {
        id: '7',
        label: t('loyalty_score'),
        subtitle: t('nps_based'),
        value: '72',
        change: '-1',
        trend: 'neutral',
        icon: <Heart size={18} />,
        sparklineData: [74, 74, 73, 73, 72, 72],
        color: 'blue',
      },
      {
        id: '8',
        label: t('win_back_rate'),
        subtitle: t('returning_lost'),
        value: '18%',
        change: '+3%',
        trend: 'up',
  ...
  const MONTHS = useMemo(() => [t('jan'), t('feb'), t('mar'), t('apr'), t('may'), t('jun')], [t]);

  const RISK_LEVELS = useMemo(() => [t('safe'), t('watch'), t('risk'), t('critical')], [t]);
    ...
    () => ({
      ...
      tooltip: {
        ...
        formatter: (params: CallbackDataParams | CallbackDataParams[]) => {
          const v = (params as CallbackDataParams).value as number[];
          return `${MONTHS[v[0]]} - ${RISK_LEVELS[v[1]]}: ${v[2]} ${t('customers').toLowerCase()}`;
        },
      },
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {CHURN_TABLE.map((row, index) => (
                    <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                      ...
                      <td className="px-5 py-3 text-start">
                        <span
                          className={`font-medium ${
                            row.risk.includes(t('high'))
                              ? 'text-red-600'
                              : row.risk.includes(t('medium'))
                                ? 'text-amber-600'
                                : 'text-green-600'
                          }`}
                        >
                      ...
                      <td className="px-5 py-3 text-gray-600 dark:text-gray-400 text-xs italic text-start">
                        {row.reason}
                      </td>
