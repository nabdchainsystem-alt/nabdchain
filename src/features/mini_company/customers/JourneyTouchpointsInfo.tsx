...
import { X, Info, CaretRight, CaretDown, Path, MapTrifold } from 'phosphor-react';
...
export const JourneyTouchpointsInfo: React.FC<JourneyTouchpointsInfoProps> = ({ isOpen, onClose }) => {
    ...
    const questions = [
        { q: 'What is a "Touchpoint"?', a: 'Any interaction between a customer and the brand, e.g., ad click, website visit, email open, purchase, or support ticket.' },
        { q: 'How is "Friction Index" measured?', a: 'Based on the average time spent at bottlenecks and the rate of abandonment at specific journey stages.' },
        { q: 'What does "Journey Completion %" track?', a: 'The percentage of customers who successfully move from the initial Awareness stage to final Conversion.' },
        { q: 'What is an "Engagement Score"?', a: 'An aggregate metric of interaction depth and frequency across all channels.' }
    ];
            ...
            >
                <div className="flex-none flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-700 bg-white dark:bg-[#1a1d24] z-10">
                    <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Lifecycle Mapping</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8 text-gray-600 dark:text-gray-300 pb-24">
                    <section>
                        ...
                        <p className="text-sm leading-relaxed p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-700">
                            This dashboard maps the entire customer lifecycle, identifying key interactions, drop-off points, and opportunities to streamline the path to purchase.
                        </p>
                    </section>
                            ...
                            {questions.map((item, i) => (
                                <div key={i} className="rounded-lg border border-transparent hover:border-gray-100 dark:hover:border-gray-700 transition-colors overflow-hidden">
                                    ...
                                    >
                                        ...
                                        <span className={`font-medium ${openQuestionIndex === i ? 'text-teal-600 dark:text-teal-400' : 'text-gray-700 dark:text-gray-300'}`}>
                                            {item.q}
                                        </span>
                                    </button>
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <div className="space-y-3">
                                    <DetailItem title="Touchpoints Count" desc="Total interactions recorded." />
                                    <DetailItem title="Avg Journey Time" desc="Duration from discovery to deal." />
                                    <DetailItem title="Drop-Off Points" desc="Stages with highest exit rates." />
                                    <DetailItem title="Friction Index" desc="Resistance score (0-100)." />
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="space-y-3">
                                    <DetailItem title="Interaction Stages" desc="Volume per lifecycle phase." />
                                    <DetailItem title="Conversion Funnel" desc="Step-by-step yield rates." />
                                    <DetailItem title="Journey Log" desc="Latest customer movement inputs." />
                                    <DetailItem title="Traffic Flow" desc="Sankey diagram of user paths." />
                                </div>
                            </div>
                        </div>
                    </section>
