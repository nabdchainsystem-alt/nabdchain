...
export const SegmentationValueDashboard: React.FC = () => {
  ...
  const TOP_KPIS = useMemo<(KPIConfig & { rawValue?: number; isCurrency?: boolean; color?: string })[]>(
    () => [
      {
        id: '1',
        label: t('high_value_clients'),
        subtitle: t('spend_over_10k'),
        value: '125',
        change: '+5',
        trend: 'up',
        icon: <Crown size={18} />,
        sparklineData: [115, 118, 120, 122, 124, 125],
        color: 'blue',
      },
      {
        id: '2',
        label: t('mid_value_clients'),
        subtitle: t('spend_1k_10k'),
        value: '850',
        change: '+20',
        trend: 'up',
        icon: <Diamond size={18} />,
        sparklineData: [800, 810, 820, 830, 840, 850],
        color: 'blue',
      },
      {
        id: '3',
        label: t('low_value_clients'),
        subtitle: t('spend_under_1k'),
        value: '1,475',
        change: '-10',
        trend: 'down',
        icon: <Coin size={18} />,
        sparklineData: [1500, 1490, 1480, 1475, 1470, 1475],
        color: 'blue',
      },
      {
        id: '4',
        label: t('avg_clv'),
        subtitle: t('lifetime_value'),
        value: '0',
        rawValue: 2850,
        isCurrency: true,
        change: '+$150',
        ...
        sparklineData: [2600, 2650, 2700, 2750, 2800, 2850],
        color: 'blue',
      },
    ],
    [t],
  ...
  const SIDE_KPIS = useMemo<(KPIConfig & { rawValue?: number; isCurrency?: boolean; color?: string })[]>(
    () => [
      {
        id: '5',
        label: t('revenue_concentration'),
        subtitle: t('top_20_share'),
        value: '75%',
        change: '-1%',
        trend: 'neutral',
        icon: <ChartPieSlice size={18} />,
        sparklineData: [76, 76, 75, 75, 75, 75],
        color: 'blue',
      },
      {
        id: '6',
        label: t('segment_growth'),
        subtitle: t('high_value'),
        value: '4.5%',
        change: '+0.5%',
        trend: 'up',
        icon: <TrendUp size={18} />,
        sparklineData: [3, 3.5, 3.8, 4, 4.2, 4.5],
        color: 'blue',
      },
      {
        id: '7',
        label: t('risky_segment'),
        subtitle: t('churn_risk'),
        value: '12%',
        change: '+2%',
        trend: 'down',
      ...
      {
        id: '8',
        label: t('tier_migration'),
        subtitle: t('upgrades_this_month'),
        value: '24',
        change: '+6',
        trend: 'up',
        icon: <Crown size={18} />,
        sparklineData: [15, 17, 19, 21, 22, 24],
        color: 'blue',
      },
    ],
    [t],
    ...
    () => ({
      ...
      tooltip: {
        formatter: (params: CallbackDataParams | CallbackDataParams[]) => {
          ...
          return `<b>${v[2]}</b><br/>Freq: ${v[0]}<br/>CLV: $${v[1]}`;
        },
      },
      ...
      series: [
        {
          type: 'scatter',
          symbolSize: (data: number[]) => Math.min(Math.max(data[1] / 5000, 10), 40),
          data: SCATTER_DATA,
          itemStyle: {
            color: (params: CallbackDataParams) => {
              const val = (params.value as unknown[])[1] as number;
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {TRANSLATED_TABLE.map((row) => (
                    <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                      ...
                      <td className="px-5 py-3 text-start">
                        <span
                          className={`inline-flex px-2 py-0.5 rounded text-[10px] font-bold ${
                            row.segmentKey === 'high_value'
                              ? 'bg-indigo-100 text-indigo-700'
                              : row.segmentKey === 'low_value'
                      ...
                      <td className="px-5 py-3 text-end text-emerald-600 font-bold">
                        {formatCurrency(row.rawClv, currency.code, currency.symbol)}
                      </td>
