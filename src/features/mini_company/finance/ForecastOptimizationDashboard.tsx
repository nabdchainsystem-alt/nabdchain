...
const getTopKPIs = (
  ...
  {
    id: '1',
    label: t('forecasted_expenses'),
    subtitle: t('next_month'),
    value: '0',
    rawValue: 135000,
    isCurrency: true,
    change: '+5%',
    trend: 'up',
    icon: <Graph size={18} />,
    sparklineData: [120, 125, 128, 130, 132, 135],
    color: 'blue',
  },
  {
    id: '2',
    label: t('forecast_accuracy'),
    subtitle: t('historical'),
    value: '94%',
    change: '+1%',
    trend: 'up',
    icon: <Crosshair size={18} />,
    sparklineData: [92, 93, 93, 94, 94, 94],
    color: 'blue',
  },
  {
    id: '3',
    label: t('expected_savings'),
    subtitle: t('identified'),
    value: '0',
    rawValue: 8500,
    isCurrency: true,
    change: '+500',
    ...
    sparklineData: [5, 6, 7, 7, 8, 8.5],
    color: 'blue',
  },
  {
    id: '4',
    label: t('cost_reduction'),
    subtitle: t('potential'),
    value: '12%',
    change: '0%',
    trend: 'neutral',
...
const getSideKPIs = (
...
): (KPIConfig & { rawValue?: number; isCurrency?: boolean; color?: string })[] => [
  {
    ...
    sparklineData: [3, 2.8, 2.5, 2.2, 2.1, 2],
    color: 'blue',
  },
  {
    id: '6',
    label: t('optimization_actions'),
    subtitle: t('open_items'),
    value: '5',
    change: '+2',
    trend: 'up',
    icon: <MagicWand size={18} />,
    sparklineData: [3, 3, 3, 4, 4, 5],
    color: 'blue',
  },
  {
    id: '7',
    ...
    sparklineData: [90, 90, 90, 90, 90, 90],
    color: 'blue',
  },
  {
    id: '8',
    label: t('roi_expected'),
    subtitle: t('initiatives'),
    value: '18%',
    change: '+3%',
    trend: 'up',
    icon: <TrendUp size={18} />,
    sparklineData: [12, 13, 14, 15, 16, 18],
    color: 'blue',
  },
];
    ...
    () => ({
      ...
      tooltip: {
        formatter: (params: CallbackDataParams | CallbackDataParams[]) => {
          ...
          return `<b>${v[2]}</b><br/>Risk: ${v[0]}<br/>Saving: $${v[1]}`;
        },
      },
      ...
      series: [
        {
          type: 'scatter',
          symbolSize: (data: number[]) => Math.min(Math.max(data[1] / 100, 15), 50),
          data: LANDSCAPE_DATA,
          itemStyle: {
          ...
          label: {
            show: true,
            formatter: (param: CallbackDataParams) => (param.value as unknown[])[2] as string,
            position: 'top',
            color: '#6b7280',
          },
        },
      ...
      series: [
        {
          type: 'bar',
          name: 'Current',
          data: FORECAST_BY_CATEGORY.map((d) => d.Current),
        ...
        },
        {
          type: 'bar',
          name: 'Forecast',
          data: FORECAST_BY_CATEGORY.map((d) => d.Forecast),
                    ...
                    {OPTIMIZATION_TABLE.map((row, index) => (
                      <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="px-5 py-3 text-end text-gray-600 dark:text-gray-400">
                          {formatCurrency(row.rawCurrent, currency.code, currency.symbol)}
                        </td>
                        <td className="px-5 py-3 text-end text-gray-600 dark:text-gray-400">
                          {formatCurrency(row.rawForecast, currency.code, currency.symbol)}
                        </td>
                        <td className="px-5 py-3 text-start text-indigo-600 font-medium">{row.action}</td>
                        <td className="px-5 py-3 text-end text-green-600 font-bold">
                          {formatCurrency(row.rawSaving, currency.code, currency.symbol)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
