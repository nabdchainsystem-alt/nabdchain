...
import { X, Info, CaretRight, CaretDown, Funnel, Table, Calculator } from 'phosphor-react';

interface PurchaseFunnelInfoProps {
    isOpen: boolean;
    onClose: () => void;
}

export const PurchaseFunnelInfo: React.FC<PurchaseFunnelInfoProps> = ({ isOpen, onClose }) => {
    ...
    const questions = [
        { q: 'Where are requests getting stuck?', a: 'Check the "Bottleneck Stage" KPI and the "Avg Time per Stage" chart. High durations indicate process inefficiencies.' },
        { q: 'How many requests are we rejecting?', a: 'Monitor "Rejected Requests" and "Approval Rate". A high rejection rate suggests clarity issues with initial requests.' },
        { q: 'Are we meeting approval SLAs?', a: 'Look at "Avg Approval Time". If it exceeds internal targets (e.g., 2 days), the process needs optimization.' },
        { q: 'What is the volume of our pipeline?', a: '"Requests Submitted" gives the total intake volume, while the Funnel Chart visualizes the conversion to orders.' },
        { q: 'How do requests flow through the system?', a: 'The "Approval Flow Sankey" shows the complex movement, including re-routes and rejections.' }
    ];
            ...
            >
                <div className="flex-none flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-700 bg-white dark:bg-monday-dark-surface z-10">
                    <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Approval Flow & Bottlenecks</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8 text-gray-600 dark:text-gray-300 pb-24">
                    <section>
                        ...
                        <p className="text-sm leading-relaxed p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-700">
                            This dashboard tracks the lifecycle of purchase requests from submission to final approval, highlighting bottlenecks, delays, and approval efficiency.
                        </p>
                    </section>
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <div className="space-y-3">
                                    <DetailItem title="Requests Submitted" desc="Total number of new purchase requests initiated." />
                                    <DetailItem title="Approved Requests" desc="Requests that have successfully passed all approval stages." />
                                    <DetailItem title="Rejected Requests" desc="Requests denied by approvers." />
                                    <DetailItem title="Avg Approval Time" desc="Average duration from submission to final decision." />
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <div className="space-y-4">
                                    <TableSchema
                                        name="1. PurchaseRequests"
                                        desc="Initial request records."
                                        columns={['Request ID', 'Requester', 'Submission Date', 'Status']}
                                    />
                                    <TableSchema
                                        name="2. ApprovalLogs"
                                        desc="History of approval actions."
                                        columns={['Log ID', 'Request ID', 'Stage', 'Action Date', 'Outcome']}
                                    />
                                    <TableSchema
                                        name="3. ApprovalWorkflows"
                                        desc="Configuration of stages."
                                        columns={['Stage Name', 'SLA (Days)', 'Approver Role']}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="bg-blue-50 dark:bg-blue-900/10 p-4 rounded-xl border border-blue-100 dark:border-blue-800/30">
                                ...
                                <ul className="space-y-2.5 text-xs text-blue-900/80 dark:text-blue-200/80 ml-1">
                                    <li className="flex gap-2">
                                        ...
                                        <span><strong>Approval Time</strong> = Final Decision Date - Submission Date.</span>
                                    </li>
                                    <li className="flex gap-2">
                                        <span className="w-1.5 h-1.5 rounded-full bg-blue-400 mt-1.5 shrink-0" />
                                        <span><strong>Delayed</strong> = Current Date - Stage Entry Date &gt; Stage SLA.</span>
                                    </li>
                                </ul>
                            </div>
                        </div>
