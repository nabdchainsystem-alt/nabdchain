...
import { ArrowsOut, ArrowsIn, Info, TrendUp, Warning, Target, ListChecks, CurrencyDollar } from 'phosphor-react';
...
export const ForecastPlanningDashboard: React.FC = () => {
  ...
  const TOP_KPIS = useMemo(
    () => [
      {
        id: '1',
        label: t('forecasted_spend'),
        subtitle: t('next_30_days'),
        value: '0',
        rawValue: 42000,
        isCurrency: true,
        change: '+5%',
        ...
        sparklineData: [38, 39, 40, 40, 41, 41, 42],
      },
      {
        id: '2',
        label: t('forecast_accuracy'),
        subtitle: t('last_period'),
        value: '92%',
        change: '+1%',
        trend: 'up' as const,
        icon: <Target size={18} />,
        sparklineData: [88, 89, 90, 90, 91, 91, 92],
      },
      {
        id: '3',
        label: t('est_cat_growth'),
        subtitle: t('highest_office'),
        value: '+12%',
        change: '+2%',
        trend: 'up' as const,
  ...
  const SIDE_KPIS = useMemo(
    () => [
      {
        id: '5',
        label: t('supply_risk_ahead'),
        subtitle: t('potential_issues'),
        value: t('low'),
        change: '',
        trend: 'neutral' as const,
        icon: <Warning size={18} />,
        sparklineData: [0, 1, 1, 1, 0, 0, 0],
      },
      {
        id: '6',
        label: t('planned_orders'),
        subtitle: t('drafted'),
        value: '8',
        change: '+2',
        trend: 'up' as const,
        icon: <ListChecks size={18} />,
        sparklineData: [5, 6, 6, 7, 7, 8, 8],
      },
      {
        id: '7',
        label: t('budget_impact'),
        subtitle: t('pct_of_annual_limit'),
        value: '15%',
        change: '+1%',
        trend: 'up' as const,
        icon: <CurrencyDollar size={18} />,
        sparklineData: [12, 13, 13, 14, 14, 14, 15],
      },
      {
        id: '8',
        label: t('variance_trend'),
        subtitle: t('forecast_vs_actual'),
        value: '-2.1%',
        change: '-0.5%',
        trend: 'up' as const,
        icon: <TrendUp size={18} />,
        sparklineData: [4, 3.5, 3.2, 2.8, 2.5, 2.3, 2.1],
      },
    ],
    [t],
  );
                  ...
                  {FORECAST_DETAILS.map((r) => (
                    <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                      ...
                      <td className="px-5 py-3 text-end text-gray-600 dark:text-gray-400">
                        {formatCurrency(r.rawLast, currency.code, currency.symbol)}
                      </td>
                      <td className="px-5 py-3 text-end font-medium text-gray-900 dark:text-gray-100">
                        {formatCurrency(r.rawForecast, currency.code, currency.symbol)}
                      </td>
                      <td
                        className={`px-5 py-3 text-end font-medium ${r.delta.startsWith('+') ? 'text-red-500' : 'text-green-500'}`}
                      >
                        {r.delta}
                      </td>
                    </tr>
