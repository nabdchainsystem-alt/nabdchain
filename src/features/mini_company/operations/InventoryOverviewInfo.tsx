...
import { X, Info, CaretRight, CaretDown, Package, Table, Calculator } from 'phosphor-react';
...
export const InventoryOverviewInfo: React.FC<InventoryOverviewInfoProps> = ({ isOpen, onClose }) => {
    ...
    const questions = [
        { q: 'What is our current stock health?', a: 'Check "In-Stock Rate" and "Out-of-Stock Items". A high in-stock rate (>98%) indicates good availablity.' },
        { q: 'How valuable is our inventory?', a: '"Total Stock Value" gives the financial worth of all held items, usually at cost price.' },
        { q: 'Which items need attention?', a: 'Look at "Low Stock Alerts". These are items falling below their reorder point and need replenishment.' },
        { q: 'Is our inventory stagnant?', a: '"Avg Inventory Age" tells you how long, on average, items sit before being sold. Lower is generally better.' },
        { q: 'How is stock distributed?', a: 'The "Radial Inventory Density Map" and Pie Charts show how inventory is spread across warehouses and categories.' }
    ];
            ...
            >
                <div className="flex-none flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-700 bg-white dark:bg-monday-dark-surface z-10">
                    <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Stock Levels & Health</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8 text-gray-600 dark:text-gray-300 pb-24">
                    <section>
                        ...
                        <p className="text-sm leading-relaxed p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-700">
                            This dashboard provides a real-time snapshot of inventory levels, total value, and movement health, helping to prevent stockouts and overstocking.
                        </p>
                    </section>
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <div className="space-y-3">
                                    <DetailItem title="Total Stock Value" desc="Monetary value of current inventory holdings." />
                                    <DetailItem title="Total SKUs" desc="Count of unique stock keeping units managed." />
                                    <DetailItem title="In-Stock Rate %" desc="Percentage of SKUs with quantity &gt; 0." />
                                    <DetailItem title="Out-of-Stock Items" desc="Count of SKUs currently at 0 quantity." />
                                    <DetailItem title="Low Stock Alerts" desc="SKUs below their defined reorder point." />
                                    <DetailItem title="Avg Inventory Age" desc="Average days items have been in stock." />
                                    <DetailItem title="Inventory Turnover" desc="Ratio of COGS to Average Inventory." />
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="space-y-3">
                                    <DetailItem title="Stock by Category" desc="Bar chart (Recharts) showing volume per category." />
                                    <DetailItem title="Stock Distribution" desc="Pie chart (ECharts) showing warehouse allocation." />
                                    <DetailItem title="Inventory List" desc="Table listing key SKU details and status." />
                                    <DetailItem title="Radial Density Map" desc="Visualizes concentration of stock across attributes." />
                                </div>
                            </div>
                        </div>
                    </section>
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <div className="space-y-4">
                                    <TableSchema
                                        name="InventoryItems"
                                        desc="Main item master data."
                                        columns={['SKU', 'Name', 'QoH', 'Cost', 'Category']}
                                    />
                                    <TableSchema
                                        name="Warehouses"
                                        desc="Location definitions."
                            ...
                            <div className="bg-emerald-50 dark:bg-emerald-900/10 p-4 rounded-xl border border-emerald-100 dark:border-emerald-800/30">
                                ...
                                <ul className="space-y-2.5 text-xs text-emerald-900/80 dark:text-emerald-200/80 ml-1">
                                    <li className="flex gap-2">
                                        ...
                                        <span><strong>Value</strong> = Sum(QoH * Unit Cost).</span>
                                    </li>
                                    <li className="flex gap-2">
                                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 mt-1.5 shrink-0" />
                                        <span><strong>In-Stock Rate</strong> = (SKUs &gt; 0 / Total SKUs) * 100.</span>
                                    </li>
                                </ul>
                            </div>
                        </div>
