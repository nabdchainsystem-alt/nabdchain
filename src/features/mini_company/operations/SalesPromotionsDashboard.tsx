...
export const SalesPromotionsDashboard: React.FC<SalesPromotionsDashboardProps> = ({ hideFullscreen = false }) => {
  ...
  const TOP_KPIS = useMemo(
    () => [
      {
        id: '1',
        label: t('campaigns_active'),
        subtitle: t('live_promotional_events'),
        value: '4',
        change: t('stable'),
      ...
      },
      {
        id: '2',
        label: t('total_spend'),
        subtitle: t('marketing_budget_used'),
        value: '0',
        rawValue: 42500,
        isCurrency: true,
        change: '+12%',
        trend: 'down' as const,
        icon: <Tag size={18} />,
        sparklineData: [32, 35, 38, 40, 41, 42, 42.5],
      },
      {
        id: '3',
        label: t('revenue_from_promo'),
        subtitle: t('attributed_gross_volume'),
        value: '0',
        rawValue: 284500,
        isCurrency: true,
        change: '+24%',
        ...
        sparklineData: [180, 200, 220, 245, 260, 275, 284.5],
      },
      {
        id: '4',
        label: t('overall_roi'),
        subtitle: t('campaign_profitability'),
        value: '569%',
        change: '+45%',
        trend: 'up' as const,
        icon: <TrendUp size={18} />,
        sparklineData: [380, 420, 460, 500, 530, 550, 569],
      },
    ],
    [t],
  );
  ...
  const SIDE_KPIS = useMemo(
    () => [
      {
        id: '5',
        label: t('promo_conversion'),
        subtitle: t('engaged_vs_purchased'),
        value: '18.4%',
        change: '+3.2%',
        trend: 'up' as const,
        icon: <Percent size={18} />,
        sparklineData: [12, 13.5, 15, 16, 17, 17.8, 18.4],
      },
      {
        id: '6',
        label: t('incremental_sales'),
        subtitle: t('above_organic_baseline'),
        value: '0',
        rawValue: 105000,
        isCurrency: true,
        change: '+18%',
        ...
        sparklineData: [75, 80, 85, 90, 95, 100, 105],
      },
      {
        id: '7',
        label: t('engagement_rate'),
        subtitle: t('clicks_engagement_avg'),
        value: '4.2%',
        change: '+1.1%',
        trend: 'up' as const,
        icon: <Users size={18} />,
        sparklineData: [2.8, 3.2, 3.5, 3.8, 4, 4.1, 4.2],
      },
      {
        id: '8',
        label: t('coupon_redemption'),
        subtitle: t('codes_used_issued'),
        value: '32.5%',
        change: '+5.2%',
        trend: 'up' as const,
        icon: <Tag size={18} />,
        sparklineData: [22, 24, 26, 28, 30, 31, 32.5],
      },
    ],
    [t],
  );
      ...
      tooltip: {
        ...
        formatter: (params: CallbackDataParams | CallbackDataParams[]) => {
          ...
          return `<div class="p-2 font-sans">
                    ...
                    <p class="text-xs text-gray-500 mt-1">${t('roi')}: ${d[0]}%</p>
                    <p class="text-xs text-gray-500">${t('conv_percent')}: ${d[1]}%</p>
                </div>`;
        },
      },
            ...
            <div className="lg:col-span-1 bg-white dark:bg-monday-dark-elevated rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden flex flex-col text-start animate-fade-in-up">
                ...
                <table className="w-full text-sm text-start">
                  ...
                  <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
                    {paginatedData.map((row) => (
                      <tr
                        key={row.id}
                        className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors border-b dark:border-gray-700 last:border-none"
                      >
                        ...
                        <td className="px-6 py-5 text-end font-medium text-gray-500">
                          {formatCurrency(row.budget, currency.code, currency.symbol)}
                        </td>
                        <td className="px-6 py-5 text-end font-bold text-gray-900 dark:text-white">
                          {formatCurrency(row.revenue, currency.code, currency.symbol)}
                        </td>
                        <td className="px-6 py-5 text-end">
                          <span className={`font-bold ${row.roi > 300 ? 'text-emerald-500' : 'text-amber-500'}`}>
                            {row.roi}%
                          </span>
                        </td>
              ...
              <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between bg-gray-50/30 dark:bg-gray-800/10 mt-auto">
                <span className="text-xs text-gray-500">
                  {t('page')} <span className="font-bold text-gray-700 dark:text-gray-300">{currentPage}</span>{' '}
                  {t('of')} {totalPages}
                </span>
