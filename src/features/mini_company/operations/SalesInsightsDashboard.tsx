...
// --- Visual Constants ---
const _COLORS_SEQUENCE = ['#6366f1', '#10b981', '#f59e0b', '#f43f5e', '#8b5cf6', '#ec4899', '#06b6d4'];
...
const TOP_KPIS: (KPIConfig & { rawValue?: number; isCurrency?: boolean })[] = [
  {
    ...
    sparklineData: [4000, 3000, 5000, 4800, 6000, 5500, 7000],
  },
  {
    id: '2',
    label: 'Net Revenue',
    ...
    sparklineData: [2000, 2500, 2200, 2800, 3000, 2900, 3500],
  },
  {
    id: '3',
    label: 'Orders Count',
    ...
    sparklineData: [50, 45, 60, 40, 35, 30, 28],
  },
  {
    id: '4',
    ...
    sparklineData: [90, 95, 100, 102, 101, 102, 102],
  },
];
...
const SIDE_KPIS: KPIConfig[] = [
  ...
  },
  {
    ...
    trend: 'up',
    icon: <Users size={18} />,
    sparklineData: [15, 16, 15, 17, 18, 17, 18],
  },
  {
    id: '7',
    label: 'Profit Margin',
    subtitle: 'Net earnings ratio',
    value: '23.5%',
    change: '+0.5%',
    trend: 'up',
    icon: <CurrencyDollar size={18} />,
    sparklineData: [20, 21, 22, 22, 23, 23, 24],
  },
  {
    id: '8',
    ...
    sparklineData: [3.0, 3.2, 3.1, 3.5, 3.6, 3.7, 3.8],
  },
];
...
export const SalesInsightsDashboard: React.FC<SalesInsightsDashboardProps> = memo(({ hideFullscreen = false }) => {
  ...
  const translatedTopKPIs = useMemo(
    () =>
  ...
  const translatedSideKPIs = useMemo(
    () =>
  ...
  const totalRevenue = useMemo(() => TOP_PRODUCTS_DATA.reduce((acc, curr) => acc + curr.revenue, 0), []);
  ...
  const treemapOption: EChartsOption = useMemo(
    () => ({
      tooltip: {
        formatter: (info: CallbackDataParams | CallbackDataParams[]) => {
          const p = info as CallbackDataParams;
          const value = p.value as number;
          const percentage = ((value / totalRevenue) * 100).toFixed(1);
          return `
                    <div class="font-medium text-sm text-gray-900 mb-1">${p.name}</div>
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex flex-col gap-0.5">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    {salesTimePeriod === 'hourly'
                      ? t('hourly_performance_overview')
                      : salesTimePeriod === 'weekly'
                        ? t('weekly_performance_overview')
                        : salesTimePeriod === 'monthly'
                          ? t('monthly_performance_overview')
            ...
            <div className="bg-white dark:bg-monday-dark-elevated p-5 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 h-full flex flex-col animate-fade-in-up">
              <div className="flex items-start justify-between mb-4">
                <div className="flex flex-col gap-0.5">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    {channelTimePeriod === 'hourly'
                      ? t('hourly_performance_overview')
                      : channelTimePeriod === 'weekly'
                        ? t('weekly_performance_overview')
                        : channelTimePeriod === 'monthly'
                          ? t('monthly_performance_overview')
                          : t('yearly_performance_overview')}
                  </p>
                </div>
                ...
                <table className="w-full text-sm text-start">
                  ...
                  <tbody>
                    {TOP_PRODUCTS_DATA.map((product, index) => (
                      <tr
                        key={product.id}
                        className="border-b dark:border-gray-700 last:border-none hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors"
                      >
                        ...
                        <td className="px-6 py-4 text-end font-medium text-gray-900 dark:text-gray-100">
                          {formatCurrency(product.revenue, currency.code, currency.symbol)}
                        </td>
                        <td className="px-6 py-4 text-end text-green-600 dark:text-green-400 font-medium">
                          {formatCurrency(product.profit, currency.code, currency.symbol)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
