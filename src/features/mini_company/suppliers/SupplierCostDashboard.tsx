...
export const SupplierCostDashboard: React.FC = () => {
  ...
  const TOP_KPIS = useMemo(
    () => [
      ...
      {
        id: '2',
        label: t('avg_spend_supplier'),
        subtitle: t('vendor_concentration'),
        value: '0',
        rawValue: 48000,
        isCurrency: true,
        change: '+5%',
        trend: 'up' as const,
        icon: <Wallet size={18} />,
        sparklineData: [42, 43, 45, 46, 47, 48],
        color: 'blue',
      },
      {
        id: '3',
        label: t('cost_variance_pct'),
        subtitle: t('actual_vs_contract'),
        value: '2.4%',
        change: '-0.5%',
        trend: 'down' as const,
        icon: <Percent size={18} />,
        sparklineData: [3.0, 2.9, 2.8, 2.6, 2.5, 2.4],
        color: 'blue',
      },
      {
        id: '4',
        label: t('savings_achieved'),
        subtitle: t('cost_avoidance'),
        value: '0',
        rawValue: 120000,
        isCurrency: true,
        change: '+20%',
        trend: 'up' as const,
        icon: <Coins size={18} />,
        sparklineData: [80, 90, 95, 100, 110, 120],
        color: 'blue',
      },
    ],
    [t],
    ...
    () => [
      {
        id: '5',
        label: t('tail_spend'),
        subtitle: t('pct_of_total'),
        value: '15%',
        change: '-2%',
        trend: 'down' as const,
        icon: <ChartLineUp size={18} />,
        sparklineData: [18, 17, 17, 16, 15, 15],
        color: 'blue',
      },
      {
        id: '6',
        label: t('open_po_value'),
        subtitle: t('committed_cost'),
        value: '0',
        rawValue: 850000,
        isCurrency: true,
        change: t('stable'),
        ...
        sparklineData: [850, 850, 850, 850, 850, 850],
        color: 'blue',
      },
      {
        id: '7',
        label: t('sourcing_savings'),
        subtitle: t('projected'),
        value: '0',
        rawValue: 50000,
        isCurrency: true,
        change: '+5%',
      ...
      {
        id: '8',
        label: t('budget_utilization'),
        subtitle: t('pct_of_allocated'),
        value: '78%',
        change: '+3%',
        trend: 'up' as const,
        icon: <Percent size={18} />,
  ...
  const TRANSLATED_SPEND_TREND = useMemo(
    () =>
      MONTHLY_SPEND_TREND.map((item) => ({
        ...item,
    ...
    () => ({
      tooltip: {
        ...
        formatter: (params: CallbackDataParams | CallbackDataParams[]) => {
          const tar = (params as CallbackDataParams[])[1];
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {COST_TABLE.map((row, index) => (
                    <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                      ...
                      <td className={`px-5 py-3 text-gray-600 dark:text-gray-400 text-end`}>
                        {formatCurrency(row.rawContract, currency.code, currency.symbol)}
                      </td>
                      <td className={`px-5 py-3 font-medium text-gray-800 dark:text-gray-200 text-end`}>
                        {formatCurrency(row.rawActual, currency.code, currency.symbol)}
                      </td>
                      <td
                        className={`px-5 py-3 text-center font-bold ${row.variance.includes('-') ? 'text-green-600' : 'text-red-500'}`}
                      >
                        {row.variance}
                      </td>
                      <td className={`px-5 py-3 text-green-600 font-medium text-end`}>
                        {formatCurrency(row.rawSavings, currency.code, currency.symbol)}
                      </td>
                    </tr>
                  ))}
                </tbody>
