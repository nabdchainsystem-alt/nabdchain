...
export const ScheduledReports: React.FC<ScheduledReportsProps> = ({ schedules = [], onAdd, onToggle, onDelete }) => {
  if (schedules.length === 0) {
    return (
      <div className="p-8 text-center">
        ...
        <p className="text-sm text-stone-500 mb-4">Automatically generate and send reports on a schedule</p>
        <button onClick={onAdd} className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg">
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-stone-800 dark:text-stone-200">Scheduled Reports</h3>
        <button
          onClick={onAdd}
              ...
              <div className="flex items-start gap-3">
                <div
                  className={`p-2 rounded-lg ${
                    schedule.active ? 'bg-emerald-100 dark:bg-emerald-900/30' : 'bg-stone-100 dark:bg-stone-800'
                  }`}
                >
                  <Calendar size={20} className={schedule.active ? 'text-emerald-600' : 'text-stone-400'} />
                </div>
                <div>
                  <h4 className="font-medium text-stone-800 dark:text-stone-200">Report Schedule</h4>
                  <p className="text-sm text-stone-500 mt-0.5">{getFrequencyLabel(schedule)}</p>
