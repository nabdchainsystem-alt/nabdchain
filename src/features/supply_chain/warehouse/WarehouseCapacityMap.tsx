    ...
    recentActivity: [
      {
        ...
        time: '15 mins ago',
        actor: 'System',
        action: 'flagged for maintenance check',
        location: 'B-04-A',
        status: 'warning',
      },
    ],
...
const ACTIVITY: ActivityEvent[] = [
  ...
  },
  {
    ...
    time: '42 mins ago',
    actor: 'Admin',
    action: 'receipt completed for PO-9921 at',
    location: 'Zone A',
    status: 'success',
  },
...
const KpiCards: React.FC<{
  ...
  const cards = [
    ...
    { label: 'Total Units', value: totals.quantity.toLocaleString(), delta: null, positive: null },
    {
      ...
      delta: totals.issues ? '+1' : '0',
      positive: totals.issues === 0,
    },
  ];
                  ...
                  <div className="grid grid-cols-6 gap-2">
                    {levels.map((level) => {
                      ...
                      return (
                        ...
                        >
                          {bin && <span className="text-[10px] font-semibold">{bin.id}</span>}
                        </button>
                      );
                    })}
                  </div>
...
const DetailsPanel: React.FC<{
  ...
  aiTip: string;
  aiLoading: boolean;
}> = ({ selectedBin, activity, aiTip, aiLoading }) => (
        ...
        <div className="p-5 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 bg-slate-50 rounded-lg border border-slate-200">
              ...
              <span
                className={`text-xs font-bold ${selectedBin.status === OccupancyStatus.ISSUE ? 'text-amber-700' : 'text-emerald-600'}`}
              >
                {selectedBin.status}
              </span>
            </div>
            <div className="p-3 bg-slate-50 rounded-lg border border-slate-200">
              ...
              <span className="text-xs font-bold">
                {Math.round((selectedBin.quantity / selectedBin.maxQuantity) * 100)}%
              </span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${(selectedBin.quantity / selectedBin.maxQuantity) * 100}%` }}
              ></div>
            </div>
          </div>
          ...
          <div className="p-4 border border-slate-200 rounded-xl bg-slate-50">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              {aiLoading ? 'Analyzing live capacity...' : aiTip || 'Enable Gemini API to see live recommendations.'}
            </p>
          </div>
            ...
            <div className="space-y-4">
              {(selectedBin.recentActivity.length ? selectedBin.recentActivity : activity).map((ev) => (
                <div key={ev.id} className="flex gap-3 text-xs">
                  <div
                    className={`size-2 mt-1 rounded-full ${ev.status === 'warning' ? 'bg-amber-500' : ev.status === 'success' ? 'bg-emerald-500' : 'bg-slate-300'}`}
                  ></div>
                  <div>
                    <p className="font-semibold text-slate-900">{ev.actor}</p>
                    <p className="text-slate-600">
                      {ev.action} {ev.location}
                    </p>
  ...
  useEffect(() => {
    const runAi = async () => {
      try {
        setAiLoading(true);
        const utilization =
          filteredBins.reduce((sum, b) => sum + b.quantity, 0) /
          Math.max(
            1,
            filteredBins.reduce((sum, b) => sum + b.maxQuantity, 0),
          );
        const issues = filteredBins.filter((b) => b.status === OccupancyStatus.ISSUE).length;
        const _prompt = `You are an operations analyst. Current warehouse utilization is ${(utilization * 100).toFixed(1)}% with ${issues} flagged issues. Provide one concise, actionable tip (max 35 words).`;

        // SECURITY: AI calls now go through backend proxy
        ...
        const text = result.tips?.[0] || result.response || result.text || '';
        if (text) setAiTip(text);
        else setAiTip('Smart Tips are temporarily unavailable.');
      } catch (_err) {
        setAiTip('Smart Tips are temporarily unavailable.');
      } finally {
        setAiLoading(false);
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex-1 flex overflow-hidden">
            <WarehouseGrid bins={filteredBins} selectedBinId={selectedBin?.id} onSelectBin={setSelectedBin} />
