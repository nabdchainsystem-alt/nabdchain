...
const GTDDashboard = React.lazy(() =>
  import('../../features/gtd/GTDDashboard').then((m) => ({ default: m.GTDDashboard })),
);
const OverviewView = React.lazy(() =>
  import('./views/Overview/OverviewView').then((m) => ({ default: m.OverviewView })),
);
const ProcurementOverview = React.lazy(() =>
  import('./views/Procurement/ProcurementOverview').then((m) => ({ default: m.ProcurementOverview })),
);
...
export const BoardView: React.FC<BoardViewProps> = memo(
  ({
    board: initialBoard,
    onUpdateBoard: initialOnUpdateBoard,
    onUpdateTasks: initialOnUpdateTasks,
    renderCustomView,
    dashboardSections,
    onNavigate,
    isDepartmentLayout,
    ...
    const handleBoardSave = React.useCallback(
      (updatedBoard: any) => {
    ...
    const { board, tasks, addTask, updateTask, deleteTask, onUpdateTasks, addGroup, deleteGroup, updateGroupTitle } =
      useRoomBoardData(effectiveKey, initialBoard, handleBoardSave);
    ...
    const _DEFAULT_VIEWS: BoardViewType[] = ['overview', 'table', 'kanban'];
    ...
    const sanitizedAvailableViews = useMemo(() => {
      ...
      const viewsWithoutOverview = views.filter(
        (v) => (v as any) !== 'overview' && (v as any) !== 'listboard' && (v as any) !== 'list_board',
      );
    ...
    const [activeView, setActiveView] = useState<BoardViewType>(() => {
      ...
      return sanitizedAvailableViews.includes('overview' as any)
        ? ('overview' as any)
        : sanitizedAvailableViews[0] || 'kanban';
    });
    ...
    const handleDragEnd = (event: DragEndEvent) => {
      ...
      if (over && active.id !== over.id && onUpdateBoard) {
        const _oldIndex = sanitizedAvailableViews.indexOf(active.id as BoardViewType);
        const _newIndex = sanitizedAvailableViews.indexOf(over.id as BoardViewType);
    ...
    const _aiButtonRef = useRef<HTMLButtonElement>(null);
    ...
    useEffect(() => {
      if (board.id === 'dept-sales') {
        ...
        if (onUpdateBoard && board.availableViews) {
          ...
          if (
            uniqueViews.length !== board.availableViews.length ||
    ...
    const [dashboardConfig, _setDashboardConfig] = useState<DashboardConfig | null>(() => {
      try {
    ...
    const _isViewAvailable = (view: BoardViewType) => {
    ...
    useEffect(() => {
      if (!isWarehouseBoard && activeView === 'warehouse_capacity_map') {
        const fallbackFromDefault =
          board.defaultView && board.defaultView !== 'warehouse_capacity_map' ? board.defaultView : undefined;
    ...
    const clearViewStorage = (boardId: string, vid: string) => {
      try {
        ...
        const patterns = [`-${boardId}-${vid}`, `-${vid}-${boardId}`, `-${vid}`];
    ...
    const VIEW_OPTIONS = [
      {
        label: isDepartmentLayout ? t('insights') : t('overview'),
        icon: PhOverview,
        id: 'overview',
        description: t('board_overview_desc'),
      },
    ...
    const _handleRenameBoard = (newName: string) => {
    ...
    const renderView = () => {
      ...
      switch (baseViewType) {
        ...
        case 'list':
          return <Lists roomId={board.id} viewId={activeView} tasks={tasks} onUpdateTasks={onUpdateTasks} />;
        case 'calendar':
          return (
            <CalendarView
              key={`${board.id}-${activeView}`}
    ...
    return (
      <div className="flex-1 flex flex-col h-full overflow-hidden bg-transparent">
        ...
        <div
          className={`flex-shrink-0 bg-white dark:bg-monday-dark-surface grid transition-[grid-template-rows] duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] will-change-[grid-template-rows] transform-gpu ${
            isFullScreen ? 'grid-rows-[0fr]' : 'grid-rows-[1fr]'
          }`}
        >
          <div className="overflow-hidden">
            <div className="pl-[24px] pr-[20px] pt-4 pb-0">
                          ...
                          <div className="space-y-4">
                            <div>
                              <div className="flex items-center gap-2 mb-1.5">
                                ...
                                <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-tight">
                                  {t('name')}
                                </label>
                              </div>
                              <input
              ...
              {/* Board Description */}
              {board.description && (
                <div className="pt-1 pb-4 text-sm text-gray-500 dark:text-gray-400 max-w-4xl">{board.description}</div>
              )}

              {/* Tabs Row */}
              <div
                className={`flex items-center gap-0 border-b border-gray-200 dark:border-gray-800 -ml-[24px] -mr-[20px] ${dir === 'rtl' ? 'pr-[24px] pl-6' : 'pl-[24px] pr-6'}`}
              >
                    ...
                    (() => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-2 py-1.5 border-b-2 text-[13.6px] font-medium transition-colors whitespace-nowrap ${
                            activeView === 'overview'
                              ...
                              {VIEW_OPTIONS.filter(
                                ...
                                >
                                  ...
                                  <div className="text-start">
                                    <span className="block text-[13px] text-gray-700 dark:text-gray-200 font-medium leading-tight">
                                      {option.label}
                                    </span>
                                    <span className="block text-[10px] text-gray-500 dark:text-gray-400 mt-0.5 leading-tight">
                                      {option.description}
                                    </span>
                                  </div>
                                </button>
        ...
        {(() => {
          ...
          const isFullWidth =
            ...
            (type === 'overview' && isDashboardBoard);
          const _currentViewIndex = fullscreenNavigableViews.indexOf(activeView);
          const _currentViewOption = effectiveViewOptions.find((v) => v.id === getBaseViewType(activeView));
            ...
            >
              <div
                className={`flex-1 overflow-hidden transition-all duration-300 ease-out ${
                  isAnimating ? (slideDirection === 'left' ? 'animate-slide-in-right' : 'animate-slide-in-left') : ''
                }`}
                style={{
              ...
              >
                ...
                >
                  ...
                  {renderCustomView &&
                    Array.from(visitedCustomViews).map((viewType) => {
            ...
            >
              ...
              <div className="px-1.5 space-y-0.5">
                ...
                >
                  <Pin
                    size={16}
                    weight={(board.pinnedViews || []).includes(contextMenu.viewId) ? 'fill' : 'regular'}
