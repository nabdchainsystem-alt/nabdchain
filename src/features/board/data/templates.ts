...
export interface TemplateGroup {
  ...
  title: string;
  color: string;
  tasks?: Record<string, unknown>[]; // Allow pre-filled tasks if needed
}
...
const mapColumns = (cols: string[]): TemplateColumn[] => {
  ...
  const mapped: TemplateColumn[] = [{ id: 'select', label: '', type: 'select', width: 40 }];
  ...
  cols.forEach((c) => {
    ...
    else if (c === 'Progress %' || c === 'Completion %')
      mapped.push({ id: 'progress', label: c, type: 'number', width: 120 });
