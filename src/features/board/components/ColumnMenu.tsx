...
export const ColumnMenu: React.FC<ColumnMenuProps> = ({ onClose, onSelect, darkMode }) => {
  ...
  const essentials: ColumnType[] = [
    ...
    {
      id: 'currency',
      label: t('col_currency'),
      icon: CurrencyDollar,
      color: 'bg-emerald-500',
      description: t('desc_currency'),
    },
    {
      id: 'auto_number',
      label: t('col_auto_number'),
      icon: NumberSquareOne,
      color: 'bg-slate-500',
      description: t('desc_auto_number'),
    },
  ];
  ...
  const superUseful: ColumnType[] = [
    {
      id: 'timeline',
      label: t('col_timeline'),
      icon: Calendar,
      color: 'bg-purple-500',
      description: t('desc_timeline'),
    },
    { id: 'url', label: t('col_url'), icon: Link2, color: 'bg-slate-500', description: t('desc_url') },
    {
      id: 'checkbox',
      label: t('col_checkbox'),
      icon: CheckSquare,
      color: 'bg-orange-400',
      description: t('desc_checkbox'),
    },
    ...
    {
      id: 'time_tracking',
      label: t('col_time_tracking'),
      icon: Timer,
      color: 'bg-indigo-500',
      description: t('desc_time_tracking'),
    },
    { id: 'progress', label: t('col_progress'), icon: ChartBar, color: 'bg-teal-500', description: t('desc_progress') },
    {
      id: 'dependencies',
      label: t('col_dependencies'),
      icon: GitBranch,
      color: 'bg-amber-500',
      description: t('desc_dependencies'),
    },
  ];
  ...
  const powerUps: ColumnType[] = [
    ...
    {
      id: 'world_clock',
      label: t('col_world_clock'),
      icon: Globe,
      color: 'bg-sky-400',
      description: t('desc_world_clock'),
    },
  ...
  const [dropdownOptions, setDropdownOptions] =
    useState<{ id: string; label: string; color: string }[]>(getDefaultOptions);
  ...
  const handleAddOption = () => {
    ...
    setDropdownOptions([...dropdownOptions, { id: Date.now().toString(), label: newOptionName, color: randomColor }]);
    setNewOptionName('');
  };
  ...
  const handleCreateCurrency = () => {
    ...
    onSelect('currency', currencyName, [], undefined, {
      currency: { code: selectedCurrency.code, symbol: selectedCurrency.symbol },
    });
  };
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-semibold text-stone-500 dark:text-stone-400">
                {t('dropdown_options')} <span className="text-red-500">*</span>
              </label>
              <button
    ...
    return (
      ...
      >
        <div className="p-4">
          ...
          <div className="mb-4">
            <label className="block text-xs font-semibold text-stone-500 dark:text-stone-400 mb-1">
              {t('column_name_input')} <span className="text-red-500">*</span>
            </label>
            <input
    ...
    >
      ...
      <div className="p-4 pb-2">
        <div className="relative group">
          <Search
            size={14}
            className="absolute start-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-blue-500"
          />
          <input
            type="text"
      ...
      <div className="flex-1 overflow-y-auto px-4 py-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
        ...
        {hasEssentials && (
          <div className="mb-4">
            <div className="text-[13px] text-gray-500 dark:text-gray-400 mb-2 font-medium">
