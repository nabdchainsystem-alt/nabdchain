...
export const AIReportModal: React.FC<AIReportModalProps> = ({ isOpen, onClose, columns, rows, onAddChart }) => {
  ...
  const generateAIAnalysis = async (overrideCols?: Set<string>, overrideSize?: number) => {
    ...
    try {
      ...
      aiLogger.info('Calling backend AI service for chart analysis...');

      const response = await fetch(`${API_URL}/ai/chart`, {
        method: 'POST',
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-stone-500 dark:text-stone-400">
                {mode === 'mode-selection'
                  ? 'Choose how you want to generate insights'
                  : mode === 'config'
            ...
            <div className="p-12 flex flex-col items-center justify-center gap-8 h-full min-h-[400px]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-3xl">
                ...
                >
                  ...
                  <div>
                    <h4 className="text-xl font-bold text-stone-800 dark:text-stone-100 mb-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                      Auto-Generate Report
                    </h4>
                    <p className="text-stone-500 dark:text-stone-400 leading-relaxed">
                      Let AI automatically scan all columns, detect patterns, and generate the most relevant charts for
                      you instantly.
                    </p>
                  </div>
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-stone-500 dark:text-stone-400 leading-relaxed">
                      Manually select specific columns, define sample sizes, and fine-tune exactly what you want to
                      analyze.
                    </p>
                  </div>
                  <div className="mt-auto pt-4 flex items-center gap-2 text-sm font-bold text-blue-600 dark:text-blue-400 opacity-0 group-hover:opacity-100 transition-opacity transform translate-y-2 group-hover:translate-y-0">
                    ...
                    .map((col) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span
                            className={`text-sm font-medium truncate ${isSelected ? 'text-blue-900 dark:text-blue-100' : 'text-stone-600 dark:text-stone-400'}`}
                          >
                            {col.label}
                          </span>
                        </button>
                ...
                <div className="h-full flex flex-col items-center justify-center space-y-4 py-12">
                  ...
                  <div className="text-center">
                    <p className="font-bold text-stone-700 dark:text-stone-300">
                      {isAutoMode
                        ? 'Auto-Analyzing your data...'
                        : `Analyzing ${selectedColumnIds.size} columns & ${sampleSize} rows...`}
                    </p>
                    <p className="text-sm text-stone-500 dark:text-stone-400">
                      Discovering patterns and correlations in your data.
                    </p>
                  </div>
                </div>
              ) : error ? (
        ...
        {mode === 'config' && (
          <div className="p-6 border-t border-stone-100 dark:border-stone-800 bg-stone-50/80 dark:bg-stone-900/80 backdrop-blur-sm flex justify-between items-center">
            <p className="text-sm text-stone-500">
              Selected:{' '}
              <span className="font-bold text-stone-800 dark:text-stone-200">{selectedColumnIds.size} columns</span>
            </p>
            <button
              onClick={() => generateAIAnalysis()}
