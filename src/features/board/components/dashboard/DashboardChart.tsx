...
export const DashboardChart: React.FC<DashboardChartProps> = memo(({ title, options, _height = '300px' }) => {
  // Cache the options to prevent unnecessary chart updates
  const optionsRef = useRef<string>('');
  const cachedOptions = useRef<Record<string, unknown> | null>(null);
  ...
  return (
    <div className="bg-white dark:bg-monday-dark-surface p-6 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm h-full flex flex-col">
      <h3 className="text-sm font-bold text-gray-700 dark:text-gray-200 mb-4 uppercase tracking-wide shrink-0">
        {title}
      </h3>
