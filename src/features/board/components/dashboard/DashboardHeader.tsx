...
export interface ChartConfig {
  ...
  title: string;
  type: 'line' | 'bar' | 'pie';
  data: Record<string, unknown>; // ECharts option object partial
}
      ...
      {config.charts.length > 0 && (
        ...
        >
          {config.charts.map((chart) => (
            <div
              key={chart.id}
              className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-xl p-4 shadow-sm"
            >
