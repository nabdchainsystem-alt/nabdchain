...
export const KPICard: React.FC<KPIConfig> = ({ label, subtitle, value, change, trend, icon, color = 'indigo', sparklineData }) => {
    ...
    return (
        <div className="flex flex-col p-4 bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-xl shadow-sm hover:shadow-md transition-shadow h-full justify-between">
            <div className="flex justify-between items-start mb-1">
                <div className="flex flex-col">
                    <span className="text-xs font-semibold text-stone-500 uppercase tracking-wider">{label}</span>
                    {subtitle && <span className="text-[10px] text-stone-400 font-medium">{subtitle}</span>}
                </div>
            ...
            <div className="flex justify-between items-end mt-1">
                <div className="flex flex-col">
                    <h3 className="text-2xl font-bold text-stone-800 dark:text-stone-100 leading-tight">{value}</h3>
