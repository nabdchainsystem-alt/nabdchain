...
import { X, UploadSimple, FileXls, Table as TableIcon, Check, Warning, ArrowRight } from 'phosphor-react';
...
export const ExcelImportModal: React.FC<ExcelImportModalProps> = ({
    ...
    (sheet: ParsedSheet) => {
      ...
      const mappings: ColumnMapping[] = sheet.headers.map((header) => {
        ...
        return {
          ...
          newColumnType: matchingColumn
            ? undefined
            : guessColumnType(
                header,
                sheet.data.slice(0, 10).map((r) => r[header]),
              ),
        };
      });
  ...
  const guessColumnType = (header: string, sampleData: any[]): string => {
    ...
    if (
      ...
      return 'currency';
    if (headerLower.includes('rating') || headerLower.includes('score') || headerLower.includes('stars'))
      return 'rating';
    if (headerLower.includes('done') || headerLower.includes('complete') || headerLower.includes('check'))
      return 'checkbox';
    ...
    if (nonEmptyValues.length > 0) {
      ...
      if (nonEmptyValues.every((v) => ['true', 'false', 'yes', 'no', '1', '0'].includes(String(v).toLowerCase())))
        return 'checkbox';
    }
  ...
  const handleImport = useCallback(() => {
    ...
    const rows: Row[] = selectedSheet.data
      .map((excelRow) => {
        const row: Row = {
          id: uuidv4(),
          groupId: groupId,
            ...
            >
              {isLoading ? (
                <div className="flex flex-col items-center gap-4">
                  ...
                  <p className="text-sm text-stone-600 dark:text-stone-400">{t('parsing_file') || 'Parsing file...'}</p>
                </div>
              ) : (
                <>
                  ...
                  <p className="text-sm text-stone-500 dark:text-stone-400 mb-4">{t('or') || 'or'}</p>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2 max-h-[400px] overflow-y-auto">
                {columnMappings.map((mapping, index) => (
                  <div key={index} className="flex items-center gap-3 p-3 bg-stone-50 dark:bg-stone-800 rounded-lg">
                    {/* Excel Column */}
                          ...
                          updateMapping(index, {
                            ...
                            newColumnType:
                              val === 'new'
                                ? guessColumnType(
                                    mapping.excelColumn,
                                    selectedSheet.data.slice(0, 10).map((r) => r[mapping.excelColumn]),
                                  )
                                : undefined,
                          });
                        }}
                    ...
                    <thead>
                      <tr className="bg-stone-100 dark:bg-stone-800">
                        {selectedSheet.headers.slice(0, 5).map((header, i) => (
                          <th
                            key={i}
                            className="px-3 py-2 text-start font-medium text-stone-600 dark:text-stone-400 whitespace-nowrap"
                          >
                            {header}
                          </th>
                        ))}
                        {selectedSheet.headers.length > 5 && <th className="px-3 py-2 text-stone-400">...</th>}
                      </tr>
                    </thead>
                    <tbody>
                      {selectedSheet.data.slice(0, 3).map((row, i) => (
                        <tr key={i} className="border-b border-stone-100 dark:border-stone-800">
                          {selectedSheet.headers.slice(0, 5).map((header, j) => (
                            <td
                              key={j}
                              className="px-3 py-2 text-stone-700 dark:text-stone-300 whitespace-nowrap truncate max-w-[150px]"
                            >
                              {String(row[header] ?? '')}
                            </td>
                          ))}
                          {selectedSheet.headers.length > 5 && <td className="px-3 py-2 text-stone-400">...</td>}
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {step === 'mapping' && (
              ...
              >
                ...
                <span className="text-emerald-200">
                  ({selectedSheet?.rowCount} {t('rows') || 'rows'})
                </span>
              </button>
            )}
          </div>
