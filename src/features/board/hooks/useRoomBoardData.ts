...
import { IBoard, ITask, Status, Priority } from '../types/boardTypes';
...
export const useRoomBoardData = (
  storageKey: string,
  initialBoardData?: IBoard | Board,
    ...
    if (initialBoardData) {
      ...
      if (savedBoard && savedBoard.id === flatBoard.id) {
        ...
        const transformedTasks = (Array.isArray(flatBoard.tasks) ? flatBoard.tasks : []).map((t: any) => {
          ...
          return savedTask
            ? { ...baseTask, ...savedTask, name: t.name, status: t.status, priority: t.priority }
            : baseTask;
        });
  ...
  useEffect(() => {
    ...
    setBoard((prev) => {
      ...
      if (!('groups' in flat)) {
        ...
        const prevTaskIds = prev.groups
          .flatMap((g) => g.tasks.map((t) => t.id))
          .sort()
          .join(',');
        const newTaskIds = (flat.tasks || [])
          .map((t) => t.id)
          .sort()
          .join(',');
  ...
  const [isAiLoading, _setIsAiLoading] = useState(false);
  ...
  const addTask = useCallback((groupId: string, name: string = 'New Task', defaults?: Partial<ITask>) => {
    ...
    const safeDefaults = defaults
      ? Object.fromEntries(Object.entries(defaults).filter(([key, value]) => key !== 'id' || value !== undefined))
      : {};
  ...
  const updateTask = useCallback((groupId: string, taskId: string, updates: Partial<ITask>) => {
    setBoard((prev) => {
      ...
      if (updates.status) {
        ...
        const targetGroup = prev.groups.find(
          (g) =>
  ...
  const updateTaskTextValue = useCallback(
    (groupId: string, taskId: string, colId: string, value: string) => {
    ...
    setBoard((prev) => ({
      ...
      groups: [
        ...prev.groups,
        {
          ...
          color:
            color || ['#579bfc', '#e11d48', '#16a34a', '#d97706', '#9333ea', '#db2777'][Math.floor(Math.random() * 6)],
          tasks: [],
          columns: [],
          isPinned: false,
        },
  ...
  const moveTaskToGroup = useCallback(
    (sourceGroupId: string, targetGroupId: string, taskId: string, newIndex: number) => {
      setBoard((prev) => {
  ...
  const tasks = board.groups.flatMap((g) =>
    g.tasks.map((t) => ({
      ...t,
      groupId: g.id,
    })),
