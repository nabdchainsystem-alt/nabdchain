    ...
    <div className="w-full min-h-full pb-32 flex flex-col items-center relative">
      {coverImage && (
        <div className="w-full h-48 md:h-60 relative group shrink-0 animate-in fade-in duration-500 mt-4">
          ...
          <div className="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 flex items-center gap-2 transition-opacity z-10">
            <button className="bg-white/90 hover:bg-white text-stone-600 text-xs font-medium px-2.5 py-1.5 rounded shadow-sm transition-colors border border-stone-200">
              {t('doc_reposition')}
            </button>
      ...
      <div className="w-full max-w-4xl px-8 sm:px-16 py-8 sm:py-12 relative animate-in fade-in duration-500">
        ...
        <div className="group relative mb-6">
          {icon && (
            <div
              className={`mb-4 relative w-fit group/icon animate-in zoom-in-50 duration-300 ${coverImage ? '-mt-10 z-10' : ''}`}
              ref={iconDisplayRef}
            >
              <div
          ...
          <div
            className={`absolute -top-8 left-0 flex items-center gap-2 text-stone-400 text-sm transition-opacity ${isTitleFocused ? 'opacity-0 pointer-events-none' : !title && !icon && !coverImage ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
          >
            {!icon && (
