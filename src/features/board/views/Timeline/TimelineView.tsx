...
export const TimelineView: React.FC<TimelineViewProps> = ({ _roomId, _boardName = 'Board', tasks, onUpdateTasks }) => {
  // RTL Support
  ...
  const _HEADER_HEIGHT = 80; // Taller header to accommodate month and days
  ...
  const handleTaskMouseDown = (e: React.MouseEvent, task: Row, type: 'move' | 'resize', edge?: 'left' | 'right') => {
    e.preventDefault();
    e.stopPropagation();
  ...
  return (
    <div
      className="flex flex-col h-full bg-white dark:bg-monday-dark-surface text-gray-900 dark:text-gray-100 overflow-hidden font-sans relative"
      dir={dir}
    >
      {/* Top Toolbar */}
      <div
        className="absolute top-0 left-0 right-0 h-14 flex items-center justify-between px-4 z-50 bg-white dark:bg-monday-dark-surface"
        dir={dir}
      >
        {/* Left Controls */}
      ...
      >
        <div className="h-full flex flex-col relative" style={{ width: days.length * CELL_WIDTH }}>
          ...
          <div
            className="flex flex-col h-[70px] border-b border-gray-100 dark:border-gray-800 absolute top-0 left-0 right-0 bg-white dark:bg-monday-dark-surface z-20 pointer-events-none"
            dir={dir}
          >
            {/* Month Row */}
            <div className="flex-1 relative">
              {days.map((d, i) => {
                ...
                if (isFirstOfMonth || isStartOfView) {
                  const posStyle = isRTL ? { right: i * CELL_WIDTH + 10 } : { left: i * CELL_WIDTH + 10 };
                  return (
                    <div
                      key={`month-${i}`}
                      className="absolute top-2 text-lg font-semibold text-gray-800 dark:text-gray-100 font-datetime"
                      style={posStyle}
                    >
          ...
          <div className="flex-1 relative mt-[70px]">
                  ...
                  >
                    ...
                    {isToday && (
                      <div className="absolute left-1/2 rtl:left-auto rtl:right-1/2 top-0 bottom-0 w-[1px] bg-red-500 z-10 flex flex-col items-center">
                        ...
                        <div className="mt-1 bg-red-500 text-white text-[9px] px-1 rounded-sm font-bold">
                          {new Date().getDate()}
                        </div>
                      </div>
                    )}
            ...
            <div className="absolute inset-0 pt-4 z-10">
              {processedTasks.map((task, idx) => {
                ...
                const positionStyle = isRTL ? { right: startPos, width, top } : { left: startPos, width, top };
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`w-2 h-2 rounded-full mr-2 rtl:mr-0 rtl:ml-2 ml-1 rtl:ml-0 rtl:mr-1 flex-shrink-0 ${task.status === 'Done' ? 'bg-green-500' : 'bg-yellow-500'}`}
                    />
                    <span className="text-xs font-medium truncate text-gray-700 dark:text-gray-200 select-none">
                      {task.name}
                    </span>
                    ...
                    <div className="absolute left-full rtl:left-auto rtl:right-full ml-2 rtl:ml-0 rtl:mr-2 bg-black text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50 font-datetime">
                      {new Date(task._start).toLocaleDateString(isRTL ? 'ar' : undefined)} -{' '}
                      {new Date(task._end).toLocaleDateString(isRTL ? 'ar' : undefined)}
                    </div>
