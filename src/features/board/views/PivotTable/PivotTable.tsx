...
const ConfigDropdown: React.FC<{
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {isOpen && (
        <PortalPopup triggerRef={ref} onClose={() => setIsOpen(false)} side="bottom" align="start">
...
export const PivotTable: React.FC<PivotTableProps> = ({ roomId }) => {
  ...
  const pivotedData = useMemo(() => {
    ...
    const dataMap = new Map<string, Record<string, unknown>[]>(); // key: "rowVal::colVal", value: aggregated

    rows.forEach((row) => {
      // Extract values, handling arrays (like assignees) or nulls
      const rVal = row[config.rowGroup];
      const cVal = row[config.colGroup];

      // Improved labeling for missing values
      const getLabel = (val: unknown, field: string) => {
    ...
    if (file.name.endsWith('.csv')) {
      ...
      reader.onload = (e) => {
        ...
        try {
          ...
          const jsonData = XLSX.utils.sheet_to_json(worksheet, { header: 1 }) as unknown[][]; // array of arrays
        ...
        <table className="min-w-full border-collapse bg-white dark:bg-monday-dark-surface">
          <thead className="sticky top-0 z-10 bg-gray-50 dark:bg-monday-dark-elevated">
            <tr>
              ...
              <th className="p-4 text-left text-[14.7px] font-semibold text-gray-500 border-b border-gray-200 dark:border-gray-800 border-r dark:border-gray-800 w-48 bg-gray-100/50 dark:bg-monday-dark-elevated">
                <div className="flex items-center gap-2">
                  <span className="text-gray-400 font-normal">
                    {AVAILABLE_FIELDS.find((f) => f.id === config.rowGroup)?.label}
                  </span>
                  <ChevronDown size={12} className="-rotate-90 text-gray-300" />
                  <span className="text-gray-700 dark:text-gray-200">
                    {AVAILABLE_FIELDS.find((f) => f.id === config.colGroup)?.label}
                  </span>
                </div>
              </th>
              ...
              {pivotedData.colHeaders.map((colHeader) => (
                <th
                  key={colHeader}
                  className="p-4 text-left text-[14.7px] font-semibold text-gray-700 dark:text-gray-200 border-b border-gray-200 dark:border-gray-800 min-w-[150px]"
                >
                  {colHeader}
                </th>
              ))}
          ...
          <tbody>
                    ...
                    return (
                      <td key={colHeader} className="p-4 text-sm border-b border-gray-100 dark:border-gray-800">
                        {config.aggregation === 'count' ? (
                          <span
                            className={`inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-xs font-medium ${items.length > 0 ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300' : 'text-gray-400'}`}
                          >
                            {items.length || '-'}
                          </span>
                        ) : (
                          <div className="flex flex-col gap-1 max-h-[100px] overflow-y-auto">
                            {items.map((item: Row) => (
                              <div
                                key={item.id}
                                className="text-xs text-gray-600 dark:text-gray-400 truncate max-w-[150px]"
                              >
            ...
            <tr className="bg-gray-50 dark:bg-monday-dark-elevated font-semibold border-t-2 border-gray-200 dark:border-gray-700">
              ...
              <td className="p-4 text-sm text-indigo-600 dark:text-indigo-400 font-bold">{rows.length}</td>
            </tr>
          </tbody>
        </table>
        {rows.length === 0 && (
