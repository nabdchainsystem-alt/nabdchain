...
import { Priority, Subtask, Task, ColumnType, BoardData, INITIAL_DATA, priorityConfig } from './kanbanConfig';
...
const TaskCard: React.FC<TaskCardProps> = React.memo(
  ({
    ...
    const [activeMenu, setActiveMenu] = useState<'none' | 'priority' | 'tags' | 'context' | 'date' | 'assignee'>(
      'none',
    );
      ...
      >
        <div className="flex justify-between items-start mb-3 group/header">
          {isRenaming ? (
            ...
            <div className="flex items-center gap-2 flex-1 min-w-0">
              {priorityKey !== 'none' && (
                <span
                  className={`w-2 h-2 rounded-full ${priorityMeta.dot}`}
                  title={`${t(priorityKey)} ${t('priority')}`}
                />
              )}
              <span className="font-medium font-serif text-stone-900 dark:text-stone-100 text-sm leading-snug block flex-1 pe-2 break-words">
                {task.title}
              </span>
            </div>
          )}
        ...
        <div className="flex items-center gap-1.5 relative flex-wrap">
          ...
          <button
            ...
            title={assigneePerson ? assigneePerson.name : t('assign')}
          >
            {assigneePerson ? (
              assigneePerson.showUserIcon ? (
                ...
                <img
                  src={assigneePerson.avatar}
                  alt={assigneePerson.name}
          ...
          >
            <Flag
              size={14}
              strokeWidth={2}
              fill={priorityKey !== 'none' && priorityKey !== 'low' ? 'currentColor' : 'none'}
            />
          </button>
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              onOpenReminder(task.id, e.currentTarget.getBoundingClientRect());
            }}
            className={`w-6 h-6 flex items-center justify-center rounded-md hover:bg-gray-100 transition-colors ${reminderTone}`}
            title={reminders.length ? t('manage_reminders') : t('add_reminder')}
          >
            <div className="relative flex items-center">
              <Bell size={14} />
              {reminders.length > 0 && (
                <span
                  className={`absolute -top-1 -end-1 w-1.5 h-1.5 rounded-full ${reminderState === 'triggered' ? 'bg-rose-500' : 'bg-amber-500'}`}
                />
              )}
            </div>
          </button>
          ...
          {task.tags.length > 0 ? (
            task.tags.map((tag) => (
              <span
                key={tag}
                className="text-[10px] px-1.5 py-0.5 rounded-full bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-400 border border-stone-200 dark:border-stone-700 font-sans cursor-pointer hover:bg-stone-200 truncate max-w-[80px]"
                onClick={(e) => {
                  e.stopPropagation();
                  setActiveMenu('tags');
                }}
        ...
        {activeMenu !== 'none' && activeMenu !== 'date' && activeMenu !== 'assignee' && (
          <div
            ref={menuRef}
            className="absolute start-0 top-full mt-2 bg-white rounded-lg shadow-xl border border-gray-100 z-50 overflow-hidden"
            style={{
              top: '100%',
              insetInlineStart: '0',
            ...
            {activeMenu === 'context' && (
              <div className="w-64">
                <div className="flex items-center justify-between p-2 border-b border-gray-100">
                  <button className="flex-1 py-1.5 text-[11px] font-medium text-gray-700 hover:bg-gray-50 rounded text-center whitespace-nowrap">
                    {t('copy_link')}
                  </button>
                ...
                <div className="py-1 max-h-[300px] overflow-y-auto custom-scrollbar">
                  <button
                    ...
                    className="w-full text-start px-4 py-2 hover:bg-gray-50 flex items-center gap-3 text-sm text-gray-700"
                  >
                    <Pencil size={16} className="text-gray-400" /> {t('rename')}
                  </button>
                  <MenuItem icon={ArrowRight} label={t('convert_to')} hasSubmenu />
                  <MenuItem icon={LayoutTemplate} label={t('task_type')} hasSubmenu />
                  <button
                    onClick={() => {
                  ...
                  <MenuItem icon={Timer} label={t('remind_me')} />
                  <MenuItem icon={Bell} label={t('follow_task')} />
                  <MenuItem icon={Mail} label={t('send_email_to_task')} />
                  <MenuItem icon={PlusIcon} label={t('add_to')} hasSubmenu />
                  <MenuItem icon={GitMerge} label={t('merge')} />
                  <MenuItem icon={Move} label={t('move')} />
                  <MenuItem icon={Timer} label={t('start_timer')} />
                  <div className="h-px bg-gray-100 my-1"></div>
                  <MenuItem icon={Link2} label={t('dependencies')} hasSubmenu />
                  <MenuItem icon={LayoutTemplate} label={t('templates')} hasSubmenu />
                  <div className="h-px bg-gray-100 my-1"></div>
                  <MenuItem icon={Archive} label={t('archive')} />
                  <button
                    onClick={() => onDeleteTask(task.id)}
                    className="w-full text-start px-4 py-2 hover:bg-red-50 flex items-center gap-3 text-sm text-red-600"
                  >
        ...
        {/* Status Color Line */}
        <div
          className={`absolute end-0 top-0 bottom-0 w-1.5 rounded-e-xl ${
            priorityKey === 'urgent'
              ? 'bg-red-500'
              : priorityKey === 'high'
                ? 'bg-orange-500'
...
const TaskCreationForm = ({
  ...
  onSave: (title: string, priority: Priority, tags: string[], date?: string) => void;
  onCancel: () => void;
  columnColor?: string;
}) => {
  const { t } = useAppContext();
  ...
  return (
    <div
      ref={containerRef}
      className={`p-3 bg-white rounded-xl shadow-lg border-2 ${getBorderColor(columnColor)} mb-3 relative z-10`}
    >
      ...
      <div className="flex gap-2 relative flex-wrap">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 text-[11px] font-medium transition-colors whitespace-nowrap ${priority !== 'none' ? priorityConfig[priority].color : 'text-gray-500 hover:text-gray-700'}`}
        >
          <Flag size={13} fill={priority !== 'none' && priority !== 'low' ? 'currentColor' : 'none'} />
          {priority !== 'none' ? t(priority) : t('add_priority')}
        </button>

        <button
          onClick={() => setActivePopup(activePopup === 'tags' ? 'none' : 'tags')}
          className={`flex items-center gap-1.5 text-[11px] font-medium transition-colors whitespace-nowrap ${tags.length > 0 ? 'text-purple-600' : 'text-gray-500 hover:text-gray-700'}`}
        >
          <Tag size={13} />
      ...
      {(activePopup === 'priority' || activePopup === 'tags') && (
        <div
          className="absolute top-full start-0 mt-2 bg-white rounded-lg shadow-xl border border-gray-100 z-50 overflow-hidden"
        ...
        >
          ...
          {activePopup === 'tags' && <TagMenu tags={tags} onUpdateTags={(newTags) => setTags(newTags)} />}
        </div>
      )}
    </div>
  );
...
const Column: React.FC<ColumnProps> = ({
  ...
  // Helper to translate standard column titles
  const getTranslatedTitle = (title: string) => {
    ...
    const key = titleMap[titleLower] || titleMap[title.toLowerCase()];
    ...
    return (
      <div className="flex-shrink-0 w-12 h-full pt-3 flex flex-col items-center bg-gray-50 border-r border-gray-100">
        ...
        <div className="writing-vertical-lr transform rotate-180 text-sm font-bold text-gray-500 tracking-wide uppercase">
          {getTranslatedTitle(column.title)}
          <span className="ms-2 px-1.5 py-0.5 rounded-full bg-gray-200 text-xs text-gray-600">{tasks.length}</span>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        className={`flex items-center justify-between px-1 py-3 mb-2 relative group/col-header ${isDragOver ? 'opacity-100' : ''}`}
      >
        {isRenaming ? (
          <input
            ref={inputRef}
          ...
          <div
            className={`px-2 py-1 rounded-md text-xs font-bold uppercase tracking-wide flex items-center gap-2 font-serif text-stone-900 dark:text-stone-100`}
          >
            <div
              className={`w-2 h-2 rounded-full`}
              style={{
                backgroundColor: column.title.toLowerCase().includes('done')
                  ? '#22c55e'
                  : column.title.toLowerCase().includes('progress')
                    ? '#3b82f6'
                    : column.title.toLowerCase().includes('stuck')
                      ? '#f97316'
                      : column.title.toLowerCase().includes('rejected')
                        ? '#be123c'
            ...
            ></div>
            {getTranslatedTitle(column.title)}
            <span className="opacity-40 font-normal ms-1 font-sans text-stone-500">{tasks.length}</span>
          </div>
        )}
          ...
          {showMenu && (
            ...
            >
              <div className="py-2 text-stone-700 dark:text-stone-300">
                <div className="px-4 py-2 text-xs font-semibold text-stone-500">{t('group_options')}</div>
                <button
                  onClick={() => {
                    setIsCollapsed(true);
                    setShowMenu(false);
                  }}
                  className="w-full text-start px-4 py-2 hover:bg-stone-50 dark:hover:bg-stone-800 flex items-center gap-3 text-sm"
                >
                  <ArrowLeftToLine size={16} className="text-stone-400" /> {t('collapse_group')}
                </button>
                <button
                  onClick={() => {
                  ...
                  className="w-full text-start px-4 py-2 hover:bg-stone-50 dark:hover:bg-stone-800 flex items-center gap-3 text-sm"
                >
                  <Archive size={16} className="text-stone-400" /> {t('archive_all_in_group')}
                </button>

                {!['To Do', 'In Progress', 'Done'].includes(column.title) &&
                  !['To Do', 'In Progress', 'Done', 'to-do', 'in-progress', 'done'].includes(column.id) && (
                    <>
                      ...
                      <button
                        ...
                        className="w-full text-start px-4 py-2 hover:bg-stone-50 dark:hover:bg-stone-800 flex items-center gap-3 text-sm text-red-600 hover:text-red-700"
                      >
                        <Trash2 size={16} /> {t('delete_group')}
                      </button>
                    </>
                  )}
              </div>
      ...
      <div className="flex-1 overflow-y-auto px-1 pb-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        {!isAddingTop && (
          <button
            ...
            className={`w-full py-2 flex items-center gap-2 text-sm font-medium ps-2 transition-colors rounded-lg mb-2 ${getButtonStyles(column.color)}`}
          >
            <PlusIcon size={18} />
            {t('add_task')}
          </button>
...
const KanbanBoard: React.FC<KanbanBoardProps> = ({
  ...
  const [teamMembers, setTeamMembers] = useState<
    { id: string; name: string; avatar?: string; showUserIcon?: boolean }[]
  >([]);
        ...
        if (Array.isArray(parsed)) {
          if (typeof parsed[0] === 'string') {
            loadedColumns = parsed.map((s: string) => {
              ...
              else if (lower.includes('progress') || lower.includes('working') || lower.includes('active'))
                color = 'blue';
  ...
  const tasks = useMemo<Task[]>(() => {
    ...
    return externalTasks.map((row: any) => {
      ...
      const targetStatusId = row.statusId || row.status || 'To Do';
  ...
  const handleTaskMove = (taskId: string, newStatusId: string) => {
    const updatedTasks = tasks.map((task) => (task.id === taskId ? { ...task, statusId: newStatusId } : task));
    ...
    <div className="flex flex-col h-full bg-white dark:bg-stone-950 text-stone-800 dark:text-stone-100 font-sans transition-colors duration-300">
      ...
      <header className="flex-none px-8 py-5 flex items-center justify-between bg-white dark:bg-stone-900/80 backdrop-blur-xl z-20 relative border-b border-stone-200 dark:border-stone-800">
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-stone-900 hover:bg-stone-50 dark:hover:bg-stone-800 border border-stone-200 dark:border-stone-700/50 rounded-lg text-xs font-medium text-stone-600 dark:text-stone-300 transition-colors shadow-sm whitespace-nowrap">
        ...
        <div className="flex items-center gap-3" ref={headerMenuRef}>
          <div className="relative group me-2">
            <Search
              size={16}
              className="absolute start-3 top-1/2 -translate-y-1/2 text-stone-400 group-focus-within:text-stone-600 transition-colors"
            />
            <input
              type="text"
              placeholder={t('search_placeholder')}
              className="ps-10 pe-4 py-2 text-sm bg-stone-100/50 dark:bg-stone-900/50 border border-stone-200 dark:border-stone-800 hover:border-stone-300 dark:hover:border-stone-700 focus:border-stone-500/50 focus:bg-white dark:focus:bg-stone-900 rounded-lg outline-none w-56 transition-all font-sans text-stone-800 dark:text-stone-100 placeholder:text-stone-400"
              value={searchQuery}
                ...
                {sortBy !== 'none' && (
                  <>
                    ...
                    <button
                      ...
                      className="w-full text-start px-4 py-2 hover:bg-stone-50 dark:hover:bg-stone-800 text-sm text-stone-500 block"
                    >
                    ...
                    <button
                      ...
                      className="w-full text-start px-4 py-2 hover:bg-stone-50 dark:hover:bg-stone-800 text-sm text-rose-500 block"
                    >
                      {t('clear')} {t('sort')}
                    </button>
                  </>
          ...
          <div className="relative">
            ...
            {activeHeaderMenu === 'filter' && (
              <div className="absolute right-0 rtl:right-auto rtl:left-0 top-full mt-2 w-64 bg-white dark:bg-stone-900 rounded-xl shadow-xl border border-stone-200 dark:border-stone-800 py-2 z-50">
                ...
                <div className="border-t border-stone-100 dark:border-stone-800 my-1" />
                <div className="px-4 py-2 text-xs font-semibold text-stone-400 uppercase tracking-wider">
                  {t('filter_by_assignee')}
                </div>
                {teamMembers.map((person) => (
                  <button
                    key={person.id}
                    onClick={() => setFilterAssignee(filterAssignee === person.id ? null : person.id)}
                    className={`w-full text-start px-4 py-2 hover:bg-stone-50 dark:hover:bg-stone-800 text-sm text-stone-700 dark:text-stone-300 flex items-center gap-2 ${filterAssignee === person.id ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600' : ''}`}
                  >
                    {person.showUserIcon ? (
          ...
          <div className="flex items-center -space-x-2 ms-2">
            <div className="w-8 h-8 rounded-full bg-stone-800 dark:bg-stone-200 text-stone-100 dark:text-stone-900 border-2 border-white dark:border-stone-950 flex items-center justify-center text-xs font-bold font-serif">
              M
            </div>
          </div>
        </div>
        ...
        <div className="flex h-full gap-8">
            ...
            .map((col) => (
              <Column
                ...
                tasks={filteredTasks.filter((t) => {
                  ...
                  if (
                    t.originalStatus &&
                    typeof t.originalStatus === 'string' &&
                    t.originalStatus.toLowerCase() === col.title.toLowerCase()
                  )
                    return true;
          ...
          <div className="flex-shrink-0 w-[19rem] pt-3">
            ...
            >
              ...
              <span className="text-sm font-medium">{t('add_group')}</span>
            </button>
          </div>
        </div>
      </main>
