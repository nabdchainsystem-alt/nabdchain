...
const SortableWidget: React.FC<SortableWidgetProps> = ({ widget, children, onResize, onDelete }) => {
...
`}
      ...
      <div className="flex-1 px-3 py-2 min-h-[60px] flex flex-col justify-center">{children}</div>
    </div>
  );
};
    ...
    <div className="flex flex-col h-full justify-center gap-1">
      <div className="flex justify-between items-end">
        ...
        <div className="w-16 h-8 opacity-50 mb-0.5">
          <svg
            viewBox="0 0 100 40"
            className={`w-full h-full stroke-2 fill-none ${isPositive ? 'stroke-emerald-500' : 'stroke-red-500'} `}
          >
      ...
      <div className="flex items-center gap-2">
        <div
          className={`flex items-center gap-0.5 text-[10px] font-bold ${isPositive ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'} `}
        >
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center bg-gray-50/50 dark:bg-monday-dark-surface rounded-lg border border-dashed border-gray-200 dark:border-gray-800 p-2">
      <div className="text-center">
        ...
        <p className="text-xs font-medium text-gray-600 dark:text-gray-300">{widget.chartType || 'Chart'}</p>
      </div>
    </div>
  );
};
...
const _TaskStatusBarChart = ({ tasks, t }: { tasks: any[]; t: (key: string) => string }) => {
...
const _PriorityGaugeWidget = ({ tasks, t }: { tasks: any[]; t: (key: string) => string }) => {
  ...
  const segments = [
    ...
    { label: 'Medium', count: priorityCounts.medium, color: '#f59e0b', offset: priorityCounts.high / total },
    {
      ...
      color: '#22c55e',
      offset: (priorityCounts.high + priorityCounts.medium) / total,
    },
    {
      label: 'None',
      count: priorityCounts.none,
      color: '#d1d5db',
      offset: (priorityCounts.high + priorityCounts.medium + priorityCounts.low) / total,
    },
  ];
...
// --- Recent Tasks Widget ---
const _RecentTasksWidget = ({ tasks, t }: { tasks: any[]; t: (key: string) => string }) => {
  const recentTasks = React.useMemo(() => {
    return [...tasks].filter((t) => t.name).slice(0, 5);
  }, [tasks]);
...
const _UpcomingDeadlinesWidget = ({ tasks, t }: { tasks: any[]; t: (key: string) => string }) => {
      ...
      <div className="space-y-2">
        {upcomingTasks.length === 0 ? (
          ...
          upcomingTasks.map((task, idx) => (
            ...
            >
              <span className="text-xs text-gray-700 dark:text-gray-300 truncate flex-1">{task.name}</span>
              <span className="text-[10px] font-medium text-amber-600 dark:text-amber-400 ml-2 font-datetime">
...
export const OverviewView: React.FC<OverviewViewProps> = ({ boardId, tasks = [] }) => {
  ...
  const [_showKPIMenu, setShowKPIMenu] = useState(false);
  const [_showChartMenu, setShowChartMenu] = useState(false);
  ...
  const _handleAddKPI = (count: number) => {
    const newWidgets: Widget[] = Array(count)
      .fill(null)
      .map((_, i) => ({
        id: `kpi - ${Date.now()} -${i} `,
        type: 'kpi',
  ...
  const _handleAddChart = (chartType: string) => {
