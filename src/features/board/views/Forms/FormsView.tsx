      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="flex-1 font-medium text-stone-800 dark:text-stone-200">{displayLabel}</span>
...
const FormPreview: React.FC<{
  ...
  const renderField = (field: FormField) => {
    const baseInputClass =
      'w-full px-3 py-2 border border-stone-300 dark:border-stone-600 rounded-lg bg-white dark:bg-stone-800 text-stone-800 dark:text-stone-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent';
        ...
        return (
          <div className={`flex items-center gap-1 ${isRTL ? 'flex-row-reverse justify-end' : ''}`}>
            {Array.from({ length: max }, (_, i) => (
              <button key={i} type="button" onClick={() => handleChange(field.id, i + 1)} className="p-0.5">
                <Star
                  size={24}
          ...
          >
            <div
              className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${
                isRTL ? (values[field.id] ? 'right-7' : 'right-1') : values[field.id] ? 'left-7' : 'left-1'
              }`}
            />
          </button>
        );
...
export const FormsView: React.FC<FormsViewProps> = ({ roomId }) => {
  ...
  const [selectedFormId, setSelectedFormId] = useState<string | null>(forms.length > 0 ? forms[0].id : null);
  ...
  const updateForm = useCallback(
    (formId: string, updates: Partial<Form>) => {
      const newForms = forms.map((f) => (f.id === formId ? { ...f, ...updates, updatedAt: Date.now() } : f));
      saveForms(newForms);
    },
    [forms, saveForms],
  );
  ...
  const updateField = useCallback(
    (fieldId: string, updates: Partial<FormField>) => {
      ...
      const newFields = selectedForm.fields.map((f) => (f.id === fieldId ? { ...f, ...updates } : f));
  ...
  return (
    <div className="h-full flex bg-stone-50 dark:bg-stone-900" dir={isRTL ? 'rtl' : 'ltr'}>
      {/* Sidebar - Forms List */}
      <div
        className={`w-64 flex-shrink-0 bg-white dark:bg-stone-800 ${isRTL ? 'border-l' : 'border-r'} border-stone-200 dark:border-stone-700 flex flex-col`}
      >
          ...
          {forms.length === 0 ? (
            ...
            forms.map((form) => (
              <button
                ...
                className={`w-full ${isRTL ? 'text-right' : 'text-left'} px-3 py-2 rounded-lg mb-1 transition-colors ${
              ...
              >
                ...
                <p className="text-xs text-stone-500 mt-0.5">
                  {form.fields.length} {t('forms_fields')}
                </p>
              </button>
            ))
          )}
      ...
      {/* Preview Modal */}
      {previewForm && <FormPreview form={previewForm} onClose={() => setPreviewForm(null)} t={t} isRTL={isRTL} />}
    </div>
  );
};
