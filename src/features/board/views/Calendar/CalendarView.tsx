...
import { ITask, IBoard, STATUS_COLORS } from '../../types/boardTypes';
...
const _WEEKDAY_SHORT = ['S', 'M', 'T', 'W', 'T', 'F', 'S'];
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <span
          className="text-sm font-semibold text-stone-700 dark:text-stone-300 tracking-tight"
...
// --- Draggable Task ---
const DraggableTask: React.FC<{ task: ITask; onClick: (e: React.MouseEvent) => void; compact?: boolean }> = ({
  task,
  onClick,
  compact,
}) => {
        ...
        {days.map((day, i) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`text-xs ${isToday ? 'text-stone-700 dark:text-stone-300 font-medium' : 'text-stone-400'}`}
              >
        ...
        <div className="flex">
          ...
          <div className="w-16 flex-shrink-0">
            {HOURS.map((hour, i) => (
              <div
                key={i}
                className="h-14 border-b border-stone-100 dark:border-stone-800 flex items-start justify-end pe-2 pt-0"
              >
...
export const CalendarView: React.FC<CalendarViewProps> = (props) => {
  ...
  const effectiveKey = propBoard ? `shadow-${roomId || 'calendar'}` : storageKey || roomId || 'demo-board';
  ...
  return (
    <div className="flex h-full bg-white dark:bg-stone-900" dir={dir}>
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        {/* Sidebar */}
        <div
          className={`${sidebarCollapsed ? 'w-0' : 'w-52'} flex-shrink-0 border-e border-stone-200 dark:border-stone-700 flex flex-col transition-all overflow-hidden`}
        >
          <div className="p-4">
            {/* New Event Button */}
            <button
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-4 px-4 py-3 border-b border-stone-200 dark:border-stone-700 flex-shrink-0">
            ...
            <div className="flex items-center gap-1">
              <button
                onClick={prev}
                className="p-1.5 hover:bg-stone-100 dark:hover:bg-stone-800 rounded text-stone-500 transition-colors"
              >
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="grid grid-cols-7 border-b border-stone-200 dark:border-stone-700 flex-shrink-0">
                {[0, 1, 2, 3, 4, 5, 6].map((dayIndex) => (
                  <div
                    key={dayIndex}
                    className="px-2 py-2 text-xs font-medium text-stone-500 dark:text-stone-400 border-e border-stone-200 dark:border-stone-700 last:border-e-0"
                  >
