...
export const RequestsBacklogAging: React.FC = () => {
  ...
  const agingBuckets = {
    ...
    yAxis: { type: 'value' },
    series: [{ name: 'Requests', type: 'bar', data: [110, 35, 45, 18, 6], itemStyle: { color: '#3b82f6' } }],
  };
    ...
    series: [
      {
        name: 'Total Backlog',
        type: 'line',
        data: [180, 185, 190, 205, 210, 215, 215],
        smooth: true,
    ...
    <div className="flex flex-col h-full bg-gray-50 dark:bg-monday-dark-bg p-6 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 h-96">
        <div className="lg:col-span-2 h-full">
          <DashboardChart title="Aging Buckets" options={agingBuckets} height="100%" />
        </div>
        <div className="h-full">
          <DashboardChart title="Blocked Orders by Reason" options={blockedByReason} height="100%" />
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 h-96">
        <div className="h-full">
          <DashboardChart title="Backlog Trend (7 Days)" options={backlogTrend} height="100%" />
        </div>
