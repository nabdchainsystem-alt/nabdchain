...
export const ApprovalsFlowDashboard: React.FC = () => {
  ...
  const approvalTimeByStage = {
    ...
    series: [{ name: 'Avg Time', type: 'line', data: [4, 12, 36, 24], smooth: true, itemStyle: { color: '#3b82f6' } }],
  };
  ...
  const tableColumns = [
    ...
    { header: 'Time Pending', accessor: 'approval_time_days' },
    {
      ...
      render: (val: string) => (
        <span
          className={`px-2 py-1 rounded-full text-xs font-semibold ${val === 'Breached' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}
        >
          {val}
        </span>
      ),
  ...
  const tableData = [
    {
      ...
      approval_time_days: '3.5',
      sla_status: 'Breached',
    },
    {
      request_id: 'PR-8824',
      department: 'IT',
      approval_stage: 'Dept Head',
      approval_status: 'Pending',
    ...
    },
    {
      ...
      approval_stage: 'Final Auth',
      approval_status: 'Pending',
      approver: 'CEO',
      created_date: '2024-12-17',
      approval_time_days: '1.2',
      sla_status: 'On Track',
    },
    {
      request_id: 'PR-8830',
      ...
      approval_time_days: '2.1',
      sla_status: 'Warning',
    },
    {
      request_id: 'PR-8835',
      department: 'HR',
      approval_stage: 'Line Manager',
      approval_status: 'Pending',
      approver: 'HR Mgr',
      created_date: '2024-12-18',
      approval_time_days: '0.1',
      sla_status: 'On Track',
    },
  ];
    ...
    <div className="flex flex-col h-full bg-gray-50 dark:bg-monday-dark-bg p-6 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 h-96">
        <div className="lg:col-span-2 h-full">
          <DashboardChart title="Pending by Stage" options={pendingByStage} height="100%" />
        </div>
        <div className="h-full">
          <DashboardChart title="Avg Approval Time (Hours)" options={approvalTimeByStage} height="100%" />
        </div>
      </div>
