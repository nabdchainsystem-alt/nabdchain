...
interface RFQSectionProps {
  ...
  onSendToOrder?: (rfq: Record<string, unknown>) => void;
}
  ...
  const kpis = useMemo(() => {
    ...
    const stats = rfqs.reduce(
      (acc, rfq) => {
        ...
        const created = rfq.createdDate
          ? new Date(rfq.createdDate).getTime()
          : rfq.date
            ? new Date(rfq.date).getTime()
            : now;
    ...
    const avgDaysOpen =
      stats.pendingForAvg.length > 0
        ? Math.round(
            stats.pendingForAvg.reduce((acc, created) => acc + (now - created) / (1000 * 60 * 60 * 24), 0) /
              stats.pendingForAvg.length,
          )
        : 0;
      ...
      <div className="bg-white dark:bg-monday-dark-surface border border-gray-200 dark:border-gray-800 rounded-lg shadow-sm">
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {paginatedRfqs.map((rfq, idx) => {
                ...
                const sendLabel = isDeleted
                  ? 'Marked as deleted'
                  : alreadySentToOrder
                    ? 'Already sent to Order'
                    : 'Send to Order';
        ...
        <div className="px-5 py-3 border-t border-gray-200 dark:border-gray-800 flex items-center justify-between bg-gray-50/50 dark:bg-gray-800/20 rounded-b-lg">
          <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
            ...
            <span className="ml-2">
              {rfqs.length === 0 ? '0-0 of 0' : `${rfqStart + 1}-${rfqEnd} of ${rfqs.length}`}
            </span>
          </div>
