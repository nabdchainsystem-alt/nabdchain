...
const APPROVAL_THEMES: Record<string, { badge: string; border: string; rowBg: string }> = {
  Approved: {
    badge:
      'bg-emerald-50 text-emerald-700 border-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-900/40',
    border: 'border-emerald-500',
    rowBg: 'hover:bg-emerald-50/50 dark:hover:bg-emerald-500/5',
  },
  'On Hold': {
    badge:
      'bg-amber-50 text-amber-700 border-amber-100 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-900/40',
    ...
    >
      ...
      >
        <div className="flex items-start justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800">
          <div>
            <p className="text-[11px] font-semibold uppercase text-gray-500 dark:text-gray-400 tracking-wide">
              Request
            </p>
        ...
        <div className="px-6 py-4 space-y-4 overflow-y-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {meta.map((entry) => (
              ...
              >
                ...
                <p className="text-sm font-semibold text-gray-900 dark:text-gray-100 break-all">{entry.value}</p>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-gray-500 dark:text-gray-400">Each item in this request at a glance</span>
            </div>
                  ...
                  {items.length === 0 ? (
                    ...
                    items.map((item: any) => (
                      <tr key={item.id || item.itemCode}>
                        ...
                        <td className="px-4 py-2 text-gray-700 dark:text-gray-300">{item.dueDate || '-'}</td>
                      </tr>
                    ))
                  )}
                </tbody>
...
export const ProcurementOverview: React.FC = () => {
  ...
  const [_loadingRfqs, setLoadingRfqs] = useState(false);
  ...
  const handleSendRfqToOrder = async (rfq: any) => {
    ...
    try {
      ...
      setRfqs((prev) =>
        prev.map((r) =>
          r.id === rfq.id ? { ...r, status: 'Sent to PO', sentToOrder: true, orderId: createdOrder.id } : r,
        ),
      );
      await procurementService.updateRfq(token, rfq.id, {
        status: 'Sent to PO',
        sentToOrder: true,
  ...
  const totalKpis = React.useMemo(() => {
    ...
    const today = activeRequests.filter(
  ...
  const calendarData = React.useMemo(() => {
    ...
    if (calendarView === 'daily') {
      ...
      heading = calendarDate.toLocaleDateString('default', {
        weekday: 'long',
        month: 'long',
        day: 'numeric',
        year: 'numeric',
  ...
  const scheduleTimeSlots = React.useMemo(
  ...
  return (
    <div
      ref={containerRef}
      className={`w-full h-full p-6 pb-20 overflow-y-auto bg-white dark:bg-monday-dark-surface relative font-sans space-y-6 ${isFullScreen ? 'fixed inset-0 z-[999]' : ''} [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]`}
    >
                ...
                <div className="flex items-center gap-1 ml-2">
                  <button
                    onClick={() => {
                      ...
                      else if (calendarView === '5days' || calendarView === 'weekly')
                        setCalendarDate((prev) => addDays(prev, -7));
                      else if (calendarView === 'yearly')
                        setCalendarDate((prev) => new Date(prev.getFullYear() - 1, 0, 1));
              ...
              {isScheduleView ? (
                <div className="flex-1 flex flex-col overflow-hidden border border-gray-200 dark:border-gray-800 rounded-lg">
                  <div
                    ...
                    style={{
                      gridTemplateColumns: `80px repeat(${isScheduleView ? (calendarView === '5days' ? workWeekDays.length : fullWeekDays.length) : 0}, minmax(0, 1fr))`,
                    }}
                  >
                  ...
                  <div
                    ...
                    style={{
                      gridTemplateColumns: `80px repeat(${calendarView === '5days' ? workWeekDays.length : fullWeekDays.length}, minmax(0, 1fr))`,
                    }}
                  >
                    <div className="border-r border-gray-200 dark:border-gray-800 bg-gray-50/70 dark:bg-gray-900/30 text-[11px] text-gray-500 dark:text-gray-400">
                      {scheduleTimeSlots.map((slot, idx) => (
                        <div
                          key={slot}
                          className={`h-full px-3 flex items-start ${idx === 0 ? 'pt-2 pb-1' : 'py-4'} border-b border-gray-200 dark:border-gray-800`}
                        >
                          {slot}
                        </div>
                      ))}
                    ...
                    {(calendarView === '5days' ? workWeekDays : fullWeekDays).map((day) => {
                      ...
                      return (
                        <div
                          key={key}
                          className="grid border-r border-gray-200 dark:border-gray-800"
                          style={{ gridTemplateRows: `repeat(${scheduleTimeSlots.length}, minmax(0, 1fr))` }}
                        >
                          {scheduleTimeSlots.map((slot, idx) => (
                            <div
                              key={slot}
                              className={`relative border-b border-gray-200 dark:border-gray-800 ${idx === scheduleTimeSlots.length - 1 ? '' : ''}`}
                            >
                              {idx === 0 && counts && (
                              ...
                              {!counts && idx === 0 && (
                                <div className="absolute inset-x-2 top-2 text-[11px] text-gray-400 dark:text-gray-600">
                                  No entries
                                </div>
                              )}
                            </div>
                    ...
                    {calendarData.cells.map((day, idx) => {
                      if (!day) {
                        return (
                          <div
                            key={idx}
                            className="h-full rounded-lg border border-dashed border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/40"
                          ></div>
                        );
                      }
                      const key = day.toISOString().split('T')[0];
                      ...
                      const dayLabel =
                        calendarView === 'yearly' ? day.toLocaleString('default', { month: 'short' }) : day.getDate();
                      const subLabel =
                        calendarView === 'yearly'
                      ...
                      return (
                        <div
                          key={key}
                          className={`h-full rounded-lg border bg-white dark:bg-monday-dark-surface flex flex-col p-2 transition-colors ${
                            isToday
          ...
          <div className="grid grid-cols-4 gap-4">
            {activeKpis.map((kpi, index) => (
              ...
              >
                ...
                <div>
                  <h3 className="text-2xl font-bold text-gray-900 dark:text-gray-100 tracking-tight mb-1">
                    {kpi.value}
                  </h3>
                  <div className="flex items-center justify-between">
      ...
      <div className="bg-white dark:bg-monday-dark-surface border border-gray-200 dark:border-gray-800 rounded-lg shadow-sm flex-1 flex flex-col">
                ...
                <div className="absolute right-0 mt-2 w-52 bg-white dark:bg-monday-dark-surface border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-3 space-y-3 z-20">
                  <div className="space-y-1">
                    ...
                    >
                      ...
                      {requestFilterOptions.statuses.map((s) => (
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      <option value="All">All</option>
                      {requestFilterOptions.priorities.map((p) => (
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      <option value="All">All</option>
                      {requestFilterOptions.departments.map((d) => (
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-800/40 text-[11px] font-semibold text-gray-500 uppercase tracking-wider border-b border-gray-200 dark:border-gray-800 whitespace-nowrap">
                ...
                <th className="px-4 py-3 cursor-pointer hover:text-gray-700 dark:hover:text-gray-300 group select-none">
                  <div className="flex items-center gap-1">
                    Date <ArrowUpDown size={12} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
                </th>
                <th className="px-4 py-3 cursor-pointer hover:text-gray-700 dark:hover:text-gray-300 group select-none">
                  <div className="flex items-center gap-1">
                ...
                <th className="px-4 py-3 cursor-pointer hover:text-gray-700 dark:hover:text-gray-300 group select-none">
                  <div className="flex items-center gap-1">
                    Warehouse <ArrowUpDown size={12} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
                </th>
                <th className="px-4 py-3">Related to</th>
                <th className="px-4 py-3 cursor-pointer hover:text-gray-700 dark:hover:text-gray-300 group select-none">
                  <div className="flex items-center gap-1">
                    Status <ArrowUpDown size={12} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
                </th>
                <th className="px-4 py-3 cursor-pointer hover:text-gray-700 dark:hover:text-gray-300 group select-none">
                  <div className="flex items-center gap-1">
                    Priority <ArrowUpDown size={12} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
                </th>
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <span
                            className={`px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wide border ${theme.badge}`}
                          >
                            {approvalStatus}
                          </span>
                          <div className="flex items-center gap-1">
        ...
        <div className="px-5 py-3 border-t border-gray-200 dark:border-gray-800 flex items-center justify-between bg-gray-50/50 dark:bg-gray-800/20 rounded-b-lg">
          <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
            ...
            <span className="ml-2">
              {tableData.length === 0 ? '0-0 of 0' : `${startIdx + 1}-${endIdx} of ${tableData.length}`}
            </span>
          </div>
      ...
      <RequestDetailsModal request={selectedRequestDetails} onClose={() => setSelectedRequestDetails(null)} />
      ...
      <RFQSection rfqs={rfqs} onDeleteRfq={handleDeleteRfq} onSendToOrder={handleSendRfqToOrder} />
        ...
        <div className="bg-white dark:bg-monday-dark-surface border border-gray-200 dark:border-gray-800 rounded-lg p-5 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                Distribution of orders across departments
              </p>
            </div>
            <button className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 rounded text-gray-400 transition-colors">
              <MoreHorizontal size={16} />
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {paginatedOrders.length === 0 ? (
                  ...
                  paginatedOrders.map((order, idx) => {
                    const approvalTheme =
                      ORDER_APPROVAL_THEMES[order.approvals || 'Pending'] || ORDER_APPROVAL_THEMES.Pending;
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-center">
                          <div className="flex items-center justify-center gap-2 w-full">
                            <span
                              className={`px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wide border ${approvalTheme.badge}`}
                            >
            ...
            <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
              ...
              <span className="ml-2">
                {filteredOrders.length === 0
                  ? '0-0 of 0'
                  : `${ordersStart + 1}-${ordersEnd} of ${filteredOrders.length}`}
              </span>
            </div>
