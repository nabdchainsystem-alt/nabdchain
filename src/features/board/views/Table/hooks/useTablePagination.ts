...
export function useTablePagination({
    ...
    const startIndex = useMemo(() =>
        rowsPerPage > 0 ? (currentPage - 1) * rowsPerPage : 0
    , [currentPage, rowsPerPage]);

    const endIndex = useCallback((totalRows: number) => {
        if (rowsPerPage <= 0) return totalRows;
        return Math.min(startIndex + rowsPerPage, totalRows);
    }, [startIndex, rowsPerPage]);

    const totalPages = useCallback((totalRows: number) => {
        if (rowsPerPage <= 0) return 1;
        return Math.ceil(totalRows / rowsPerPage);
    }, [rowsPerPage]);

    const getPaginatedRows = useCallback((rows: Row[]): Row[] => {
    ...
    const goToNextPage = useCallback((totalRows: number) => {
    ...
    const goToLastPage = useCallback((totalRows: number) => {
