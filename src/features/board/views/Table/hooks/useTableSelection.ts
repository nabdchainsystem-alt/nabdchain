...
export function useTableSelection({ columns }: UseTableSelectionProps): UseTableSelectionReturn {
  const selectColumnId = useMemo(() => columns.find((c) => c.id === 'select')?.id || 'select', [columns]);

  const getSelectedRows = useCallback(
    (tableGroups: TableGroup[]): Row[] => {
      const allRows: Row[] = [];
  ...
  const handleSelectAll = useCallback(
    (checked: boolean, setTableGroups: React.Dispatch<React.SetStateAction<TableGroup[]>>) => {
  ...
  const areAllSelected = useCallback(
    (tableGroups: TableGroup[]): boolean => {
  ...
  const toggleRowSelection = useCallback(
    (rowId: string, groupId: string, setTableGroups: React.Dispatch<React.SetStateAction<TableGroup[]>>) => {
      setTableGroups((prevGroups) => {
        return prevGroups.map((group) => {
