...
import { MagnifyingGlass, UserCircle, Funnel, ArrowsDownUp, EyeSlash, Export, UploadSimple } from 'phosphor-react';
...
const PersonAvatarItem: React.FC<PersonAvatarItemProps> = ({ person, isActive, onClick }) => {
  ...
  const initial = (
    typeof person === 'string' ? person.charAt(0) : person.name ? person.name.charAt(0) : '?'
  ).toUpperCase();
      ...
      <div className="flex items-center gap-3 text-stone-500 dark:text-stone-400 relative">
        ...
        <div className="relative">
          ...
          {isPersonFilterOpen && (
            ...
            >
              <div className="mb-3">
                <span className="text-sm font-semibold text-stone-800 dark:text-stone-200">Filter by person</span>
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                {uniquePeople.length === 0 && (
                  <div
                    ...
                    title={t('no_assigned_users_found')}
                  >
              ...
              <div className="space-y-3">
                {filters.map((filter, _idx) => (
                  <div key={filter.id} className="flex items-center gap-2">
                    ...
                    <button onClick={() => removeFilter(filter.id)} className="text-stone-400 hover:text-red-500 p-1">
                      <Trash size={16} />
                    </button>
                  </div>
                ))}
                {filters.length === 0 && <p className="text-stone-400 text-sm text-center py-4">No filters applied</p>}
              </div>
            </div>
          )}
        </div>
              ...
              <div className="space-y-3">
                {sortRules.map((rule) => (
                  <div key={rule.id} className="flex items-center gap-2">
                    ...
                    <button onClick={() => removeSortRule(rule.id)} className="text-stone-400 hover:text-red-500 p-1">
                      <Trash size={16} />
                    </button>
                  </div>
                ))}
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {filterableColumns
                  ...
                  .map((col) => (
                    <label
                      key={col.id}
                      className="flex items-center gap-2 cursor-pointer hover:bg-stone-50 dark:hover:bg-stone-700 p-2 rounded-lg"
                    >
                      <input
                        type="checkbox"
