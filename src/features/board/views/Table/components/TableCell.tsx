...
import { safeEvalFormula, safeEvalArithmetic } from '@/utils/safeEval';
import {
  MapPin,
  FileText,
...
export const TableCell: React.FC<TableCellProps> = React.memo(
  ({
    ...
    const { currency: globalCurrency, t, _dir, language, globalTags, addGlobalTag } = useAppContext();
    ...
    if (col.type === 'status') {
      ...
      const statusObj = customStatuses.find(
        (s) => s.title === normalizedValue || s.id === normalizedValue || s.title === value || s.id === value,
      );
      const color =
        statusObj?.color ||
      ...
      const translatedStatus =
        normalizedValue && STATUS_TRANSLATION_KEYS[normalizedValue]
          ? t(STATUS_TRANSLATION_KEYS[normalizedValue])
          : value;
        ...
        <div className="relative w-full h-full flex items-center justify-center p-1">
          ...
          >
            {value ? (
              ...
              <span
                className={`${row.id === CREATION_ROW_ID ? 'text-[11px]' : 'text-xs'} text-stone-400 group-hover:text-stone-500`}
              >
                -
              </span>
            )}
    ...
    if (col.type === 'date') {
      ...
      return (
        <div className="relative w-full h-full">
          ...
          >
            <span
              className={`text-sm font-datetime truncate ${value ? 'text-stone-600 dark:text-stone-300' : 'text-stone-400'}`}
              dir="ltr"
            >
        ...
        <div className="relative w-full h-full">
          ...
          {isActiveCell && activeCell?.trigger && (
            <CurrencyPicker
              ...
              onSelect={(newValue, _newCurrency) => {
        ...
        <div className="relative w-full h-full p-1">
          ...
          {isActiveCell && activeCell?.trigger && (
            <SelectPicker
              ...
              onEdit={
                onEditColumnOption
                  ? (optionId, newLabel, newColor) => onEditColumnOption(col.id, optionId, newLabel, newColor)
                  : undefined
              }
        ...
        <div className="relative w-full h-full p-1 group/doc">
          ...
          >
            {value ? (
              ...
              <span className="text-xs text-stone-400 group-hover/doc:text-stone-500 transition-colors">
                {t('select_doc')}
              </span>
            )}
          </button>
    ...
    if (col.type === 'priority') {
      ...
      const bgClass =
        normalized && PRIORITY_STYLES[normalized]
          ? PRIORITY_STYLES[normalized]
          : 'hover:bg-stone-100 dark:hover:bg-stone-800/50';
      // Get translated priority label
      const translatedPriority =
        normalized && PRIORITY_TRANSLATION_KEYS[normalized] ? t(PRIORITY_TRANSLATION_KEYS[normalized]) : normalized;
        ...
        <div className="relative w-full h-full group/url">
          ...
          >
            {urlString ? (
              ...
              <span className={`${row.id === CREATION_ROW_ID ? 'text-[11px]' : 'text-xs'} text-stone-400`}>
                {t('add_url')}
              </span>
            )}
          </button>
          ...
          >
            {displayText ? (
              ...
              <div className="flex items-center gap-2 text-stone-400">
                <MapPin size={14} />
                <span className={`${row.id === CREATION_ROW_ID ? 'text-[11px]' : 'text-xs'}`}>
            ...
            {files.length > 0 && files[0] ? (
              <div className="flex items-center gap-1.5 truncate flex-1 min-w-0">
                ...
                <span className="text-sm text-stone-600 dark:text-stone-300 truncate">
                  {getShortName(files[0]?.title || files[0]?.name)}
                </span>
                {files.length > 1 && <span className="text-xs text-stone-400 shrink-0">+{files.length - 1}</span>}
              </div>
            ) : (
                ...
                <div className="max-h-48 overflow-y-auto">
                  {files
                    ...
                    .map((file: any, idx: number) => (
                      <div
                        key={file?.id || idx}
                        className="flex items-center gap-2 p-2 hover:bg-stone-50 dark:hover:bg-stone-800/50 border-b border-stone-50 dark:border-stone-800/50 last:border-0"
                      >
            ...
            {ratingValue ? (
              ...
              <div className="flex items-center gap-0.5">
                {Array.from({ length: maxRating }, (_, i) => (
                  <Star key={i} size={14} weight="regular" className="text-stone-300 dark:text-stone-600" />
                ))}
              </div>
            )}
          </button>
          ...
          >
            {emailValue ? (
              <div className="flex items-center gap-1.5 max-w-full overflow-hidden">
                ...
                <span className="text-xs text-stone-700 dark:text-stone-300 truncate">{emailValue}</span>
              </div>
            ) : (
              <div className="flex items-center gap-1">
          ...
          >
            {phoneValue ? (
              <div className="flex items-center gap-1.5 max-w-full overflow-hidden">
                ...
                <span className="text-xs text-stone-700 dark:text-stone-300 truncate">{phoneValue}</span>
              </div>
            ) : (
              <div className="flex items-center gap-1">
          ...
          >
            {tzDisplay ? (
              <div className="flex items-center gap-1.5 max-w-full overflow-hidden">
                ...
                <span className="text-xs text-stone-700 dark:text-stone-300 truncate">{tzDisplay.city}</span>
                <span className="text-xs font-mono text-stone-500 dark:text-stone-400">{tzDisplay.time}</span>
              </div>
            ) : (
              <div className="flex items-center gap-1">
          ...
          >
            {tagsValue && tagsValue.length > 0 ? (
              <div className="flex items-center gap-1 flex-wrap justify-center max-w-full overflow-hidden">
                ...
                {tagsValue.length > 3 && <span className="text-[10px] text-stone-400">+{tagsValue.length - 3}</span>}
              </div>
            ) : (
              <div className="flex items-center gap-1">
    ...
    if (col.type === 'auto_number') {
      ...
      return (
        <div className="w-full h-full flex items-center justify-center">
          <span className="text-sm text-stone-500 dark:text-stone-400">{displayNumber}</span>
        </div>
      );
    }
    ...
    if (col.type === 'time_tracking') {
      const timeData =
        typeof value === 'object' && value !== null
          ? value
          : { totalTime: typeof value === 'number' ? value : 0, isRunning: false };
      ...
      return (
        <div className="w-full h-full flex items-center justify-center gap-2">
          ...
          <span
            className={`text-sm tabular-nums ${isRunning ? 'text-emerald-600 dark:text-emerald-400' : 'text-stone-500 dark:text-stone-400'}`}
          >
          ...
          >
            {totalDeps > 0 ? (
              <div className="flex items-center gap-2">
                {blockedBy.length > 0 && (
                  <div
                    className="flex items-center gap-0.5 px-1.5 py-0.5 bg-red-100 dark:bg-red-900/30 rounded"
                    title={t('blocked_by') || 'Blocked by'}
                  >
                ...
                {blocking.length > 0 && (
                  <div
                    className="flex items-center gap-0.5 px-1.5 py-0.5 bg-amber-100 dark:bg-amber-900/30 rounded"
                    title={t('blocking') || 'Blocking'}
                  >
        ...
        try {
          ...
          if (columnRefs) {
            ...
            if (/^[\d\s+\-*/().'"a-zA-Z\u0600-\u06FF]+$/.test(evalFormula) || evalFormula.includes('"')) {
              try {
                const result = safeEvalFormula(evalFormula);
                if (result === null) {
                  hasError = true;
                } else {
                  calculatedValue =
                    typeof result === 'number' && !isNaN(result) ? Math.round(result * 100) / 100 : result;
                }
              } catch (err) {
            ...
            const result = safeEvalArithmetic(formula);
            if (result !== null) {
    ...
    if (col.type === 'button') {
      const cellConfig =
        typeof value === 'object' && value !== null
          ? (value as { label?: string; color?: string; action?: { type: string; config?: Record<string, unknown> } })
          : null;
    ...
    return (
      <div className="h-full w-full group/namecell relative">
        <input
          ...
          style={{
            ...cellStyle,
            textAlign: col.id === primaryColId ? 'start' : 'center',
            paddingRight: isNameColumn && !isCreationRow ? '32px' : undefined,
          }}
