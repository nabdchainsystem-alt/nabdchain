...
import { Funnel as FilterIcon, X, Plus, CaretDown as ChevronDown } from 'phosphor-react';
...
export const TableFilter: React.FC<TableFilterProps> = ({ columns, filters, onChange }) => {
  ...
  return (
    <>
      ...
      {isOpen && (
        <PortalPopup triggerRef={triggerRef} onClose={() => setIsOpen(false)} side="bottom" align="end">
              ...
              {filters.length === 0 ? (
                ...
                filters.map((filter, index) => {
                  ...
                  return (
                    <div
                      key={filter.id}
                      className="flex items-center gap-2 bg-stone-50 dark:bg-stone-800/50 p-2 rounded-md border border-stone-100 dark:border-stone-800"
                    >
