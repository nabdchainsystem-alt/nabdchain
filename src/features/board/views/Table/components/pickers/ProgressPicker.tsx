...
export const ProgressPicker: React.FC<ProgressPickerProps> = memo(
  ({ value, onSelect, onClose, triggerRect, autoMode = false }) => {
    ...
    const getStatusMessage = (percent: number) => {
      if (percent >= 100)
        return {
          icon: Trophy,
          text: t('progress_complete') || 'Complete!',
          color: 'text-emerald-600 dark:text-emerald-400',
        };
      if (percent >= 75)
        return {
          icon: Sparkle,
          text: t('progress_almost') || 'Almost there!',
          color: 'text-teal-600 dark:text-teal-400',
        };
      if (percent >= 50)
        return {
          icon: CheckCircle,
          text: t('progress_halfway') || 'Halfway done',
          color: 'text-blue-600 dark:text-blue-400',
        };
      if (percent >= 25)
        return {
          icon: CheckCircle,
          text: t('progress_good_start') || 'Good start',
          color: 'text-amber-600 dark:text-amber-400',
        };
      if (percent > 0)
        return {
          icon: CheckCircle,
          text: t('progress_just_started') || 'Just started',
          color: 'text-orange-600 dark:text-orange-400',
        };
      return {
        icon: CheckCircle,
        text: t('progress_not_started') || 'Not started',
          ...
          <div className="flex flex-col items-center py-5 px-4">
            <div className="relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className={`text-3xl font-bold ${colors.text} transition-colors`}>{clampedValue}</span>
...
export const ProgressBar: React.FC<{
  ...
  return (
    <div className="flex items-center gap-2 cursor-pointer group" onClick={onClick}>
      <div
        className={`flex-1 ${heights[size]} bg-stone-200 dark:bg-stone-700 rounded-full overflow-hidden min-w-[60px]`}
      >
        <div
