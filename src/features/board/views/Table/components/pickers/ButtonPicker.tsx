...
import {
    ...
    Plus, Gear, ArrowRight, Check, Globe, Code, Warning
} from 'phosphor-react';
import { useLanguage } from '../../../../../../contexts/LanguageContext';
import { ButtonAction, ButtonConfig } from '../../types';
            ...
            >
                ...
                {!isConfigMode && config && (
                    <div className="p-4 flex flex-col items-center gap-3">
                        ...
                        <div className={`text-xs text-stone-400 flex items-center gap-1 ${isRtl ? 'flex-row-reverse text-right' : ''}`}>
                ...
                {isConfigMode && (
                    <div className="p-3 space-y-3 max-h-[400px] overflow-y-auto">
                            ...
                            <div className={`flex flex-wrap gap-1 ${isRtl ? 'flex-row-reverse' : ''}`}>
                                {BUTTON_COLORS.map((color) => (
                                    <button
                                        ...
                                        className={`w-7 h-7 rounded-lg transition-all ${localConfig.color === color.value
                            ...
                            <div className="space-y-1">
                                {ACTION_TYPES.map((action) => (
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-2 px-2 py-2 rounded-lg transition-colors text-left ${isRtl ? 'flex-row-reverse' : ''} ${localConfig.action.type === action.type
                        ...
                        <div className="p-3 bg-stone-50 dark:bg-stone-800/50 rounded-lg space-y-3">
                            <div className={`flex items-center gap-1 text-xs font-medium text-stone-500 uppercase ${isRtl ? 'flex-row-reverse' : ''}`}>
                                <Gear size={12} />
                            ...
                            {localConfig.action.type === 'run_automation' && (
                                <div>
                                    <label className={`block text-[10px] text-stone-400 mb-1 ${isRtl ? 'text-right' : ''}`}>{t('automation_id')}</label>
                                    <input
                                        type="text"
                            ...
                            {localConfig.action.type === 'update_status' && (
                                <div>
                                    <label className={`block text-[10px] text-stone-400 mb-1 ${isRtl ? 'text-right' : ''}`}>{t('new_status')}</label>
                                    <select
                            ...
                            {localConfig.action.type === 'send_notification' && (
                                <>
                                    <div>
                                        <label className={`block text-[10px] text-stone-400 mb-1 ${isRtl ? 'text-right' : ''}`}>{t('message')}</label>
                                        <input
                                            type="text"
                                    ...
                                    <div>
                                        <label className={`block text-[10px] text-stone-400 mb-1 ${isRtl ? 'text-right' : ''}`}>{t('recipient_id')}</label>
                                        <input
                                            type="text"
                            ...
                            {localConfig.action.type === 'create_item' && (
                                <div>
                                    <label className={`block text-[10px] text-stone-400 mb-1 ${isRtl ? 'text-right' : ''}`}>{t('item_name')}</label>
                                    <input
                                        type="text"
                            ...
                            {localConfig.action.type === 'custom_webhook' && (
                                <div>
                                    <label className={`block text-[10px] text-stone-400 mb-1 ${isRtl ? 'text-right' : ''}`}>{t('webhook_url')}</label>
                                    <input
                                        type="url"
                            ...
                            {localConfig.action.type === 'custom_script' && (
                                <div className="flex flex-col gap-2">
                                    <div className={`flex items-start gap-2 p-2 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 text-[10px] rounded border border-amber-100 dark:border-amber-800 ${isRtl ? 'flex-row-reverse text-right' : ''}`}>
                                        <Warning size={14} className="shrink-0 mt-0.5" />
                                        <p>{t('script_warning') || 'Write JavaScript code. Available objects: utils.alert(), utils.openUrl(), context.'}</p>
                                    </div>
                                    <div>
                                        <label className={`block text-[10px] text-stone-400 mb-1 ${isRtl ? 'text-right' : ''}`}>{t('script')}</label>
                                        <textarea
