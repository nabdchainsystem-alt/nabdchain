...
export const FilesPicker: React.FC<FilesPickerProps> = memo(
  ({
    ...
    const handleFiles = async (newFiles: File[]) => {
      // Uses local object URLs; replace with server upload when file storage is available
      boardLogger.debug('[Files] Upload files (local)', newFiles);
    ...
    const handlePreview = (file: FileAttachment) => {
      // Opens file in a new tab; future: in-app preview modal
      boardLogger.debug('[Files] Preview file', file);
      window.open(file.url, '_blank');
    };

    const handleDownload = (file: FileAttachment) => {
      // Uses anchor-click download; works for same-origin and blob URLs
      ...
      <>
        ...
        >
            ...
            <div className="text-center">
              {uploading ? (
                ...
                <>
                  <CloudArrowUp
                    size={32}
                    className={`mx-auto mb-2 ${isDragging ? 'text-blue-500' : 'text-stone-400'}`}
                  />
          ...
          <div className="flex-1 overflow-y-auto max-h-[250px]">
            {files.length === 0 ? (
              <div className="p-6 text-center text-sm text-stone-400">{t('no_files_attached')}</div>
            ) : (
                ...
                {files.map((file) => {
                  ...
                  return (
                    ...
                    >
                      {/* Thumbnail or Icon */}
                      <div
                        className={`w-10 h-10 rounded-lg flex items-center justify-center bg-white dark:bg-stone-700 ${getFileColor(file.type)}`}
                      >
                      ...
                      <div className={`flex-1 min-w-0 ${isRtl ? 'text-right' : ''}`}>
                        <div className="text-sm text-stone-700 dark:text-stone-300 truncate">{file.name}</div>
                        <div className="text-[10px] text-stone-400">{formatFileSize(file.size)}</div>
                      </div>

                      {/* Actions */}
                      <div
                        className={`flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity ${isRtl ? 'flex-row-reverse' : ''}`}
                      >
                        <button
...
export const FilesDisplay: React.FC<{
  ...
  return (
    <button onClick={onClick} className={`flex items-center gap-1 ${isRtl ? 'flex-row-reverse' : ''}`}>
      ...
      {files.length > maxDisplay && <span className="text-xs text-stone-500">+{files.length - maxDisplay}</span>}
      <span className="text-xs text-stone-400">
        {files.length} {t('uploaded_files')}
      </span>
    </button>
  );
});
