...
export const WorldClockPicker: React.FC<WorldClockPickerProps> = memo(({ value, onSelect, onClose, triggerRect }) => {
    ...
    <>
      ...
      >
        ...
        <div className="px-3 py-2 border-b border-stone-100 dark:border-stone-800 bg-stone-50 dark:bg-stone-800/50">
          <div
            className={`flex items-center gap-2 text-xs font-medium text-stone-600 dark:text-stone-400 ${isRtl ? 'flex-row-reverse' : ''}`}
          >
            <Globe size={14} />
          ...
          <div className="px-3 py-2 bg-blue-50 dark:bg-blue-900/20 border-b border-blue-100 dark:border-blue-800">
            <div className={`flex items-center justify-between ${isRtl ? 'flex-row-reverse' : ''}`}>
              <div className={isRtl ? 'text-right' : ''}>
                <div className="text-sm font-medium text-blue-700 dark:text-blue-300">{selectedTimezone.city}</div>
            ...
            <div className="flex flex-col gap-0.5">
              {filteredTimezones.map((tz) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-2 py-2 rounded-lg hover:bg-stone-100 dark:hover:bg-stone-800 group transition-colors ${
                ...
                >
                  <div className={`flex items-center gap-2 min-w-0 ${isRtl ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`min-w-0 ${isRtl ? 'text-right' : ''}`}>
                      <div className="text-xs font-medium text-stone-700 dark:text-stone-300 truncate">{tz.city}</div>
                      <div className="text-[10px] text-stone-400 truncate">{tz.label}</div>
                    </div>
                  </div>
                  <div className={`flex items-center gap-2 shrink-0 ${isRtl ? 'flex-row-reverse' : ''}`}>
                    <span className="text-[10px] text-stone-400">{tz.offset}</span>
