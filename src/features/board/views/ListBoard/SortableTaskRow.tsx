...
import { Status, Priority } from '../../types/boardTypes';
...
interface SortableTaskRowProps {
  ...
  setActiveDatePicker: (data: { taskId: string; groupId: string; rect: DOMRect } | null) => void;
...
export const SortableTaskRow: React.FC<SortableTaskRowProps> = ({
  ...
  const style: React.CSSProperties = {
    ...
    transition: isDragging ? 'none' : 'transform 200ms cubic-bezier(0.25, 0.1, 0.25, 1), opacity 150ms ease',
  ...
  return (
    <React.Fragment>
      ...
      >
        <div
          className={`flex items-center justify-center py-1.5 border-r relative transition-colors sticky left-0 z-10 border-r-2 ${darkMode ? 'bg-[#1a1d24] border-gray-800 border-r-gray-800 group-hover/row:bg-white/5' : 'bg-white border-gray-100 border-r-gray-200/50 hover:bg-gray-50'}`}
        >
            ...
            >
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <PriorityCell
                    priority={
                      col.id === 'col_priority'
                        ? task.priority
                        : (task.textValues[col.id] as Priority) || Priority.Normal
                    }
                    onChange={(p) => {
              ...
              {col.type === 'money' && (
                <div className="w-full h-full px-2 py-1.5 text-sm text-gray-700 flex items-center justify-end font-mono">
                  {task.textValues[col.id] ? (
                    <>
                      <span className={`mr-1 ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
                        {col.currency || '$'}
                      </span>
                      <input
                        type="text"
              ...
              {col.type === 'website' && (
                <div className="w-full h-full px-2 py-1.5 flex items-center gap-2">
                  <div
                    className={`p-1 rounded ${darkMode ? 'bg-indigo-500/20 text-indigo-400' : 'bg-indigo-50 text-indigo-500'}`}
                  >
                    <Globe size={12} />
                  </div>
                  <input
              ...
              {col.type === 'phone' && (
                <div className="w-full h-full px-2 py-1.5 flex items-center gap-2">
                  <div
                    className={`p-1 rounded ${darkMode ? 'bg-orange-500/20 text-orange-400' : 'bg-orange-50 text-orange-500'}`}
                  >
                    <Phone size={12} />
                  </div>
                  <input
              ...
              {col.type === 'progress_manual' && (
                <div className="w-full h-full px-2 py-1.5 flex items-center gap-2">
                  <div
                    className={`flex-1 h-2 rounded-full overflow-hidden relative group/progress cursor-pointer ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}
                  >
                    <div
                  ...
                  <span className={`text-xs w-8 text-right ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    {Number(task.textValues[col.id]) || 0}%
                  </span>
                </div>
              )}
        ...
        {/* Delete Row Action */}
        <div
          className={`flex items-center justify-center border-l opacity-0 group-hover/row:opacity-100 transition-opacity ${darkMode ? 'border-gray-800 bg-[#0f1115]' : 'border-gray-100 bg-white'}`}
        >
          <button
        ...
        <div className="contents">
          ...
          {task.subtasks?.map((subtask) => (
            ...
            >
              ...
              <div
                className={`flex items-center justify-center py-1.5 border-r relative sticky left-0 z-10 border-r-2 ${darkMode ? 'bg-[#1a1d24]/50 border-gray-800 border-r-gray-800' : 'bg-gray-50/30 border-gray-100 border-r-gray-200/50'}`}
              >
              ...
              <div
                className={`border-l ${darkMode ? 'bg-[#1a1d24]/50 border-gray-800' : 'bg-white border-gray-100'}`}
              />
            </div>
          ))}
          ...
          {/* Add Subtask Row */}
          <div
            className={`grid gap-px relative z-20 shadow-lg my-2 mx-4 rounded-lg border overflow-hidden animate-in slide-in-from-top-2 duration-200 ${darkMode ? 'bg-[#1a1d24] border-gray-700' : 'bg-white border-gray-200'}`}
            style={{ gridColumn: `1 / -1` }}
          >
            <div className={`flex items-center p-2 gap-3 ${darkMode ? 'bg-[#1a1d24]' : 'bg-white'}`}>
              <div
                className={`w-5 h-5 rounded-full border-2 border-dashed animate-spin-slow ${darkMode ? 'border-gray-600' : 'border-gray-300'}`}
              />
              <input
                autoFocus
              ...
              />
              <div
                className={`flex items-center gap-1 border-l pl-3 ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}
              >
                <button
                  className={`p-1.5 rounded transition-colors ${darkMode ? 'text-gray-400 hover:text-gray-200 hover:bg-white/10' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-100'}`}
                >
                  <Box size={16} />
                </button>
                <button
