...
import { GroupData, StatusOption, TaskItem, ColumnWidths } from './types';
...
const ListBoard: React.FC<ListBoardProps> = ({ roomId, viewId }) => {
      ...
      try {
        ...
        if (savedStatuses) {
          ...
          if (Array.isArray(parsed) && typeof parsed[0] === 'string') {
            ...
            loadedOptions = parsed.map((s: { id?: string; label?: string; title?: string; color?: string }) => {
              ...
              if (!color.startsWith('#')) {
                ...
                else if (lower.includes('yellow') || lower.includes('orange') || lower.includes('amber'))
                  color = '#e59935';
  ...
  const persistData = (currentGroups: GroupData[]) => {
    try {
      ...
      const existingRows: Record<string, unknown>[] = existingRowsString ? JSON.parse(existingRowsString) : [];
