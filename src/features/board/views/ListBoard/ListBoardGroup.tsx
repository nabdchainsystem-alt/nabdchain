...
export const ListBoardGroup: React.FC<ListBoardGroupProps> = React.memo(
  ({
    ...
    return (
      <div
        id={`group-${group.id}`}
        className={`relative flex flex-col w-full mb-10 shadow-sm border rounded-xl overflow-hidden transition-colors ${darkMode ? 'border-gray-800 bg-[#1a1d24]' : 'border-gray-200/60'}`}
      >
        {/* Group Header */}
        <div
          className={`flex items-center px-4 py-3 relative w-full border-b transition-colors ${darkMode ? 'bg-[#1a1d24] border-gray-800' : 'bg-white border-gray-200'}`}
        >
          <div className="flex items-center gap-3 pr-4">
            ...
            >
              <svg
                className={`w-4 h-4 text-gray-400 group-hover/menu:text-blue-500 transition-transform ${collapsedGroups.has(group.id) ? '-rotate-90' : 'rotate-0'}`}
                viewBox="0 0 20 20"
                fill="currentColor"
              >
                <path
            ...
            <div className="ml-4 flex items-center gap-2">
              <div className="flex h-1.5 w-28 overflow-hidden rounded-full bg-gray-100">
                {(['todo', 'pending', 'working', 'almostFinish', 'done', 'stuck'] as const).map((key) => {
                  ...
                  const color =
                    key === 'todo'
                      ? statusColorMap[Status.ToDo]
                      : key === 'pending'
                        ? statusColorMap[Status.Pending]
                        : key === 'working'
                          ? statusColorMap[Status.WorkingOnIt]
                          : key === 'almostFinish'
                            ? statusColorMap[Status.AlmostFinish]
                            : key === 'done'
                              ? statusColorMap[Status.Done]
          ...
          <div className="flex items-center gap-1 px-2">
            <button
              ...
              className={
                ...
                (group.isPinned ? 'text-blue-500 bg-blue-50' : 'text-gray-300 hover:text-gray-500 hover:bg-gray-100')
              }
        ...
        {!collapsedGroups.has(group.id) && (
          <div className="overflow-x-auto w-full [&::-webkit-scrollbar]:hidden">
            {/* Columns Header */}
            <div
              className={`sticky top-0 z-[1] min-w-full w-fit transition-colors ${darkMode ? 'bg-[#1a1d24]' : 'bg-white'}`}
              style={{ boxShadow: '0 2px 5px -2px rgba(0,0,0,0.05)' }}
            >
              <div
                className={`grid gap-0 border-y text-xs font-normal transition-colors ${darkMode ? 'bg-gray-800 border-gray-800 text-gray-400' : 'bg-white border-gray-200 text-gray-500'}`}
                style={{
                  gridTemplateColumns:
                    selectionColumnWidth + ' ' + group.columns.map((c) => c.width).join(' ') + ' ' + actionColumnWidth,
                }}
              >
                <div
                  className={`flex items-center justify-center sticky left-0 z-20 border-r-2 transition-colors ${darkMode ? 'bg-[#1a1d24]/95 border-r-gray-800' : 'bg-gray-50/80 border-r-gray-200/50'}`}
                >
                  <input
                    type="checkbox"
                    ...
                    <div className="flex items-center justify-between h-full">
                      ...
                      <div
                        ...
                        onClick={(e) => {
                          ...
                          setActiveColumnMenu(
                            activeColumnMenu?.groupId === group.id ? null : { groupId: group.id, rect },
                          );
                        }}
                      >
                ...
                <div
                  className={`flex items-center justify-center relative group ${darkMode ? 'bg-[#1a1d24]/95' : 'bg-gray-50/80'}`}
                >
                  <div
                    className={
                      'cursor-pointer w-6 h-6 rounded flex items-center justify-center transition-all duration-200 ' +
                      (activeColumnMenu?.groupId === group.id
            ...
            <div
              className={`divide-y relative z-0 min-w-full w-fit ${darkMode ? 'divide-gray-800' : 'divide-gray-100'}`}
            >
            ...
            {/* Add Task Bar */}
            <div
              className={`grid gap-0 border-t group/add-row transition-colors min-w-full w-fit ${darkMode ? 'bg-[#1a1d24] border-gray-800 hover:bg-white/5' : 'bg-white border-gray-200 hover:bg-gray-50'}`}
              style={{
                gridTemplateColumns:
                  selectionColumnWidth + ' ' + group.columns.map((c) => c.width).join(' ') + ' ' + actionColumnWidth,
              }}
            >
              <div
                className={`flex items-center justify-center border-r sticky left-0 z-10 border-r-2 transition-colors relative ${darkMode ? 'bg-[#1a1d24] border-gray-800 border-r-gray-800 group-hover/add-row:bg-white/5' : 'bg-white border-gray-100 border-r-gray-200/50 group-hover/add-row:bg-gray-50'}`}
              >
              ...
              <div
                className={`flex items-center pl-2 py-2 border-r sticky left-[50px] z-10 border-r-2 transition-colors ${darkMode ? 'bg-[#1a1d24] border-gray-800 border-r-gray-800 group-hover/add-row:bg-white/5' : 'bg-white border-gray-100 border-r-gray-200/50 group-hover/add-row:bg-gray-50'}`}
              >
                <input
                  type="text"
              ...
              {group.columns.slice(1).map((col) => (
                <div
                  key={col.id}
                  className={`border-r min-h-[32px] flex items-center justify-center ${darkMode ? 'bg-[#1a1d24] border-gray-800' : 'bg-white border-gray-100'}`}
                >
                  {col.type === 'status' ? (
                    <div className="w-full h-full flex items-center justify-center">
                      <StatusCell
                        status={
                          col.id === 'col_status'
                            ? draftTasks[group.id]?.status || Status.ToDo
                            : (draftTasks[group.id]?.textValues?.[col.id] as Status) || Status.ToDo
                        }
                        onChange={(s) => {
                          if (col.id === 'col_status') {
                            updateDraftTask(group.id, { status: s });
                          } else {
                            updateDraftTask(group.id, {
                              textValues: { ...draftTasks[group.id]?.textValues, [col.id]: s },
                            });
                          }
                        }}
                        tabIndex={0}
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <PriorityCell
                        priority={
                          col.id === 'col_priority'
                            ? draftTasks[group.id]?.priority || Priority.Normal
                            : (draftTasks[group.id]?.textValues?.[col.id] as Priority) || Priority.Normal
                        }
                        onChange={(p) => {
                          if (col.id === 'col_priority') {
                            ...
                            updateDraftTask(group.id, {
                              textValues: { ...draftTasks[group.id]?.textValues, [col.id]: p },
                            });
                          }
                        }}
                        tabIndex={0}
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <PersonCell
                        personId={
                          col.id === 'col_person'
                            ? draftTasks[group.id]?.personId || null
                            : draftTasks[group.id]?.textValues?.[col.id] || null
                        }
                        onChange={(pid) => {
                          if (col.id === 'col_person') {
                            ...
                            updateDraftTask(group.id, {
                              textValues: { ...draftTasks[group.id]?.textValues, [col.id]: pid || '' },
                            });
                          }
                        }}
                        tabIndex={0}
                    ...
                    <div className="w-full h-full p-1 flex items-center justify-center">
                      ...
                      >
                        ...
                        <span className="truncate">
                          {col.config?.targetName ? `Go to ${col.config.targetName}` : 'Connect'}
                        </span>
                      </button>
                    </div>
                  ) : col.type === 'date' ? (
                      ...
                      <div
                        ...
                        onClick={(e) => {
                          ...
                          const currentValue = isPrimary
                            ? draftTasks[group.id]?.dueDate || ''
                            : draftTasks[group.id]?.textValues?.[col.id] || '';
                          setActiveDatePicker({
                            ...
                            onSelect: (dateStr: string) => {
                              if (isPrimary) {
                                ...
                                updateDraftTask(group.id, {
                                  textValues: { ...draftTasks[group.id]?.textValues, [col.id]: dateStr },
                                });
                              }
                            },
                          });
                      ...
                      >
                        {(() => {
                          ...
                          return val ? (
                            ...
                            <span className="text-gray-300">Set Date</span>
                          );
                        })()}
                      </div>
                    </div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateDraftTask(group.id, {
                          textValues: { ...draftTasks[group.id]?.textValues, [col.id]: e.target.value },
                        })
                      }
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (/^[0-9.,]*$/.test(val)) {
                          updateDraftTask(group.id, {
                            textValues: { ...draftTasks[group.id]?.textValues, [col.id]: val },
                          });
                        }
                      }}
              ...
              <div
                className={`transition-colors ${darkMode ? 'bg-[#1a1d24] group-hover/add-row:bg-white/5' : 'bg-white group-hover/add-row:bg-gray-50'}`}
              ></div>
            </div>

            {/* Group Summary Footer */}
            <div
              className={`grid gap-0 border-t rounded-b-xl min-w-full w-fit ${darkMode ? 'bg-[#1a1d24] border-gray-800' : 'bg-white border-gray-200'}`}
              style={{
                gridTemplateColumns:
                  selectionColumnWidth + ' ' + group.columns.map((c) => c.width).join(' ') + ' ' + actionColumnWidth,
              }}
            >
              <div
                className={`flex items-center justify-center border-r sticky left-0 z-10 border-r-2 ${darkMode ? 'bg-[#1a1d24] border-gray-800 border-r-gray-800' : 'bg-white border-gray-100 border-r-gray-200/50'}`}
              ></div>
              <div
                className={`flex items-center justify-center sticky left-[50px] z-10 border-t ${darkMode ? 'bg-[#1a1d24] border-gray-800' : 'bg-white border-gray-200'}`}
              >
              ...
              {group.columns.slice(1).map((col) => (
                <div
                  key={col.id}
                  className={`border-r min-h-[32px] flex items-center justify-center px-2 ${darkMode ? 'bg-[#1a1d24] border-gray-800' : 'bg-white border-gray-100'}`}
                >
                  {col.type === 'status' && (
                      ...
                      {Object.values(Status).map((s) => {
                        ...
                        return (
                          <div
                            ...
                            style={{ width: width + '%' }}
                            title={s + ': ' + count}
                            className={STATUS_COLORS[s] + ' h-full hover:opacity-80 transition-opacity'}
                          />
                        );
                      })}
                    </div>
                      ...
                      {Object.values(Priority).map((p) => {
                        ...
                        return (
                          <div
                            ...
                            style={{ width: width + '%' }}
                            title={p + ': ' + count}
                            className={PRIORITY_COLORS[p] + ' h-full hover:opacity-80 transition-opacity'}
                          />
                        );
                      })}
                    </div>
