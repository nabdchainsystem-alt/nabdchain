...
const getToolsData = (isRTL: boolean) => [
  {
    ...
    icon: Chalkboard,
    description: isRTL ? 'لوحة بصرية' : 'Visual canvas',
    preview: WhiteboardPreview,
  },
  {
    name: isRTL ? 'ملاحظات كورنيل' : 'Cornell Notes',
    icon: Note,
    description: isRTL ? 'ملاحظات ذكية' : 'Smart notes',
    preview: NotesPreview,
  },
  {
    name: isRTL ? 'لوحات المعلومات' : 'Dashboards',
    icon: ChartBar,
    description: isRTL ? 'تحليلات مباشرة' : 'Live analytics',
    preview: DashboardPreviewMini,
  },
  {
    name: isRTL ? 'عبء العمل' : 'Workload',
    icon: UsersFour,
    description: isRTL ? 'السعة' : 'Capacity',
    preview: WorkloadPreview,
  },
  {
    name: isRTL ? 'الأتمتة' : 'Automation',
    icon: Lightning,
    description: isRTL ? 'بدون برمجة' : 'No-code flows',
...
const ToolCard: React.FC<{
  tool: ReturnType<typeof getToolsData>[0];
  index: number;
  isActive: boolean;
  onClick: () => void;
}> = memo(({ tool, index, isActive, onClick }) => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Preview Area */}
        <div
          className={`h-24 mb-4 rounded-xl overflow-hidden transition-colors ${
    ...
    {[3, 2, 4].map((count, col) => (
      <div key={col} className="flex-1 space-y-1">
        {Array.from({ length: count }).map((_, i) => (
          <div
            key={i}
            className={`h-4 rounded ${col === 2 ? 'bg-zinc-900 dark:bg-white' : 'bg-zinc-200 dark:bg-zinc-700'}`}
          />
        ))}
      </div>
    ))}
  ...
  <div className="h-full p-2">
    <div className="grid grid-cols-7 gap-0.5 h-full">
      {Array.from({ length: 14 }).map((_, i) => (
        <div
          key={i}
          className={`rounded-sm ${[3, 7, 10].includes(i) ? 'bg-zinc-900 dark:bg-white' : 'bg-zinc-100 dark:bg-zinc-800'}`}
        />
      ))}
    </div>
  </div>
...
const getBoardViewsData = (isRTL: boolean) => [
  {
    ...
    icon: Kanban,
    preview: KanbanViewPreview,
    description: isRTL ? 'لوحات سير عمل بصرية' : 'Visual workflow boards',
  },
  {
    name: isRTL ? 'جدول' : 'Table',
    icon: Rows,
    preview: TableViewPreview,
    description: isRTL ? 'بيانات كجداول' : 'Spreadsheet-like data',
  },
  {
    name: isRTL ? 'التقويم' : 'Calendar',
    icon: Calendar,
    preview: CalendarViewPreview,
    description: isRTL ? 'تخطيط بالتواريخ' : 'Date-based planning',
  },
  {
    name: isRTL ? 'الجدول الزمني' : 'Timeline',
    icon: Timer,
    preview: TimelineViewPreview,
    description: isRTL ? 'خرائط المشاريع' : 'Project roadmaps',
  },
  {
    name: isRTL ? 'جانت' : 'Gantt',
    icon: ChartLine,
    preview: GanttViewPreview,
    description: isRTL ? 'جدولة المشاريع' : 'Project scheduling',
  },
];

// View Card
const ViewCard: React.FC<{
  view: ReturnType<typeof getBoardViewsData>[0];
  index: number;
  isActive: boolean;
  onClick: () => void;
}> = memo(({ view, index, isActive, onClick }) => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Preview */}
        <div
          className={`h-16 mb-3 rounded-lg overflow-hidden ${
        ...
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-xs font-medium ${isActive ? 'text-white dark:text-zinc-900' : 'text-zinc-700 dark:text-zinc-300'}`}
          >
            {view.name}
          </span>
        </div>
...
export const ToolsShowcase: React.FC<ToolsShowcaseProps> = ({ onBoardViewsMount }) => {
  ...
  const boardViewsCallbackRef = useCallback(
  ...
  return (
    <section ref={sectionRef} className="bg-black">
      ...
      <div className="py-20 sm:py-28 md:py-32 relative overflow-hidden">
        ...
        <div
          className="absolute top-20 left-1/3 w-[500px] h-[350px] bg-purple-500/15 rounded-full blur-3xl pointer-events-none"
          style={{ animation: 'spotlightPulse 8s ease-in-out infinite' }}
        />
        <div
          className="absolute bottom-20 right-1/4 w-[400px] h-[300px] bg-blue-500/10 rounded-full blur-3xl pointer-events-none"
      ...
      {/* Board Views Section */}
      <div
        ref={boardViewsCallbackRef}
        className="py-20 sm:py-28 md:py-32 bg-white dark:bg-zinc-900 relative overflow-hidden"
      >
        <div className="max-w-[1200px] mx-auto px-6 sm:px-8 lg:px-12">
          ...
          {isInView && (
            <div
              className="mt-12 flex justify-center items-center gap-8 sm:gap-12 pt-8 border-t border-zinc-200 dark:border-zinc-800"
