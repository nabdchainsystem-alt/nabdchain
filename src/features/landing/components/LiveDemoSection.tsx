      ...
      <div className="mt-4 pt-3 border-t border-zinc-200 dark:border-zinc-800 flex items-center gap-2">
        <div className="flex -space-x-1">
          {['K', 'T', 'G'].map((l, i) => (
            <div
              key={i}
              className="flex items-center gap-2 px-2 py-1 rounded-md bg-zinc-100 dark:bg-zinc-800 text-[10px] text-zinc-500"
            >
              {l === 'K' && <Kanban size={10} />}
              {l === 'T' && <Table size={10} />}
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="text-base font-semibold text-zinc-900 dark:text-white">{isRTL ? 'الأتمتة' : 'Automations'}</h3>
          ...
          return (
            <div
              ...
              className={`relative flex items-center justify-between p-3 rounded-xl transition-all duration-300 ${
                isActive ? 'bg-zinc-900 dark:bg-white' : 'bg-zinc-100 dark:bg-zinc-800/50'
              }`}
            >
              <div className="flex items-center gap-2">
                <div
                  className={`w-6 h-6 rounded-lg flex items-center justify-center ${
                    isActive ? 'bg-white dark:bg-zinc-900' : 'bg-zinc-200 dark:bg-zinc-700'
                  }`}
                >
              ...
              <div className="flex items-center gap-1">
                {isRTL ? (
                  <ArrowLeft
                    size={10}
                    className={isActive ? 'text-zinc-400 dark:text-zinc-600' : 'text-zinc-300 dark:text-zinc-700'}
                  />
                ) : (
                  <ArrowRight
                    size={10}
              ...
              {isActive && (
                <div
                  className={`absolute ${isRTL ? 'left-2' : 'right-2'} top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-white dark:bg-zinc-900 animate-pulse`}
                />
              )}
            </div>
          );
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 relative bg-zinc-100 dark:bg-zinc-800/50 rounded-xl p-3 overflow-hidden">
        ...
        >
          ...
          <div
            className={`${isRTL ? 'mr-3' : 'ml-3'} -mt-1 px-1.5 py-0.5 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 text-[8px] rounded whitespace-nowrap font-medium`}
          >
            {isRTL ? 'آية ك.' : 'Aya K.'}
          </div>
        </div>
        ...
        <div className="absolute" style={{ left: '60%', top: '60%' }}>
          ...
          <div
            className={`${isRTL ? 'mr-2' : 'ml-2'} px-1.5 py-0.5 bg-zinc-500 text-white text-[8px] rounded whitespace-nowrap`}
          >
            {isRTL ? 'أحمد م.' : 'John D.'}
          </div>
        </div>
      ...
      <div className="mt-3 flex items-center justify-between">
        <div className={`flex ${isRTL ? '-space-x-reverse' : ''} -space-x-2`}>
          {['SK', 'JD', 'MR'].map((initials, i) => (
            <div
              key={i}
              className="w-6 h-6 rounded-full bg-zinc-200 dark:bg-zinc-700 border-2 border-white dark:border-zinc-900 flex items-center justify-center text-[8px] text-zinc-600 dark:text-zinc-400 font-medium"
            >
              {initials}
            </div>
          ))}
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="mt-3 p-2 rounded-lg bg-zinc-50 dark:bg-zinc-800/30 text-center">
        <span className="text-[10px] text-zinc-500">{isRTL ? '99.99% ضمان وقت التشغيل' : '99.99% uptime SLA'}</span>
      </div>
    </div>
  );
});
