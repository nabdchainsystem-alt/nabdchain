...
const FeatureItem: React.FC<{ feature: string; highlighted?: boolean; index: number }> = memo(
  ({ feature, highlighted, index }) => (
    <li
    ...
    >
      <div
        className={`mt-0.5 w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${
          highlighted ? 'bg-white text-zinc-900' : 'bg-zinc-700/50 border border-zinc-600/50 text-zinc-300'
        }`}
      >
...
const AnimatedPrice: React.FC<{ price: number; highlighted?: boolean }> = memo(({ price, _highlighted }) => {
  ...
  return <span className="text-5xl sm:text-6xl font-bold tracking-tight text-white">{displayPrice}</span>;
});
...
const PricingCard: React.FC<PricingCardProps> = memo(({ tier, isYearly, onGetStarted, index, isRTL }) => {
  ...
  return (
    ...
    >
      <div
        className={`h-full rounded-3xl p-6 sm:p-8 transition-all duration-300 relative ${
          tier.highlighted
      ...
      >
        ...
        <div className="mb-6">
          <div
            className={`inline-flex items-center justify-center w-12 h-12 rounded-2xl mb-4 ${
              tier.highlighted ? 'bg-white text-zinc-900' : 'bg-zinc-700/50 border border-zinc-600/50 text-zinc-300'
            }`}
          >
            <Icon size={24} weight="fill" />
          </div>
          <h3 className={`text-xl font-bold mb-2 ${tier.highlighted ? 'text-white' : 'text-white'}`}>{tier.name}</h3>
          <p className={`text-sm ${tier.highlighted ? 'text-zinc-400' : 'text-zinc-500'}`}>{tier.description}</p>
        </div>
            ...
            <>
              ...
              <div className="flex items-baseline gap-2">
                ...
                <div className="flex flex-col">
                  <span className="text-sm font-medium text-zinc-400">{isRTL ? 'ر.س' : 'SAR'}</span>
                  <span className="text-xs text-zinc-500">{isRTL ? '/سنة' : '/year'}</span>
                </div>
              </div>
              {/* Per month breakdown */}
              <div className="mt-2 flex items-center gap-2">
                ...
                <span
                  className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                    tier.highlighted ? 'bg-white/10 text-white' : 'bg-zinc-800 text-zinc-300'
                  }`}
                >
        ...
        >
          ...
          <ArrowIcon
            size={14}
            weight="bold"
            className={`group-hover/btn:${isRTL ? '-translate-x-0.5' : 'translate-x-0.5'} transition-transform`}
          />
        </button>
...
export const PricingSection: React.FC<PricingSectionProps> = ({ onGetStarted }) => {
  ...
  return (
    <section ref={sectionRef} id="pricing" className="py-20 sm:py-28 md:py-36 bg-black relative overflow-hidden">
      ...
      />
      <div
        className="absolute top-1/3 left-1/4 w-[400px] h-[300px] bg-blue-500/10 rounded-full blur-3xl pointer-events-none"
        style={{ animation: 'spotlightPulse 10s ease-in-out infinite', animationDelay: '-4s' }}
      />
      <div
        className="absolute bottom-1/4 right-1/4 w-[350px] h-[250px] bg-purple-500/10 rounded-full blur-3xl pointer-events-none"
      ...
      <div className="max-w-[1200px] mx-auto px-6 sm:px-8 lg:px-12 relative z-10">
          ...
          >
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-sm font-medium transition-all ${
                !isYearly ? 'bg-white text-zinc-900' : 'text-zinc-500 hover:text-white'
              }`}
            >
              {isRTL ? 'شهري' : 'Monthly'}
            </button>
            ...
            >
              ...
              <span
                className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                  isYearly ? 'bg-zinc-900/20 text-zinc-900' : 'bg-zinc-800 text-zinc-400'
                }`}
              >
                -55%
              </span>
        ...
        {isInView && (
          <div
            className="mt-12 sm:mt-16 p-6 rounded-2xl bg-zinc-800/50 border border-zinc-700/50"
