...
export const VaultSidebar: React.FC<VaultSidebarProps> = ({
  ...
  return (
    <div
      className={`w-64 bg-gray-50 dark:bg-monday-dark-surface border-r border-gray-200 dark:border-monday-dark-border flex flex-col p-4 shadow-sm ${className}`}
        ...
        <div className="pt-4 mt-4 border-t border-gray-200 dark:border-monday-dark-border">
          ...
          <div className="space-y-1">
            {folders.length === 0 ? (
              ...
              folders.map((folder) => {
                const metadata = folder.metadata as Record<string, unknown> | null;
                const iconKey = metadata?.icon as string | undefined;
                const Icon = iconKey && ICON_MAP[iconKey] ? ICON_MAP[iconKey] : Folder;
