  ...
  return (
    <div className="h-full w-full flex flex-col gap-4 py-4">
            ...
            <button
              ...
              className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors ${
                range === option
                  ...
                  : 'text-gray-600 dark:text-monday-dark-text-secondary'
              }`}
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <WidgetCard
          title={t('open_items')}
          value={String(metrics.total - metrics.completed)}
          subtitle={t('in_progress_planned')}
          icon={Activity}
        ...
        <WidgetCard
          ...
          subtitle={`${t('due_within')} ${range === 'week' ? t('this_week') : range === 'month' ? t('this_month') : t('this_quarter')}`}
          icon={BarChart3}
        ...
        <WidgetCard
          ...
          subtitle={`${metrics.completed} ${t('of_done')} ${metrics.total} ${t('done_label')}`}
          icon={TrendingUp}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="col-span-2 bg-white dark:bg-monday-dark-surface border border-gray-200 dark:border-monday-dark-border rounded-xl p-4 shadow-sm">
          ...
          <div className="space-y-2">
            ...
            {(topStatuses as [string, number][]).map(([status, count]) => {
              const ratio = metrics.total ? (count / metrics.total) * 100 : 0;
              return (
                <div key={status} className="flex items-center gap-3">
                  <div className="w-24 text-xs font-semibold text-gray-600 dark:text-monday-dark-text-secondary truncate">
                    {status}
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-monday-dark-surface border border-gray-200 dark:border-monday-dark-border rounded-xl p-4 shadow-sm col-span-2">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-monday-dark-text-secondary">
                {t('upcoming_overdue_range')}
              </p>
            </div>
              ...
              .map((task) => {
                ...
                return (
                  <div key={task.id} className="flex items-center gap-3 text-sm">
                    <div
                      className={`w-2 h-2 rounded-full ${isOverdue ? 'bg-amber-500' : withinRange ? 'bg-emerald-500' : 'bg-gray-300'}`}
                    />
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <span className="font-semibold text-gray-800 dark:text-monday-dark-text truncate">
                          {task.name}
                        </span>
                        <span className="text-[11px] text-gray-500 font-datetime">
        ...
        <div className="bg-white dark:bg-monday-dark-surface border border-gray-200 dark:border-monday-dark-border rounded-xl p-4 shadow-sm">
          ...
          <div className="space-y-2">
            {Object.entries(metrics.priorities).length === 0 && (
              <p className="text-sm text-gray-500">{t('no_priority_data')}</p>
            )}
            {(Object.entries(metrics.priorities) as [string, number][]).map(([priority, count]) => {
              const ratio = metrics.total ? (count / metrics.total) * 100 : 0;
              return (
