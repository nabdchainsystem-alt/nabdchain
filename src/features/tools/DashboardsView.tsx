...
const WidgetCard = ({ title, value, subtitle, icon: Icon, color }: { title: string; value: string; subtitle: string; icon: React.ComponentType<IconProps>; color: string }) => (
    <div className="bg-white dark:bg-monday-dark-surface border border-gray-200 dark:border-monday-dark-border rounded-xl p-4 shadow-sm flex items-center gap-3">
        ...
        <div className="flex flex-col">
            ...
            <span className="text-2xl font-bold text-gray-900 dark:text-monday-dark-text leading-none">{value}</span>
            <span className="text-[11px] text-gray-500 dark:text-monday-dark-text-muted mt-1">{subtitle}</span>
        </div>
    </div>
);
    ...
    return (
        <div className="h-full w-full flex flex-col gap-4 py-4">
            <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-monday-dark-surface border border-gray-200 dark:border-monday-dark-border rounded-xl shadow-sm">
                    ...
                    <div className="flex flex-col leading-tight">
                        ...
                        <span className="text-sm font-bold text-gray-800 dark:text-monday-dark-text">{boardName}</span>
                    </div>
                </div>
                    ...
                    {(['week', 'month', 'quarter'] as const).map((option) => (
                        <button
                            ...
                            className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors ${
                                range === option ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-200' : 'text-gray-600 dark:text-monday-dark-text-secondary'
                            }`}
                        >
                            {option === 'week' ? t('this_week') : option === 'month' ? t('this_month') : t('this_quarter')}
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <WidgetCard title={t('open_items')} value={String(metrics.total - metrics.completed)} subtitle={t('in_progress_planned')} icon={Activity} color="bg-blue-500 text-blue-600" />
                <WidgetCard title={t('upcoming')} value={String(metrics.upcoming)} subtitle={`${t('due_within')} ${range === 'week' ? t('this_week') : range === 'month' ? t('this_month') : t('this_quarter')}`} icon={BarChart3} color="bg-emerald-500 text-emerald-600" />
                <WidgetCard title={t('overdue')} value={String(metrics.overdue)} subtitle={t('needs_attention')} icon={AlertTriangle} color="bg-amber-500 text-amber-600" />
                <WidgetCard title={t('completion_rate')} value={`${metrics.completion}%`} subtitle={`${metrics.completed} ${t('of_done')} ${metrics.total} ${t('done_label')}`} icon={TrendingUp} color="bg-purple-500 text-purple-600" />
            </div>

            {/* Status + Priority */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="col-span-2 bg-white dark:bg-monday-dark-surface border border-gray-200 dark:border-monday-dark-border rounded-xl p-4 shadow-sm">
                    <div className="flex items-center justify-between mb-3">
                        <div>
                            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400">{t('status_distribution')}</p>
                            <p className="text-sm text-gray-600 dark:text-monday-dark-text-secondary">{t('fast_overview')}</p>
                        </div>
                        ...
                        {topStatuses.map(([status, count]: [string, number]) => {
                            ...
                            return (
                                <div key={status} className="flex items-center gap-3">
                                    <div className="w-24 text-xs font-semibold text-gray-600 dark:text-monday-dark-text-secondary truncate">{status}</div>
                                    <div className="flex-1 h-2.5 rounded-full bg-gray-100 dark:bg-monday-dark-hover overflow-hidden">
                                        <div
                ...
                <div className="bg-white dark:bg-monday-dark-surface border border-gray-200 dark:border-monday-dark-border rounded-xl p-4 shadow-sm">
                    <div className="flex items-center justify-between mb-3">
                        <div>
                            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400">{t('readiness')}</p>
                            <p className="text-sm text-gray-600 dark:text-monday-dark-text-secondary">{t('health_overdue_risk')}</p>
                        </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                            <div className="h-2.5 rounded-full bg-gray-100 dark:bg-monday-dark-hover overflow-hidden mb-2">
                                <div className="h-full bg-blue-500 rounded-full" style={{ width: `${metrics.health}%` }} />
                            </div>
                            <p className="text-xs text-gray-500">{t('higher_better')}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="bg-white dark:bg-monday-dark-surface border border-gray-200 dark:border-monday-dark-border rounded-xl p-4 shadow-sm col-span-2">
                    <div className="flex items-center justify-between mb-3">
                        <div>
                            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400">{t('timeline_readonly')}</p>
                            <p className="text-sm text-gray-600 dark:text-monday-dark-text-secondary">{t('upcoming_overdue_range')}</p>
                        </div>
                    ...
                    <div className="space-y-3">
                        ...
                        {tasks.length === 0 && <p className="text-sm text-gray-500">{t('no_items_yet')}</p>}
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-monday-dark-surface border border-gray-200 dark:border-monday-dark-border rounded-xl p-4 shadow-sm">
                    <div className="flex items-center justify-between mb-3">
                        <div>
                            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400">{t('priority_mix')}</p>
                            <p className="text-sm text-gray-600 dark:text-monday-dark-text-secondary">{t('based_on_tasks')}</p>
                        </div>
                        ...
                        {Object.entries(metrics.priorities).map(([priority, count]: [string, number]) => {
                            ...
                            return (
                                <div key={priority} className="flex items-center gap-3">
                                    <div className="w-24 text-xs font-semibold text-gray-600 dark:text-monday-dark-text-secondary truncate">{priority}</div>
                                    <div className="flex-1 h-2.5 rounded-full bg-gray-100 dark:bg-monday-dark-hover overflow-hidden">
