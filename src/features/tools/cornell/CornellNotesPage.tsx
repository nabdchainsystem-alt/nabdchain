...
const CornellNotesPage: React.FC<CornellNotesPageProps> = ({ roomId = 'default' }) => {
  ...
  // Popover Refs & State
  const [activeDateSelector, setActiveDateSelector] = useState<{
    id: 'note' | 'action';
    actionId?: string;
    rect: DOMRect;
  } | null>(null);
  ...
  const handleActionDateSelect = (actionId: string, d: Date) => {
    const newActions = actionItems.map((a) =>
      a.id === actionId ? { ...a, dueDate: d.toISOString().split('T')[0] } : a,
    );
    handleChange(setActionItems, newActions);
  ...
  const removeLink = (l: string) => {
    handleChange(
      setLinkedRecords,
      linkedRecords.filter((link) => link !== l),
    );
  };
    ...
    <div className="space-y-2 mt-2">
      ...
      <div className="space-y-1">
        {actionItems.map((item) => (
          <div
            key={item.id}
            className="group flex items-center gap-3 text-sm hover:bg-gray-50 dark:hover:bg-monday-dark-hover/50 p-1.5 rounded -ml-1.5"
          >
            <button
            ...
            <span
              className={`flex-1 ${item.done ? 'text-gray-400 line-through' : 'text-gray-700 dark:text-monday-dark-text'}`}
            >
              {item.title}
            </span>
  ...
  return (
    <div className="flex h-full w-full bg-[#FCFCFD] dark:bg-monday-dark-bg overflow-hidden relative">
      ...
      >
        <div className="p-4 border-b border-gray-100 dark:border-monday-dark-border flex items-center justify-between bg-gray-50/50 dark:bg-monday-dark-bg/50">
          ...
          <button
            ...
            className="text-gray-500 hover:text-gray-700 p-1 rounded hover:bg-gray-200 dark:hover:bg-monday-dark-hover"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {filteredNotes.map((note) => (
            ...
            >
              <div
                className={`mt-1 w-1.5 h-1.5 rounded-full flex-shrink-0 ${activeNoteId === note.id ? 'bg-blue-500' : 'bg-gray-300'}`}
              />
              <div className="flex-1 min-w-0">
                <div
                  className={`text-sm font-semibold truncate ${activeNoteId === note.id ? 'text-blue-900 dark:text-blue-100' : 'text-gray-800 dark:text-monday-dark-text'}`}
                >
                ...
                <div className="flex items-center gap-2 mt-1 text-xs text-gray-500">
                  ...
                  {note.tags.length > 0 && (
                    <span className="bg-gray-100 dark:bg-monday-dark-hover px-1.5 rounded text-[10px]">
                      {note.tags[0]}
                    </span>
                  )}
                </div>
          ...
          {filteredNotes.length === 0 && (
            <div className="text-center py-10 text-gray-400 text-sm">{t('no_notes_found')}</div>
          )}
        </div>
      </div>
      ...
      {/* Main Editor Area */}
      <div
        className={`flex-1 flex flex-col min-w-0 bg-[#FCFCFD] dark:bg-monday-dark-bg h-full ${confirmationModal.isOpen ? 'blur-sm brightness-90 transition-all duration-300' : 'transition-all duration-300'}`}
      >
        {/* Editor Header */}
          ...
          <div className="flex flex-wrap items-start gap-x-6 gap-y-3">
            ...
            <div className="flex-1 min-w-[200px] space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {tags.map((tag) => (
                  <span
                    key={tag}
                    className="inline-flex items-center gap-1 bg-gray-100 dark:bg-monday-dark-hover text-gray-600 dark:text-monday-dark-text-secondary text-[10px] px-1.5 py-0.5 rounded border border-gray-200 dark:border-monday-dark-border"
                  >
                    {tag}{' '}
            ...
            <div className="w-[160px]">
              <button
                ref={dateTriggerRef}
                onClick={() =>
                  setActiveDateSelector({ id: 'note', rect: dateTriggerRef.current!.getBoundingClientRect() })
                }
            ...
            <div className="flex-1 min-w-[200px] border-s border-gray-200 dark:border-monday-dark-border ps-6 space-y-1">
              <div className="flex flex-wrap gap-2 items-center">
                ...
                {linkedRecords.map((l) => (
                  <span
                    key={l}
                    className="inline-flex items-center gap-1 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-300 text-[10px] px-1.5 py-0.5 rounded border border-blue-100 dark:border-blue-900/30"
                  >
                    {l}{' '}
                    <button onClick={() => removeLink(l)} className="hover:text-blue-800">
                      <X size={10} />
                    </button>
                  </span>
                ))}
      ...
      {activeDateSelector && (
        <PortalPopup
          triggerRef={
            { current: { getBoundingClientRect: () => activeDateSelector.rect } } as React.RefObject<HTMLElement>
          }
        ...
        >
          <SharedDatePicker
            selectedDate={
              activeDateSelector.id === 'note'
                ? date
                : actionItems.find((a) => a.id === activeDateSelector.actionId)?.dueDate
            }
            onSelectDate={(d) => {
            ...
            onClear={() => {
              if (activeDateSelector.id === 'note') {
                ...
                const newActions = actionItems.map((a) =>
                  a.id === activeDateSelector.actionId ? { ...a, dueDate: undefined } : a,
                );
                handleChange(setActionItems, newActions);
                handleSave();
              }
