...
const formulaFunctions: Record<string, (args: string[], data: GridData, visited: Set<string>) => number | string> = {
    ...
    IF: (args, data, visited) => {
        ...
        for (const op of comparisons) {
            if (condArg.includes(op)) {
                ...
                const leftVal = parseCellRef(left) ? getCellValue(left, data, visited) : (isNaN(Number(left)) ? left : Number(left));
                const rightVal = parseCellRef(right) ? getCellValue(right, data, visited) : (isNaN(Number(right)) ? right : Number(right));
        ...
        const trueVal = args[1] ? (parseCellRef(args[1].trim()) ? getCellValue(args[1].trim(), data, visited) : args[1].trim()) : '';
        const falseVal = args[2] ? (parseCellRef(args[2].trim()) ? getCellValue(args[2].trim(), data, visited) : args[2].trim()) : '';
    ...
    UPPER: (args, data, visited) => {
        ...
        const val = parseCellRef(args[0].trim()) ? getCellValue(args[0].trim(), data, visited) : args[0].replace(/^"(.*)"$/, '$1');
