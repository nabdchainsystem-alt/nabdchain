...
export const Grid: React.FC<GridProps> = ({
  ...
  const [_dragColMouseY, setDragColMouseY] = useState(0);
  ...
  const _getColumnPosition = useCallback(
  ...
  const findColumnAtPosition = useCallback(
    (clientX: number): { col: string; position: 'before' | 'after' } | null => {
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      ...
      if (draggingCol && dropTargetCol && onColumnReorder) {
        ...
        if (
          fromIndex !== toIndex &&
          !(fromIndex === toIndex - 1 && dropPosition === 'after') &&
          !(fromIndex === toIndex + 1 && dropPosition === 'before')
        ) {
  ...
  const handleContextMenu = useCallback(
    (e: React.MouseEvent, col: string, row: number) => {
      e.preventDefault();
              ...
              if (refMatch) {
                ...
                requestAnimationFrame(() => {
                  input.setSelectionRange(
                    cursorPositionRef.current + cellRef.length,
                    cursorPositionRef.current + cellRef.length,
                  );
                });
              }
        ...
        >
          ...
          {onInsertRow && (
            <>
              <button
                onClick={() => {
                  onInsertRow(contextMenu.row, true);
                  setContextMenu((prev) => ({ ...prev, visible: false }));
                }}
                className="flex items-center gap-3 w-full px-4 py-2 text-sm text-left hover:bg-[#e8f0fe] transition-colors"
              >
              ...
              <button
                onClick={() => {
                  onInsertRow(contextMenu.row, false);
                  setContextMenu((prev) => ({ ...prev, visible: false }));
                }}
                className="flex items-center gap-3 w-full px-4 py-2 text-sm text-left hover:bg-[#e8f0fe] transition-colors"
              >
          ...
          {onInsertColumn && (
            <>
              <button
                onClick={() => {
                  onInsertColumn(contextMenu.col, true);
                  setContextMenu((prev) => ({ ...prev, visible: false }));
                }}
                className="flex items-center gap-3 w-full px-4 py-2 text-sm text-left hover:bg-[#e8f0fe] transition-colors"
              >
              ...
              <button
                onClick={() => {
                  onInsertColumn(contextMenu.col, false);
                  setContextMenu((prev) => ({ ...prev, visible: false }));
                }}
                className="flex items-center gap-3 w-full px-4 py-2 text-sm text-left hover:bg-[#e8f0fe] transition-colors"
              >
                  ...
                  return (
                    ...
                    >
                      {isEditing ? (
                        ...
                        <span
                          className={`block ${cellData?.style?.wrap ? '' : 'truncate'}`}
