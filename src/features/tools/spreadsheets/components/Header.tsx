          ...
          {items.map((item, index) => (
            <React.Fragment key={index}>
              ...
              >
                ...
                {item.shortcut && <span className="text-[#5f6368] text-[12px] ml-6">{item.shortcut}</span>}
              </button>
            </React.Fragment>
          ))}
        </div>
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const editMenuItems = [
    ...
    {
      label: 'Cut',
      onClick: () => document.dispatchEvent(new KeyboardEvent('keydown', { key: 'x', ctrlKey: true })),
      shortcut: 'Ctrl+X',
      divider: true,
    },
    {
      label: 'Copy',
      onClick: () => document.dispatchEvent(new KeyboardEvent('keydown', { key: 'c', ctrlKey: true })),
      shortcut: 'Ctrl+C',
    },
    {
      label: 'Paste',
      onClick: () => document.dispatchEvent(new KeyboardEvent('keydown', { key: 'v', ctrlKey: true })),
      shortcut: 'Ctrl+V',
    },
  ];
  ...
  const formatMenuItems = [
    ...
    {
      label: 'Italic',
      onClick: () => document.dispatchEvent(new KeyboardEvent('keydown', { key: 'i', ctrlKey: true })),
      shortcut: 'Ctrl+I',
    },
