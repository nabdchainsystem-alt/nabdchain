      ...
      <div className="flex-1 overflow-y-auto space-y-2 ltr:pr-1 rtl:pl-1 custom-scrollbar">
        {items.length > 0 ? (
          items.map((item) => (
            <div
              key={item.id}
              className="group relative flex items-center justify-between text-xs p-2 rounded-xl bg-gray-50 dark:bg-white/5 text-gray-600 dark:text-gray-400 border border-transparent hover:border-gray-200 dark:hover:border-white/10 transition-colors cursor-pointer"
            >
              <div className="flex flex-col flex-1 min-w-0 ltr:mr-2 rtl:ml-2">
                <span className="truncate font-medium">{item.title}</span>
                {item.scheduledAt && (
                  <span
                    className={`text-[9px] text-blue-500 font-bold uppercase ${isRTL ? '' : 'tracking-wider'} flex items-center gap-1 mt-0.5`}
                  >
                    <Calendar size={8} />
  ...
  return (
    <div className="w-full max-w-4xl mx-auto flex flex-col items-center pb-12">
      <div className="text-center mb-10">
        <h2
          className={`text-2xl font-black ${isRTL ? '' : 'tracking-widest'} uppercase mb-4 text-[#1A1A1A] dark:text-white`}
        >
          {t('organize')}
        </h2>
        <p
          className={`text-gray-400 ${isRTL ? 'font-sans font-medium' : 'font-serif italic tracking-widest'} text-sm uppercase`}
        >
          {t('clarify_outcomes_actions')}
        </p>
      </div>
