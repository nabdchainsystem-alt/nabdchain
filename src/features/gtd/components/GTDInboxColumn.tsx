...
export const GTDInboxColumn: React.FC<GTDInboxColumnProps> = ({ title, subtitle, count, _delay = 0, children }) => {
  const { language, t } = useAppContext();
  const isRTL = language === 'ar';

  return (
    <div className="flex flex-col h-[400px] border border-gray-100 dark:border-white/5 rounded-[2rem] p-8 bg-white dark:bg-monday-dark-bg shadow-[0_2px_40px_-10px_rgba(0,0,0,0.05)] relative overflow-hidden group hover:shadow-[0_8px_40px_-5px_rgba(0,0,0,0.1)] transition-all duration-500">
      <div className="flex justify-between items-start mb-2">
        <div>
          <h3
            className={`text-xs font-bold ${isRTL ? '' : 'tracking-[0.15em]'} text-[#1A1A1A] dark:text-white uppercase mb-1`}
          >
            {title}
          </h3>
      ...
      <div className="flex-1 flex flex-col items-start justify-start w-full text-gray-300 dark:text-gray-600">
        {children ? (
          ...
          <div
            className={`w-full h-full flex items-center justify-center text-[10px] ${isRTL ? '' : 'tracking-widest'} uppercase opacity-40`}
          >
            {t('empty')}
          </div>
        )}
