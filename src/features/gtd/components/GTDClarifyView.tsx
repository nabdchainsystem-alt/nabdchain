...
export const GTDClarifyView: React.FC<GTDClarifyViewProps> = ({ items, initialItemId, onProcess }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto flex flex-col items-center">
      <h2
        className={`text-2xl font-black ${isRTL ? '' : 'tracking-widest'} uppercase mb-6 text-[#1A1A1A] dark:text-white`}
      >
        {t('clarify')}
      </h2>
      ...
      {!currentItem ? (
        <div className="flex flex-col items-center justify-center p-8 text-center opacity-0 animate-[fadeIn_0.5s_ease-out_forwards]">
          ...
          <h3
            className={`text-2xl ${isRTL ? 'font-sans font-bold' : 'font-serif italic'} text-gray-800 dark:text-white mb-2`}
          >
            {t('all_clear')}
          </h3>
        ...
        <>
          ...
          <div className="relative w-full mb-6 group">
            ...
            >
              <div
                className={`absolute top-4 left-1/2 -translate-x-1/2 text-[10px] font-bold ${isRTL ? '' : 'tracking-widest'} uppercase text-gray-300`}
              >
                {t('item_x_of_y')
              ...
              <h3
                className={`text-lg ${isRTL ? 'font-sans font-medium' : 'font-serif italic'} text-[#1A1A1A] dark:text-gray-100 mb-4 px-4 leading-tight`}
              >
                "{currentItem.title || 'Untitled'}"
              </h3>

              <div
                className={`flex items-center justify-center gap-2 text-[10px] font-bold ${isRTL ? '' : 'tracking-widest'} uppercase text-gray-400 dark:text-gray-500`}
              >
                <span>{new Date(currentItem.createdAt).toLocaleDateString('en-US', { weekday: 'long' })}</span>
                <span>,</span>
                <span>
                  {new Date(currentItem.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                </span>
          ...
          <AnimatePresence mode="wait">
            {step === 'decision' && (
              ...
              >
                <h4
                  className={`text-center ${isRTL ? 'font-sans font-medium' : 'font-serif italic'} text-base text-gray-500 mb-6`}
                >
                  {t('is_this_actionable')}
                </h4>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <span className="text-sm font-bold text-gray-700 dark:text-gray-300 group-hover:text-red-500 mb-0.5">
                      {t('no')}
                    </span>
                    <span className="text-[10px] text-center text-gray-400 group-hover:text-red-400">
                      {t('trash_reference_someday')}
                    </span>
                  </button>
            ...
            {step === 'non-actionable' && (
              ...
              >
                <div className="flex items-center justify-between mb-6 px-2">
                  ...
                  <h4
                    className={`text-center ${isRTL ? 'font-sans font-medium' : 'font-serif italic'} text-base text-gray-500`}
                  >
                    {t('organize_non_actionables')}
                  </h4>
            ...
            {step === 'actionable' && (
              ...
              >
                ...
                >
                  ...
                  <ArrowRight
                    size={14}
                    className="text-blue-300 group-hover:ltr:translate-x-1 group-hover:rtl:-translate-x-1 transition-transform"
                  />
                </button>
                ...
                <div className="grid grid-cols-2 gap-3 mb-2">
                  ...
                  >
                    ...
                    <span className="font-bold text-xs text-gray-800 dark:text-white mb-0.5">
                      {t('defer_calendar')}
                    </span>
                  </button>
                </div>
                    ...
                    {[
                      {
                        label: t('today'),
                        date: new Date(),
                        sub: new Date().toLocaleDateString(language === 'ar' ? 'ar-SA' : 'en-US', { weekday: 'short' }),
                      },
                      {
                        label: t('tomorrow'),
                        date: new Date(Date.now() + 86400000),
                        sub: new Date(Date.now() + 86400000).toLocaleDateString(language === 'ar' ? 'ar-SA' : 'en-US', {
                          weekday: 'short',
                        }),
                      },
                      {
                        ...
                        sub: new Date(Date.now() + 7 * 86400000).toLocaleDateString(
                      ...
                      },
                      {
                        ...
                        sub: new Date(Date.now() + 14 * 86400000).toLocaleDateString(
                          language === 'ar' ? 'ar-SA' : 'en-US',
                          { month: 'short', day: 'numeric' },
                        ),
                      },
                      {
                        ...
                        sub: new Date(Date.now() + 28 * 86400000).toLocaleDateString(
                          language === 'ar' ? 'ar-SA' : 'en-US',
                          { month: 'short', day: 'numeric' },
                        ),
                      },
                      ...
                      <button
                        ...
                        className={`w-full text-left px-3 py-1.5 rounded-lg flex items-center justify-between group transition-colors ${
                          selectedDate && option.date.toDateString() === selectedDate.toDateString()
                            ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400'
                            : 'hover:bg-gray-200 dark:hover:bg-white/10'
                        }`}
                      >
                        <span
                          className={`text-xs font-medium ${selectedDate && option.date.toDateString() === selectedDate.toDateString() ? 'text-blue-600 dark:text-blue-400' : 'text-gray-700 dark:text-gray-200'}`}
                        >
                          {option.label}
                        </span>
                        <span
                          className={`text-[10px] font-datetime ${selectedDate && option.date.toDateString() === selectedDate.toDateString() ? 'text-blue-400 dark:text-blue-300' : 'text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300'}`}
                        >
                          {option.sub}
                        </span>
                      </button>
                ...
                <div className="flex-1 p-5 flex flex-col">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-baseline gap-2">
                      <span
                        className={`text-lg font-bold text-gray-900 dark:text-white ${isRTL ? 'font-sans' : 'font-serif'}`}
                      >
                        {currentMonth.toLocaleDateString(language === 'ar' ? 'ar-SA' : 'en-US', {
                          month: 'long',
                          year: 'numeric',
                        })}
                      </span>
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="p-1 hover:bg-gray-100 dark:hover:bg-white/10 rounded-full text-gray-500"
                      >
                        <ChevronLeft size={16} />
                      </button>
                      <button
                  ...
                  <div className="flex-1">
                    <div className="grid grid-cols-7 mb-2 text-center">
                      {(language === 'ar'
                        ...
                        <div
                          key={day}
                          className={`text-[10px] font-bold text-gray-400 uppercase ${isRTL ? '' : 'tracking-wider'}`}
                        >
                          {day}
                        </div>
                      ))}
