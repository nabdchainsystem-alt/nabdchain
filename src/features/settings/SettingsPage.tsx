...
import { ALL_PAGES } from './settingsConfig';
...
export const SettingsPage: React.FC<SettingsPageProps> = ({
  ...
  const fetchAdminData = useCallback(async () => {
    ...
    try {
      ...
      const [flags, users] = await Promise.all([adminService.getFeatureFlags(token), adminService.getUsers(token)]);
  ...
  const handleToggleUserPermission = async (pageKey: string, currentEnabled: boolean, _source: 'user' | 'global') => {
    ...
    try {
      ...
      await adminService.updateUserPermissions(token, selectedUserForPerms.id, [{ pageKey, enabled: newEnabled }]);

      // Update local state
      setUserPermissions((prev) =>
        prev.map((p) => (p.pageKey === pageKey ? { ...p, enabled: newEnabled, source: 'user' } : p)),
      );
    } catch (error) {
  ...
  const handleResetUserPermission = async (pageKey: string) => {
    ...
    try {
      ...
      await adminService.updateUserPermissions(token, selectedUserForPerms.id, [{ pageKey, enabled: null }]);

      // Update local state - revert to global
      setUserPermissions((prev) =>
        prev.map((p) => (p.pageKey === pageKey ? { ...p, enabled: p.globalEnabled, source: 'global' } : p)),
      );
    } catch (error) {
  ...
  const handleSaveName = async () => {
    ...
    if (trimmedName) {
      ...
      const userWithUpdate = user as Record<string, unknown> | null;
      if (userWithUpdate && typeof userWithUpdate.update === 'function') {
        try {
          const nameParts = trimmedName.split(' ');
          const firstName = nameParts[0];
          const lastName = nameParts.slice(1).join(' ');

          await (userWithUpdate.update as (data: Record<string, string>) => Promise<void>)({
            firstName: firstName,
            lastName: lastName,
          });
        } catch (error) {
                ...
                <div className="flex items-center gap-2">
                  <div className="relative flex-1">
                    <input
                      ...
                      className={`w-full px-4 py-2 border rounded-xl transition-all ${
                        isEditingName
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-medium text-gray-900 dark:text-white">
                  {t('region_currency') || 'Region & Currency'}
                </p>
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {language === 'ar'
                    ? `اختر دولتك لتحديث العملة (${country.currency.symbol})`
                    : `Select your country to update currency (${country.currency.symbol})`}
                </p>
              </div>
            </div>
            <select
    ...
    <div className="animate-fadeIn space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {sections.map((section) => (
          <div
            key={section}
            className="bg-white dark:bg-monday-dark-surface rounded-2xl border border-gray-100 dark:border-monday-dark-border shadow-sm overflow-hidden hover:shadow-md transition-shadow duration-300"
          >
                ...
                .map(([key, config]) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`p-2 rounded-lg transition-colors ${isVisible ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400' : 'bg-gray-100 text-gray-400 dark:bg-gray-800 dark:text-gray-500'}`}
                        >
                          <Icon size={18} />
                        </div>
                        <span
                          className={`text-sm font-medium transition-colors ${isVisible ? 'text-gray-700 dark:text-monday-dark-text' : 'text-gray-400 line-through decoration-gray-300'}`}
                        >
                          {t(config.label)}
                        </span>
                      </div>
                      ...
                      <div
                        className={`relative w-11 h-6 rounded-full transition-all duration-300 ease-out flex-shrink-0 cursor-pointer border-2 ${
                          isVisible
                      ...
                      >
                        <span
                          className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow-sm transform transition-transform duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${
  ...
  const renderNotifications = () => (
    <div className="space-y-6 animate-fadeIn">
      <div className="bg-white dark:bg-monday-dark-surface p-8 rounded-2xl border border-gray-100 dark:border-monday-dark-border text-center">
        ...
        <p className="text-gray-500 dark:text-gray-400 max-w-md mx-auto mb-6">{t('manage_notifications_desc')}</p>
        ...
        <div className="max-w-md mx-auto space-y-4 text-start">
          {[
            ...
            <div
              key={item.key}
              className="flex items-center justify-between p-3 border border-gray-100 dark:border-monday-dark-border rounded-xl"
            >
      ...
      <div className="space-y-6 animate-fadeIn">
          ...
          {isLoadingFeatures ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Object.entries(FEATURE_CATEGORIES).map(([category, keys]) => (
                <div
                  key={category}
                  className="border border-gray-100 dark:border-monday-dark-border rounded-xl overflow-hidden"
                >
                  ...
                  <div className="p-2 space-y-1">
                    {keys.map((key) => {
                      ...
                      return (
                        ...
                        >
                          <span
                            className={`text-sm ${isEnabled ? 'text-gray-700 dark:text-gray-200' : 'text-gray-400 dark:text-gray-500'}`}
                          >
        ...
        <div className="bg-white dark:bg-monday-dark-surface p-6 rounded-2xl border border-gray-100 dark:border-monday-dark-border shadow-sm">
          ...
          <div className="mb-6">
            ...
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Note: Admin users always see all pages</p>
          </div>
            ...
            <>
                        ...
                        {keys.map((key) => {
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-2">
                                <span
                                  className={`text-sm ${isEnabled ? 'text-gray-700 dark:text-gray-200' : 'text-gray-400 dark:text-gray-500 line-through'}`}
                                >
                                  {FEATURE_LABELS[key] || key}
                                </span>
                                {isUserOverride && (
              ...
              <div className="mt-4 p-3 bg-gray-50 dark:bg-monday-dark-hover/30 rounded-lg">
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  <span className="inline-block w-3 h-3 bg-green-500 rounded-full mr-1"></span> Green = Using global
                  setting
                  <span className="inline-block w-3 h-3 bg-blue-500 rounded-full ml-3 mr-1"></span> Blue = Custom
                  override
              ...
              <table className="w-full">
                ...
                <tbody>
                  {adminUsers.map((adminUser) => (
                    <tr
                      key={adminUser.id}
                      className="border-b border-gray-50 dark:border-monday-dark-border/50 hover:bg-gray-50 dark:hover:bg-monday-dark-hover/50"
                    >
                      ...
                      <td className="py-3 px-4">
                        {adminUser.id !== user?.id && (
                          <button
                            onClick={() =>
                              handleChangeUserRole(adminUser.id, adminUser.role === 'admin' ? 'member' : 'admin')
                            }
                        ...
                        {adminUser.id === user?.id && <span className="text-xs text-gray-400">(You)</span>}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        >
          ...
          <span
            className={`font-medium ${
              toast.type === 'success' ? 'text-green-800 dark:text-green-100' : 'text-red-800 dark:text-red-100'
            }`}
          >
            {toast.message}
          </span>
