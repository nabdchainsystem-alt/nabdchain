  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [statsData, ordersRes] = await Promise.all([
        marketplaceOrderService.getSellerTrackingStats(),
        marketplaceOrderService.getSellerOrders({ limit: 100 }),
      ]);

      setStats(statsData);
      // Show orders in active fulfillment (confirmed through delivered)
                          ...
                          {order.trackingNumber && (
                            ...
                            >
                              <p className="text-xs font-medium" style={{ color: styles.textSecondary }}>
                                {t('seller.tracking.trackingInfo') || 'Tracking Info'}
                              </p>
