...
export const SellerSales: React.FC<SellerSalesProps> = ({ onViewOrder }) => {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      const data = await orderService.getSellerOrders(token, filters);
      setOrders(data);
    } catch (err) {
  ...
  const getNextStatuses = useCallback((currentStatus: OrderStatus): { value: string; label: string }[] => {
    ...
    <div>
      ...
      {!loading && orders.length === 0 && !hasActiveFilters && (
        <div className="py-12">
          <EmptyState
            icon={Package}
            title={t('seller.sales.noOrders')}
            description={t('seller.sales.noOrdersDesc')}
          />
        </div>
      )}
