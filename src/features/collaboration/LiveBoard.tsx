  ...
  const ToolButton = ({ tool, icon, label }: { tool: Tool; icon: React.ReactNode; label: string }) => (
    <button
      ...
      className={`p-2 rounded-lg transition-all ${
        activeTool === tool ? 'bg-blue-500 text-white shadow-md' : 'bg-white hover:bg-gray-100 text-gray-700'
      }`}
      title={label}
    >
      {icon}
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-white rounded-2xl shadow-xl p-2 border border-gray-200 z-50">
          ...
          {BRUSH_SIZES.map((size) => (
            <button
              ...
              className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${
                brushSize === size ? 'bg-blue-500 text-white' : 'bg-white hover:bg-gray-100'
              }`}
              title={`Size ${size}`}
            >
        ...
        <div className="relative">
          <button
            ...
            title="Color picker"
          >
            <div className="w-6 h-6 rounded-full border-2 border-gray-300" style={{ backgroundColor: activeColor }} />
        ...
        <button
          ...
          className={`p-2 rounded-lg transition-colors ${
            history.length === 0 ? 'text-gray-300 cursor-not-allowed' : 'hover:bg-gray-100 text-gray-700'
          }`}
          title="Undo"
        >
