...
export const GlobalSearchDrawer: React.FC<GlobalSearchDrawerProps> = ({ isOpen, onClose, boards, onNavigate }) => {
  ...
  const results = useMemo(() => {
    ...
    if (selectedType === 'all' || selectedType === 'boards') {
      matchedBoards = boards.filter((b) => b.name.toLowerCase().includes(query));
    }

    // 2. Search Tasks
    const matchedTasks: (Task & { boardName: string; boardId: string })[] = [];
    if (selectedType === 'all' || selectedType === 'tasks') {
      for (const board of boards) {
        if (board.tasks) {
          const found = board.tasks
            .filter((task) => {
              // Text Match
              const textMatch =
                task.name.toLowerCase().includes(query) || (task.id && task.id.toLowerCase().includes(query));
        ...
        >
          ...
          >
                          ...
                          <div className="flex flex-wrap gap-2">
                            {filterOptions.people.map((person) => (
                              <button
                                ...
                                className={`flex items-center gap-2 px-2 py-1 rounded-full text-xs transition-colors border ${
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {results.tasks.map((task) => (
                      ...
                      >
                        <div
                          className={`w-1.5 h-1.5 rounded-full shrink-0 ${task.priority === 'High' ? 'bg-red-500' : task.priority === 'Medium' ? 'bg-orange-400' : 'bg-green-400'}`}
                        />

                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <h4 className="font-medium text-sm text-gray-900 truncate group-hover:text-blue-700">
                              {task.name}
                            </h4>
                          </div>
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-gray-400 shrink-0">
                          {task.date && (
                            <span className="bg-gray-50 px-1.5 py-0.5 rounded border border-gray-100 font-datetime">
                              {task.date}
                            </span>
                          )}
                        </div>
            ...
            <div className="p-3 bg-gray-50 border-t border-gray-100 text-center text-[10px] text-gray-400 flex items-center justify-between shrink-0">
              <span>
                <kbd className="px-1 py-0.5 bg-white border border-gray-200 rounded text-gray-500 font-sans">ESC</kbd>{' '}
                {t('close')}
              </span>
              <span>
                {results.tasks.length + results.boards.length} {t('results')}
              </span>
            </div>
          </motion.div>
        </motion.div>
