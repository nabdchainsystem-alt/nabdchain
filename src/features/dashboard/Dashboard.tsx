...
export const Dashboard: React.FC<DashboardProps> = memo(
  ({
    ...
    const filteredPeopleAndTeams = useMemo(() => {
      return peopleAndTeams.filter(
        (item) =>
          item.name.toLowerCase().includes(personSearchQuery.toLowerCase()) && !selectedPersons.includes(item.name),
      );
    ...
    const [weather, setWeather] = useState<{ temp: number; condition: string; city: string; country: string } | null>(
      null,
    );
    ...
    const urgentTasks = useMemo(() => {
      ...
      if (activeFilter === 'high') {
        filteredTasks = filteredTasks.filter(
          (t) => t.normalizedPriority === 'High' || t.normalizedPriority === 'Urgent',
        );
      } else if (activeFilter === 'overdue') {
        const now = new Date();
        filteredTasks = filteredTasks.filter(
          (t) => t.date && new Date(t.date) < now && t.status !== 'Done' && t.status !== 'Completed',
        );
        ...
        if (selectedPersons.length > 0) {
          ...
          filteredTasks = filteredTasks.filter((t) => {
            ...
            const personName = getPersonName(t.person || (t as Record<string, unknown>).people).toLowerCase();
    ...
    const paginatedUrgentTasks = urgentTasks.slice(
      (urgentTasksPage - 1) * ITEMS_PER_PAGE,
      urgentTasksPage * ITEMS_PER_PAGE,
    );
    ...
    const handleNewTaskSave = async (
      taskData: { name: string; priority: string; date: string },
      location: { type: 'existing' | 'new'; boardId?: string; workspaceId?: string; newBoardName?: string },
    ) => {
      try {
        const token = await getToken();
        if (!token) return;
        ...
        if (targetBoardId) {
          ...
          const boardName =
            location.type === 'new'
              ? location.newBoardName
              : boards.find((b: Board) => b.id === targetBoardId)?.name || 'board';
      ...
      <div className="flex-1 overflow-y-auto bg-gray-50 dark:bg-monday-dark-elevated dark:bg-monday-dark-bg p-6 lg:p-8 h-full">
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 dark:text-monday-dark-text">
                {t(greeting)}, {userDisplayName}!
              </h1>
              <p className="text-gray-500 dark:text-monday-dark-text-secondary dark:text-monday-dark-text-secondary mt-1">
                {t('daily_overview')}
              </p>
            </div>
              ...
              <div className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-monday-dark-surface rounded-full shadow-sm border border-gray-100 dark:border-monday-dark-border dark:border-monday-dark-border hidden sm:flex">
                <span className="text-gray-500 dark:text-monday-dark-text-secondary">
                  {weather?.condition.includes('Cloud') ? (
                    <Cloud size={20} weight="light" />
                  ) : weather?.condition.includes('Rain') ? (
                    <CloudRain size={20} weight="light" />
                  ) : weather?.condition.includes('Snow') ? (
                    <Snowflake size={20} weight="light" />
                  ) : weather?.condition.includes('Storm') ? (
                    <Lightning size={20} weight="light" />
                  ) : weather?.condition.includes('Fog') ? (
                    <CloudFog size={20} weight="light" />
                  ) : (
                    <Sun size={20} weight="light" />
                  )}
                </span>
                <span className="text-sm font-semibold text-gray-700 dark:text-monday-dark-text">
              ...
              <div className="flex items-center gap-4">
                <h2 className="text-lg font-bold text-gray-800 dark:text-monday-dark-text flex items-center gap-2">
                  <ClockCounterClockwise
                    size={24}
                    weight="light"
                    className="text-gray-400 dark:text-monday-dark-text-muted"
                  />
                  {t('recently_visited')}
                </h2>
                {/* Scroll Controls */}
                      ...
                      >
                        ...
                        <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              onNavigate(item.type, item.boardId);
                            }}
                          >
                          ...
                          <button
                            ...
                            title="Favorite"
                            onClick={(e) => {
                              e.stopPropagation(); /* Future: toggle favorite status */
                            }}
                          >
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-gray-50 mt-1">
                          {stats ? (
                            ...
                            <span className="text-xs text-black font-medium opacity-0 group-hover:opacity-100 transition-opacity">
                              {t('open')}
                            </span>
                          )}
                    ...
                    <div className="flex items-center justify-end min-w-[80px]">
                      ...
                      <div className="flex items-center bg-gray-50 dark:bg-monday-dark-elevated rounded-lg p-0.5 border border-gray-100 dark:border-monday-dark-border relative">
                        ...
                        <div ref={personSearchRef} className="relative flex items-center w-8">
                          {isPersonSearchOpen ? (
                            <div className="absolute right-0 top-1/2 -translate-y-1/2 flex items-center w-[260px] bg-white rounded-md shadow-lg border border-blue-100 z-50">
                              ...
                              <div className="flex-1 flex items-center overflow-hidden min-w-0 flex-wrap gap-1 py-1">
                                ...
                                <input
                                  ...
                                  onKeyDown={(e) => {
                                    const filteredItems = peopleAndTeams.filter(
                                      (item) =>
                              ...
                              {(personSearchQuery || selectedPersons.length === 0) && (
                                <div className="absolute top-full left-0 w-full mt-2 bg-white rounded-lg shadow-lg border border-gray-100 dark:border-monday-dark-border py-1 z-50 max-h-48 overflow-y-auto">
                                    ...
                                    <button
                                      ...
                                      className={`w-full text-left px-3 py-2 text-xs text-gray-700 dark:text-monday-dark-text flex items-center gap-2 transition-colors ${
                                        idx === highlightedIndex
                                          ...
                                          : 'hover:bg-gray-50 dark:hover:bg-monday-dark-elevated dark:bg-monday-dark-elevated'
                                      }`}
                                    >
                                      <div
                                        className={`w-5 h-5 rounded-full ${item.type === 'team' ? 'bg-indigo-500' : item.color} flex items-center justify-center text-[10px] font-bold text-white`}
                                      >
                                        {item.initials.charAt(0)}
                                      </div>
                                      <span className="flex-1">{item.name}</span>
                                      {item.type === 'team' && (
                                        <span className="text-[9px] px-1.5 py-0.5 bg-indigo-100 text-indigo-600 rounded">
                                          {t('team')}
                                        </span>
                                      )}
                                    </button>
                                  ...
                                  {filteredPeopleAndTeams.length === 0 && (
                                    <div className="px-3 py-2 text-xs text-gray-400 dark:text-monday-dark-text-muted text-center">
                                      {t('no_matches')}
                                    </div>
                                  )}
                                </div>
                    ...
                    {paginatedUrgentTasks.length > 0 ? (
                      <>
                        {paginatedUrgentTasks.map((task) => (
                          <div
                            key={task.id}
                            className="flex items-center p-3 rounded-lg border border-gray-100 dark:border-monday-dark-border hover:bg-gray-50 dark:hover:bg-monday-dark-elevated dark:bg-monday-dark-elevated transition-colors group"
                          >
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-gray-500 dark:text-monday-dark-text-secondary mt-1 flex items-center gap-1">
                                ...
                                {task.boardName}
                                {getPersonName(task.person || (task as Record<string, unknown>).people) && (
                                  <>
                                    <span className="mx-1">•</span>
                            ...
                            <div className="flex-shrink-0 w-20 flex justify-center ml-4">
                              <span
                                ...
                                ${
                                  task.normalizedPriority === 'Urgent'
                                    ? 'bg-red-100 text-red-800'
                                    : task.normalizedPriority === 'High'
                  ...
                  <div className="flex items-center justify-end pt-4 mt-auto">
                    {totalUrgentPages > 1 && (
                      <div className="flex items-center bg-gray-50 dark:bg-monday-dark-elevated rounded-lg p-0.5 border border-gray-100 dark:border-monday-dark-border shadow-sm">
                        ...
                        <span className="text-[10px] font-medium text-gray-500 dark:text-monday-dark-text-secondary px-2 select-none">
                          {urgentTasksPage}/{totalUrgentPages}
                        </span>
                        <button
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      <ListPlus
                        size={32}
                        weight="light"
                        className="mb-2 text-gray-400 dark:text-monday-dark-text-muted group-hover:text-blue-500 transition-colors"
                      />
                        ...
                        activities.slice(0, 5).map((activity, idx) => {
                          ...
                          if (language === 'ar') {
                            ...
                            if (activity.type === 'BOARD_DELETED' && metadata?.boardName) {
                              content = (t('activity_deleted_board') || 'Deleted board: {name}').replace(
                                '{name}',
                                metadata.boardName,
                              );
                            } else if (activity.type === 'BOARD_CREATED' && metadata?.boardName) {
                              content = (t('activity_created_board') || 'Created board: {name}').replace(
                                '{name}',
                                metadata.boardName,
                              );
                            } else {
                              ...
                              if (updatedStatusMatch) {
                                ...
                                content = (
                                  t('activity_updated_status') || 'Updated "{task}" status to {status} in {board}'
                                )
                                  .replace('{task}', taskName)
                              ...
                              if (sentEmailMatch) {
                                content = (t('activity_sent_email') || 'Sent email to {email}').replace(
                                  '{email}',
                                  sentEmailMatch[1],
                                );
                              }
                            ...
                            <li key={activity.id}>
                              <div className="relative pb-8">
                                {idx !== activities.length - 1 && (
                                  <span
                                    aria-hidden="true"
                                    className={`absolute top-4 h-full w-0.5 bg-gray-200 ${language === 'ar' ? 'right-4 -mr-px' : 'left-4 -ml-px'}`}
                                  ></span>
                                )}
                                <div className="relative flex gap-3">
                                  <div>
                                    {(() => {
                                      ...
                                      return (
                                        <span
                                          className={`h-8 w-8 rounded-full flex items-center justify-center ring-8 ring-white ${styles.bg}`}
                                        >
        ...
        <NewEmailModal isOpen={isEmailModalOpen} onClose={() => setIsEmailModalOpen(false)} onSend={handleEmailSend} />

        <PaymentRequestPanel isOpen={isPaymentRequestOpen} onClose={() => setIsPaymentRequestOpen(false)} />
          ...
          <div className="fixed inset-0 z-[70] flex items-center justify-center">
            <div className="bg-white dark:bg-monday-dark-surface rounded-xl shadow-2xl w-96 max-h-[80vh] flex flex-col">
              ...
              <div className="p-5 border-b border-gray-100 dark:border-monday-dark-border flex justify-between items-center">
                <h3 className="font-semibold text-lg text-gray-800 dark:text-gray-100">{t('select_workspace')}</h3>
                <button
                  onClick={() => {
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2">
                {workspaces.map((workspace) => (
                  ...
                  >
                    <div
                      className={`w-10 h-10 rounded-lg bg-gradient-to-br ${workspace.color || 'from-gray-400 to-gray-500'} flex items-center justify-center text-white`}
                    >
