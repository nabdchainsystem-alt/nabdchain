...
const ConversationSidebar: React.FC<ConversationSidebarProps> = ({ conversationId, _onNavigate }) => {
  ...
  const handleSendToBoard = async () => {
    ...
    try {
      ...
      const existingTasks = boardData.tasks
        ? typeof boardData.tasks === 'string'
          ? JSON.parse(boardData.tasks)
          : boardData.tasks
        : [];
      ...
      await boardService.updateBoard(token, selectedBoardId, {
        tasks: [...existingTasks, newBoardTask],
      });
  ...
  if (!conversationId) {
    return (
      <aside
        className="w-72 border-s flex-col h-full bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 flex shrink-0"
  ...
  return (
    <>
      <aside
        className="w-72 border-s flex-col h-full bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 flex shrink-0 transition-all"
        dir={language === 'ar' ? 'rtl' : 'ltr'}
      >
        {/* Tasks Section */}
            ...
            {showAddTask && (
              <div className="p-2 border border-gray-200 dark:border-gray-700 rounded-lg mb-2">
                ...
                <div className="flex gap-2 mt-2">
                  <button onClick={() => setShowAddTask(false)} className="flex-1 py-1.5 text-xs text-gray-500">
                    {t('cancel')}
                  </button>
                  <button
                    onClick={handleAddTask}
                    disabled={!newTaskName.trim()}
                    className="flex-1 py-1.5 text-xs bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded"
                  >
                    {t('add')}
                  </button>
                </div>
                  ...
                  <div className="flex items-start gap-2">
                    <button
                      ...
                      className={`mt-0.5 w-4 h-4 rounded border flex items-center justify-center ${
                        task.status === 'completed' ? 'bg-gray-900 dark:bg-white' : 'border-gray-300'
                      }`}
                    >
                    ...
                    <div className="flex-1 min-w-0 text-start">
                      <p
                        className={`text-sm truncate ${task.status === 'completed' ? 'line-through text-gray-400' : 'text-gray-700 dark:text-gray-300'}`}
                      >
                        {task.name}
                      </p>
                      {task.sentToBoard && (
                        <p className="text-xs text-gray-500 flex items-center gap-1 mt-0.5">
                          <ArrowSquareOut size={10} /> {task.boardName}
                        </p>
                      )}
                    </div>
          ...
          <div className="p-3 space-y-1 overflow-y-auto flex-1">
            {showAddReminder && (
              <div className="p-2 border border-gray-200 dark:border-gray-700 rounded-lg mb-2">
                ...
                />
                <div className="flex gap-2 mt-2">
                  <button onClick={() => setShowAddReminder(false)} className="flex-1 py-1.5 text-xs text-gray-500">
                    {t('cancel')}
                  </button>
                  <button
                ...
                <div key={reminder.id} className="group p-2 rounded hover:bg-gray-50 dark:hover:bg-gray-800/50">
                  <div className="flex items-start gap-2">
                    <button
                      ...
                      className={`mt-0.5 w-4 h-4 rounded border flex items-center justify-center ${
                        reminder.completed ? 'bg-gray-900 dark:bg-white' : 'border-gray-300'
                      }`}
                    >
                      {reminder.completed && (
                        <Check size={10} className="text-white dark:text-gray-900" weight="bold" />
                    ...
                    <div className="flex-1 min-w-0 text-start">
                      <p
                        className={`text-sm truncate ${reminder.completed ? 'line-through text-gray-400' : 'text-gray-700 dark:text-gray-300'}`}
                      >
                        {reminder.text}
                      </p>
                      <p
                        className={`text-xs flex items-center gap-1 mt-0.5 ${isOverdue(reminder.dueDate) && !reminder.completed ? 'text-red-500' : 'text-gray-400'}`}
                      >
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center" dir={language === 'ar' ? 'rtl' : 'ltr'}>
          ...
          <div className="bg-white dark:bg-monday-dark-surface p-5 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] max-w-sm w-full m-4 border border-gray-200 dark:border-monday-dark-border relative z-10 scale-in-center">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowSendToBoardModal(false)} className="text-gray-400 hover:text-gray-600">
                <X size={18} />
              </button>
            </div>
              ...
              <div className="max-h-40 overflow-y-auto mb-4 space-y-1 custom-scrollbar">
                {boards.map((board) => (
                  <button
                    ...
                    className={`w-full flex items-center p-2.5 rounded-lg transition-all text-start text-sm ${
                  ...
                  >
                    <div
                      className={`w-2 h-2 rounded-full me-2.5 ${selectedBoardId === board.id ? 'bg-primary' : 'bg-gray-300'}`}
                    />
                    {board.name}
                  </button>
                ))}
            ...
            <div className="flex gap-2 pt-4 border-t border-gray-100 dark:border-monday-dark-border">
              ...
              >
                {isSending ? (
                  ...
                  <>
                    <ArrowSquareOut size={16} weight="bold" /> {t('send')}
                  </>
                )}
              </button>
            </div>
