      ...
      <div className="px-8 py-4 space-y-4 max-w-5xl mx-auto w-full">
        ...
        <div className="flex items-baseline gap-4 group">
          ...
          <div className="flex-1 flex items-center justify-between border-b border-transparent group-hover:border-gray-200 py-1">
            <span className="text-sm text-gray-800 dark:text-gray-200">{accounts?.[0]?.email || 'me'}</span>
        ...
        <div className="flex items-baseline gap-4 group relative">
          ...
          <div className="absolute right-0 top-1 text-xs text-gray-500 flex gap-2">
            <button
              ...
              className={`hover:text-gray-800 ${showCc ? 'text-monday-blue font-bold' : ''}`}
            >
              Cc
            </button>
            <button
              onClick={() => setShowBcc(!showBcc)}
              className={`hover:text-gray-800 ${showBcc ? 'text-monday-blue font-bold' : ''}`}
            >
              Bcc
            </button>
          </div>
...
const ToolBtn = ({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick?: () => void }) => (
  <button
    onClick={onClick}
    className="flex items-center gap-1.5 px-2 py-1.5 hover:bg-gray-100 dark:hover:bg-monday-dark-hover rounded text-xs font-medium transition-colors"
  >
    {icon}
    <span>{label}</span>
  </button>
...
const IconBtn = ({
  ...
  <button
    className={`flex items-center gap-0.5 p-1 rounded hover:bg-gray-200 dark:hover:bg-monday-dark-hover transition-colors ${isActive ? 'bg-gray-200 dark:bg-monday-dark-hover' : ''}`}
  >
    {icon}
