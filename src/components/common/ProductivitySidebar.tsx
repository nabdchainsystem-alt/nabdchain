...
const ProductivitySidebar: React.FC<ProductivitySidebarProps> = ({
  ...
  const { _t, activeWorkspaceId } = useAppContext();
  ...
  const loadTasks = useCallback(async () => {
    try {
      ...
      boards.forEach((board) => {
        if (board.tasks) {
          try {
            const boardTasks = typeof board.tasks === 'string' ? JSON.parse(board.tasks) : board.tasks;
  ...
  const content = (
    <>
      ...
      >
        ...
        >
          <h3
            className={`font-semibold uppercase tracking-wider text-text-secondary-light dark:text-text-secondary-dark flex items-center ${isRight ? 'text-sm' : 'text-[11px]'}`}
          >
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        >
          ...
          <button onClick={handleViewFiles} className="text-primary hover:text-primary-dark text-xs font-medium">
            View All
          </button>
        </div>
        <div
        ...
        >
          <div className="text-text-secondary-light dark:text-text-secondary-dark opacity-50">
            ...
            <button onClick={handleViewFiles} className="mt-3 text-xs text-primary hover:text-primary-dark">
              Open Vault
            </button>
          </div>
        </div>
