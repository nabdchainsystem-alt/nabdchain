...
export const FocusMode: React.FC = () => {
  const { isActive, isSessionActive, timeLeft, toggleFocus, resetFocus, cancelFocus, formatTime, startFocus } =
    useFocus();
  ...
  return (
    <motion.div
      layout="position"
      key={dir}
      className={`flex items-center h-8 transition-colors duration-300 overflow-hidden ${
        isSessionActive
          ? 'ps-1 pe-2 py-0.5'
    ...
    >
      ...
      <button
        onClick={handleStart}
        className={`flex items-center justify-center rounded-full transition-colors flex-shrink-0 cursor-pointer ${
          isSessionActive
