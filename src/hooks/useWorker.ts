...
function getWorker(): Worker | null {
  ...
  if (!workerInstance) {
    try {
      workerInstance = new Worker(new URL('../workers/computeWorker.ts', import.meta.url), { type: 'module' });
...
export function useWorkerSearch<T extends Record<string, unknown>>() {
  ...
  const search = useCallback(async (data: T[], query: string, fields: string[], fuzzy = false): Promise<T[]> => {
...
export function useWorkerAggregate<T extends Record<string, unknown>>() {
  ...
  const aggregate = useCallback(
    async (
      ...
      setIsLoading(true);
      try {
        return await sendToWorker<unknown, Record<string, unknown>[]>('AGGREGATE_DATA', {
          data,
          groupBy,
          aggregations,
        });
...
export function useWorkerStats<T extends Record<string, unknown>>() {
  ...
  const calculateStats = useCallback(
    async (
      ...
      Record<string, { count: number; sum: number; avg: number; min: number; max: number; stdDev: number }>
    > => {
      setIsLoading(true);
      try {
