...
function generateFingerprint(error: Error): string {
  const parts = [error.name, error.message.slice(0, 100), error.stack?.split('\n')[1]?.trim() || ''];
  return parts.join('|');
}
...
export function captureError(
  ...
  const { severity = 'error', extra, tags, userId } = options;
  ...
  if (config.environment === 'development') {
    ...
    errorLogger.info('Context:', report.context);
    // eslint-disable-next-line no-console
    console.groupEnd();
  }
}
...
export function captureMessage(
  ...
  if (config.environment === 'development') {
    errorLogger.info(`[${severity.toUpperCase()}] ${message}`, extra);
  }
}
...
export function setUser(userId: string | null): void {
  if (userId) {
    (config as unknown as Record<string, unknown>).userId = userId;
  } else {
    delete (config as unknown as Record<string, unknown>).userId;
  }
}
...
export function withErrorTracking<T extends (...args: any[]) => any>(fn: T, name?: string): T {
