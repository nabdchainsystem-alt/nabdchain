...
export async function preloadCriticalData(
  ...
  if (fetchers.workspaces) {
    promises.push(preloadData('workspaces', fetchers.workspaces).catch(() => null));
  }

  if (fetchers.boards) {
    promises.push(preloadData('boards', fetchers.boards).catch(() => null));
  }

  if (fetchers.user) {
    promises.push(preloadData('user', fetchers.user).catch(() => null));
  }
