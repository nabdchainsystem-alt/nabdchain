...
const USE_MOCK_AUTH = import.meta.env.VITE_USE_MOCK_AUTH === 'true' || localStorage.getItem('nabd_dev_mode') === 'true';
...
export const useClerk = () => {
  if (USE_MOCK_AUTH) {
    ...
    return {
      ...
      openUserProfile: () => {
        alert(
          'Profile management is not available in Developer Mode. Please use Standard Mode for full account features.',
        );
      },
    };
  }
...
export const MockSignIn: React.FC = () => {
  const { loginAsMaster, _loginAsSam, _signOut } = useMockAuthContext();
  ...
  return (
    <div className="flex flex-col items-center gap-4 p-8 border rounded-lg shadow-md bg-white w-full max-w-md">
      ...
      <div className="text-base text-gray-500 mb-4 max-w-[260px] text-center">Choose your login method</div>

      {/* Standard User Login (Clerk) */}
      <div className="w-full space-y-2">
        <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider text-center mb-1">
          Standard Access
        </div>
        <button
          onClick={handleExitToClerk}
      ...
      {/* Developer Login (Mock) */}
      <div className="w-full space-y-2">
        <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider text-center mb-1">
          Developer Access
        </div>

        <div className="w-full bg-yellow-50 border border-yellow-200 rounded p-2 mb-2 flex flex-col items-center">
          <p className="text-xs text-yellow-800 font-semibold">Master Account credentials will be used</p>
        </div>
